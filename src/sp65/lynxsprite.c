...
static unsigned char AnalyseNextChunks(signed *newlen, signed len,
                                       char data[32], char ColorBits) {
   char longest = 1;
...
static unsigned char GetNextChunk(signed *newlen, signed len, char data[32],
                                  char ColorBits) {
   char oper = 'A';
...
static void encodeSprite(StrBuf *D, enum Mode M, char ColorBits, char ColorMask,
                         char LineBuffer[512], int len, int LastOpaquePixel) {
   //
      ...
      for (I = 0; I < PenColors; I++) {
         switch (PenPal[I]) {
            ...
            case 'F':
               Map[15] = I;
               break;
               /* The X is reserved as transparency. This allows for shaped
                * sprites */
            case 'x':
            case 'X':
               Map[16] = I;
      ...
      switch ((Quadrant + quad) & 3) {
         case 0:
            ...
            for (Y = OY; Y < (signed)GetBitmapHeight(B); ++Y) {
               ...
               encodeSprite(D, M, ColorBits, ColorMask, LineBuffer, i,
                            LastOpaquePixel);
            }
            if ((OY == 0) && (OX == 0)) {
