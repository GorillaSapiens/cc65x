...
// upper left corner.
{
   ...
   /* Copy the pixel data */
   for (Y = 0; Y < Height; ++Y) {
      memcpy(B->Data + Y * B->Width, O->Data + (OrigY + Y) * O->Width + OrigX,
...
// is returned.
{
   ...
   if (B->Pal) {
      if (P.Index >= B->Pal->Count) {
         ...
         Error("Invalid palette index %u at position %u/%u of \"%*s\"", P.Index,
               X, Y, SB_GetLen(&B->Name), SB_GetConstBuf(&B->Name));
      }
