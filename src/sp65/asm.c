...
/* Write the contents of Data to the given file in assembler (ca65) format */
{
   ...
   fprintf(F,
           ...
           ";\n"
           "\n",
           ProgName, GetVersionAsString(), SB_GetLen(S), SB_GetConstBuf(S),
   ...
   if (Ident) {
      fprintf(F,
              ...
              "        WIDTH  = %u\n"
              "        HEIGHT = %u\n",
              Ident, GetBitmapColors(B), GetBitmapWidth(B), GetBitmapHeight(B));
   }
   ...
   while (Size) {
      ...
      for (I = 0; I < Chunk; ++I) {
         ...
         switch (Base) {
            case 2:
               fprintf(F, "%%%u%u%u%u%u%u%u%u", (V >> 7) & 0x01,
                       (V >> 6) & 0x01, (V >> 5) & 0x01, (V >> 4) & 0x01,
                       (V >> 3) & 0x01, (V >> 2) & 0x01, (V >> 1) & 0x01,
                       (V >> 0) & 0x01);
               break;
