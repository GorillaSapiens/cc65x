...
// Read a structured PCX header from the given file and return it
{
   ...
   if (P->Compressed > 1) {
      Error("Unsupported compression (%d) in PCX file '%s'", P->Compressed,
            Name);
   }
   // We support:
   ...
   if (P->PalInfo != 1 && P->PalInfo != 2) {
      Error("Unsupported palette info (%u) in PCX file '%s'", P->PalInfo, Name);
   }
   if (!ValidBitmapSize(P->Width, P->Height)) {
      Error("PCX file '%s' has an unsupported size (w=%u, h=%d)", Name,
            P->Width, P->Height);
   }
