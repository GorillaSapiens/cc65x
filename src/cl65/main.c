...
static void DisableAssembling(void) { DoAssemble = 0; }

static void DisableLinking(void) { DoLink = 0; }
...
static void RemoveTempFiles(void) {
   ...
   for (I = 0; I < RM.FileCount; ++I) {
      if (remove(RM.Files[I]) < 0) {
         Warning("Cannot remove temporary file '%s': %s", RM.Files[I],
                 strerror(errno));
      }
   }
}
...
static void AssembleFile(const char *File, const char *TmpFile,
                         unsigned ArgCount)
...
static void OptAsmIncludeDir(const char *Opt attribute((unused)),
                             const char *Arg)
...
static void OptBinIncludeDir(const char *Opt attribute((unused)),
                             const char *Arg)
...
static void OptRegisterSpace(const char *Opt attribute((unused)),
                             const char *Arg)
