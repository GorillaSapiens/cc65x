...
/* Add a label */
{
   ...
   if (ExistingAttr != atNoLabel) {
      ...
      Error("Duplicate label for address $%04X (%s): '%s'", Addr,
            L ? L->Name : "<unnamed label>", Name ? Name : "<unnamed label>");
   }
...
void AddDepLabel(uint32_t Addr, attr_t Attr, const char *BaseName,
                 unsigned Offs)
...
static void AddLabelRange(uint32_t Addr, attr_t Attr, const char *Name,
                          unsigned Count)
...
static int CompareLabels(void *Data attribute((unused)), const void *L1,
                         const void *L2)
