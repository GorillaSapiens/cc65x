...
static void RangeCheck(const char *Opt, unsigned long Val, unsigned long Min,
                       unsigned long Max)
...
// the size from the table needs to be added to make up for the correct value
{
   if (D->Flags & flSizeChanges) {
      if (CPU == CPU_45GS02) {
         ...
         if ((D->Handler == OH_Immediate65816M && GetAttr(PC) & atMem16) ||
             (D->Handler == OH_Immediate65816X && GetAttr(PC) & atIdx16)) {
            PC++;
         }
      }
   }
