...
// a pointer to the other occurrence, if we didn't find it, return NULL.
{
   ...
   unsigned Hash = ((Val >> 24) & 0xFF) ^ ((Val >> 16) & 0xFF) ^
...
static void PrintLineInfo(FILE *F, const Collection *LineInfos,
                          const char *Format)
...
// Print the debug symbols in a debug file
{
   ...
   for (I = 0; I < CollCount(&ObjDataList); ++I) {
      ...
      for (J = 0; J < CollCount(&O->DbgSyms); ++J) {
         ...
         fprintf(F, "sym\tid=%u,name=\"%s\",addrsize=%s", O->SymBaseId + J,
                 GetString(S->Name), AddrSizeToStr((unsigned char)S->AddrSize));
   ...
   for (I = 0; I < CollCount(&ObjDataList); ++I) {
      ...
      for (J = 0; J < CollCount(&O->HLLDbgSyms); ++J) {
         ...
         fprintf(F, "csym\tid=%u,name=\"%s\",scope=%u,type=%u,sc=",
                 O->HLLSymBaseId + J, GetString(S->Name),
                 O->ScopeBaseId + S->ScopeId, S->Type);
         switch (SC) {
            case HLL_SC_AUTO:
               fputs("auto", F);
               break;
