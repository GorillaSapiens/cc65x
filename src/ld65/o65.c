...
/* Write a "size" word to the file */
{
   switch (D->Header.Mode & MF_SIZE_MASK) {
      ...
      case MF_SIZE_32BIT:
         ...
         break;
      default:
         Internal("Invalid size in header: %04X", D->Header.Mode);
   }
}
...
static const SegDesc *FindSeg(SegDesc **const List, unsigned Count,
                              const Segment *S)
...
static O65Option *NewO65Option(unsigned Type, const void *Data,
                               unsigned DataLen)
...
{
   static unsigned char Trailer[5] = {0x01, 0x00, 0x6F, 0x36, 0x35};
...
// table.
{
   ...
   Expr = E;
   if (E->Op == EXPR_BYTE0 || E->Op == EXPR_BYTE1 || E->Op == EXPR_BYTE2 ||
       E->Op == EXPR_BYTE3 || E->Op == EXPR_WORD0 || E->Op == EXPR_WORD1 ||
       E->Op == EXPR_FARADDR || E->Op == EXPR_DWORD || E->Op == EXPR_NEARADDR) {
   ...
   RefCount = (ED.MemRef != 0) + (ED.SegRef != 0) + (ED.SecRef != 0) +
              (ED.ExtRef != 0);
   if (RefCount > 1) {
      ED.TooComplex = 1;
   }
   ...
   switch (E->Op) {
      case EXPR_BYTE0:
         ...
         break;
      case EXPR_BYTE1:
         BinVal = (BinVal >> 8) & 0xFF;
         break;
      case EXPR_BYTE2:
         BinVal = (BinVal >> 16) & 0xFF;
         break;
      case EXPR_BYTE3:
         BinVal = (BinVal >> 24) & 0xFF;
         break;
      case EXPR_WORD0:
         BinVal &= 0xFFFF;
         break;
      case EXPR_WORD1:
         BinVal = (BinVal >> 16) & 0xFFFF;
         break;
      case EXPR_FARADDR:
         BinVal &= 0xFFFFFFUL;
         break;
...
/* Write one segment to the o65 output file */
{
   ...
   if ((D->Header.Mode & MF_SIZE_MASK) == MF_SIZE_16BIT &&
       D->SegSize > 0xFFFF) {
...
/* Set the executable alignment */
{
   ...
   switch (Alignment) {
      case 1:
         D->Header.Mode |= MF_ALIGN_1;
         break;
      case 2:
         D->Header.Mode |= MF_ALIGN_2;
         break;
      case 4:
         D->Header.Mode |= MF_ALIGN_4;
         break;
      case 256:
         D->Header.Mode |= MF_ALIGN_256;
         break;
      default:
         Error("Invalid alignment for O65 format: %u", Alignment);
   }
}
      ...
      for (J = 0; J < CollCount(&M->SegList); ++J) {
         ...
         switch (O65SegType(S)) {
            ...
            case O65SEG_ZP:
               ...
               break;
            default:
               Internal("Invalid return from O65SegType");
         }
      }
   }
...
/* Write an o65 output file */
{
   ...
   if (D->Undef > 0) {
      ...
      Error("%u unresolved external(s) found - cannot create output file",
            D->Undef);
   }
