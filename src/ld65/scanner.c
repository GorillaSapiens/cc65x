...
/* Print a warning message adding file name and line number of a given file */
{
   ...
   Warning("%s:%u: %s", GetString(Pos->Name), Pos->Line, SB_GetConstBuf(&Buf));
   SB_Done(&Buf);
...
/* Print an error message adding file name and line number of a given file */
{
   ...
   Error("%s:%u: %s", GetString(Pos->Name), Pos->Line, SB_GetConstBuf(&Buf));
   SB_Done(&Buf);
}
      ...
      switch (C) {
         ...
         case '%':
            ...
            switch (C) {
               ...
               default:
                  CfgWarning(&CfgErrorPos, "Unknown escape sequence '%%%c'", C);
                  SB_AppendChar(&CfgSVal, '%');
...
/* Map an identifier to one of the special tokens in the table */
{
   ...
   if (CfgTok == CFGTOK_IDENT) {
      ...
      /* Not found */
      CfgError(&CfgErrorPos, "%s expected, got '%s'", Name,
