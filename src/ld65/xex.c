...
static unsigned XexWriteExpr(ExprNode *E, int Signed, unsigned Size,
                             unsigned long Offs attribute((unused)), void *Data)
...
static void XexStartSegment(XexDesc *D, unsigned long Addr,
                            unsigned long Size) {
         ...
         if (S->Flags & SF_ALIGN) {
            ...
            if (DoWrite || (M->Flags & MF_FILL) != 0) {
               ...
               if (S->Flags & SF_OVERWRITE) {
                  Error("ATARI file format does not support overwrite for "
                        "segment '%s'.",
                        GetString(S->Name));
               }
               else {
