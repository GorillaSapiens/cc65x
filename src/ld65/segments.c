...
Section *NewSection(Segment *Seg, unsigned long Alignment,
...
// Read a section from a file
{
   ...
   // Print some data
   Print(stdout, 2,
         "Module '%s': Found segment '%s', size = %u, alignment = %lu, type = "
         "%u\n",
   ...
   if (Sec->Alignment > 1) {
      ...
      if (Alignment > MAX_ALIGNMENT) {
         Error("Combined alignment for segment '%s' is %lu which exceeds "
               ...
               GetString(Name), Alignment, MAX_ALIGNMENT, GetObjFileName(O));
      }
      else if (Alignment >= LARGE_ALIGNMENT && Alignment > S->Alignment &&
               Alignment > Sec->Alignment && !LargeAlignment) {
...
{
   static const unsigned long U_Hi[4] = {0x000000FFUL, 0x0000FFFFUL,
                                         0x00FFFFFFUL, 0xFFFFFFFFUL};
   static const long S_Hi[4] = {0x0000007FL, 0x00007FFFL, 0x007FFFFFL,
                                0x7FFFFFFFL};
   static const long S_Lo[4] = {~0x0000007FL, ~0x00007FFFL, ~0x007FFFFFL,
                                ~0x7FFFFFFFL};
...
void SegWrite(const char *TgtName, FILE *Tgt, Segment *S, SegWriteFunc F,
              void *Data)
...
// Output the segments to the debug file
{
   ...
   for (I = 0; I < CollCount(&SegmentList); ++I) {
      ...
      // Print the segment data
      fprintf(F,
              "seg\tid=%u,name=\"%s\",start=0x%06lX,size=0x%04lX,addrsize=%s,"
              "type=%s",
              S->Id, GetString(S->Name), S->PC, S->Size,
              AddrSizeToStr(S->AddrSize), S->ReadOnly ? "ro" : "rw");
      if (S->OutputName) {
         fprintf(F, ",oname=\"%s\",ooffs=%lu", S->OutputName, S->OutputOffs);
      }
      if (S->MemArea) {
