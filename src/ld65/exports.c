...
static Export *NewExport(unsigned Type, unsigned char AddrSize, unsigned Name,
                         ObjData *Obj);
   ...
   if (I->AddrSize == ADDR_SIZE_DEFAULT || I->AddrSize > ADDR_SIZE_LONG) {
      ...
      if (ObjHasFiles(I->Obj)) {
         ...
         Error("Invalid import size in for '%s', imported from %s:%u: 0x%02X",
               GetString(I->Name), GetSourceName(LI), GetSourceLine(LI),
               I->AddrSize);
      }
      else {
         Error("Invalid import size in for '%s', imported from %s: 0x%02X",
               GetString(I->Name), GetObjFileName(I->Obj), I->AddrSize);
      }
   }
...
// Generate a new import with the given name and address size and return it
{
   ...
   if (I->AddrSize == ADDR_SIZE_DEFAULT || I->AddrSize > ADDR_SIZE_LONG) {
      ...
      // import
      Error("Invalid import size 0x%02X for symbol '%s'", I->AddrSize,
            GetString(I->Name));
   }
      ...
      do {
         if (L->Name == E->Name) {
            ...
            if (L->Expr == 0) {
               ...
               Error("Duplicate external identifier: '%s'", GetString(L->Name));
            }
            return;
         }
         Last = L;
   ...
   while (I) {
      if (E->AddrSize != I->AddrSize) {
         ...
         if (E->Obj) {
            ...
            SB_Printf(&ExportLoc, "%s, %s:%u", GetString(E->Obj->Name),
                      GetSourceName(ExportLI), GetSourceLine(ExportLI));
         }
         else if (ExportLI) {
            SB_Printf(&ExportLoc, "%s:%u", GetSourceName(ExportLI),
         ...
         if (I->Obj) {
            ...
            SB_Printf(&ImportLoc, "%s, %s:%u", GetString(I->Obj->Name),
                      GetSourceName(ImportLI), GetSourceLine(ImportLI));
         }
         else if (ImportLI) {
            // The import is linker generated and we have line
            // information
            SB_Printf(&ImportLoc, "%s:%u", GetSourceName(ImportLI),
         ...
         Warning("Address size mismatch for '%s': "
                 ...
                 "import in %s as '%s'",
                 GetString(E->Name), SB_GetConstBuf(&ExportLoc), ExpAddrSize,
                 SB_GetConstBuf(&ImportLoc), ImpAddrSize);
         ...
         while (Imp) {
            ...
            if (count == 0) {
               ...
               for (J = 0; J < count; ++J) {
                  ...
                  fprintf(stderr, "%s:%u: Error: Unresolved external '%s'\n",
...
// Print an export map to the given file
{
   ...
   for (I = 0; I < Count; ++I) {
      ...
      if (E->Expr != 0 &&
         ...
         fprintf(F, "%-25s %06lX %c%c%c%c   ", GetString(E->Name),
                 GetExportVal(E), E->ImpCount ? 'R' : ' ',
         ...
         while (Imp) {
            ...
            if (LI) {
               fprintf(F, "    %-25s %s:%u\n", GetObjFileName(Imp->Obj),
                       GetSourceName(LI), GetSourceLine(LI));
            }
            else {
               fprintf(F, "    %-25s\n", GetObjFileName(Imp->Obj));
            }
...
// Print an error about a circular reference using to define the given export
{
   ...
   Error("Circular reference for symbol '%s', %s:%u", GetString(E->Name),
         GetSourceName(LI), GetSourceLine(LI));
}
