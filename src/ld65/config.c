...
/* Parse a MEMORY section */
{
   static const IdentTok Attributes[] = {
       ...
       {"FILE", CFGTOK_FILE},       {"FILL", CFGTOK_FILL},
       {"FILLVAL", CFGTOK_FILLVAL}, {"SIZE", CFGTOK_SIZE},
       {"START", CFGTOK_START},     {"TYPE", CFGTOK_TYPE},
   };
...
/* Parse a SEGMENTS section */
{
   static const IdentTok Attributes[] = {
       ...
       {"DEFINE", CFGTOK_DEFINE},     {"FILLVAL", CFGTOK_FILLVAL},
       {"LOAD", CFGTOK_LOAD},         {"OFFSET", CFGTOK_OFFSET},
       {"OPTIONAL", CFGTOK_OPTIONAL}, {"RUN", CFGTOK_RUN},
       {"START", CFGTOK_START},       {"TYPE", CFGTOK_TYPE},
   };
   static const IdentTok Types[] = {
       {"RO", CFGTOK_RO},
      ...
      while (CfgTok == CFGTOK_IDENT) {
         ...
         switch (AttrTok) {
            ...
            case CFGTOK_ALIGN:
               ...
               S->RunAlignment =
                   (unsigned)CfgCheckedConstExpr(1, MAX_ALIGNMENT);
               S->Flags |= SF_ALIGN;
               break;
            ...
            case CFGTOK_TYPE:
               ...
               switch (CfgTok) {
                  ...
                  case CFGTOK_BSS:
                     ...
                     break;
                  case CFGTOK_ZP:
                     S->Flags |= (SF_BSS | SF_ZP);
                     break;
                  case CFGTOK_OVERWRITE:
                     S->Flags |= (SF_OVERWRITE | SF_RO);
                     break;
                  default:
...
/* Parse the o65 format section */
{
   static const IdentTok Attributes[] = {
       ...
       {"TYPE", CFGTOK_TYPE},     {"OS", CFGTOK_OS},
       {"ID", CFGTOK_ID},         {"VERSION", CFGTOK_VERSION},
   };
   static const IdentTok Types[] = {
       {"SMALL", CFGTOK_SMALL},
       {"LARGE", CFGTOK_LARGE},
      ...
      switch (AttrTok) {
         ...
         case CFGTOK_OS:
            ...
            if (CfgTok == CFGTOK_INTCON) {
               ...
               CfgSpecialToken(OperatingSystems, ENTRY_COUNT(OperatingSystems),
                               "OS type");
               switch (CfgTok) {
                  case CFGTOK_LUNIX:
                     OS = O65OS_LUNIX;
                  ...
                  case CFGTOK_OPENCBM:
                     ...
                     break;
                  default:
                     CfgError(&CfgErrorPos, "Unexpected OS token");
               }
            }
            CfgNextTok();
            break;
...
/* Parse the CONDES feature */
{
   static const IdentTok Attributes[] = {
       ...
       {"LABEL", CFGTOK_LABEL},     {"ORDER", CFGTOK_ORDER},
       {"SEGMENT", CFGTOK_SEGMENT}, {"TYPE", CFGTOK_TYPE},
   };
   ...
   while (1) {
      ...
      switch (AttrTok) {
         ...
         case CFGTOK_ORDER:
            ...
            switch (CfgTok) {
               case CFGTOK_DECREASING:
                  Order = cdDecreasing;
                  break;
               case CFGTOK_INCREASING:
                  Order = cdIncreasing;
                  break;
               default:
                  FAIL("Unexpected order token");
            }
            break;
         ...
         case CFGTOK_TYPE:
            ...
            if (CfgTok == CFGTOK_INTCON) {
               ...
               CfgSpecialToken(Types, ENTRY_COUNT(Types), "Type");
               switch (CfgTok) {
                  case CFGTOK_CONSTRUCTOR:
                     Type = CD_TYPE_CON;
                     break;
                  case CFGTOK_DESTRUCTOR:
                     Type = CD_TYPE_DES;
                     break;
                  case CFGTOK_INTERRUPTOR:
                     Type = CD_TYPE_INT;
                     break;
                  default:
                     FAIL("Unexpected type token");
               }
            }
            break;
...
/* Parse a symbols section */
{
   ...
   static const IdentTok AddrSizes[] = {
       {"ABS", CFGTOK_ABS},    {"ABSOLUTE", CFGTOK_ABS}, {"DIRECT", CFGTOK_ZP},
       {"DWORD", CFGTOK_LONG}, {"FAR", CFGTOK_FAR},      {"LONG", CFGTOK_LONG},
       {"NEAR", CFGTOK_ABS},   {"ZEROPAGE", CFGTOK_ZP},  {"ZP", CFGTOK_ZP},
   };
...
/* Parse the config file */
{
   static const IdentTok BlockNames[] = {
       ...
       {"SEGMENTS", CFGTOK_SEGMENTS}, {"FORMATS", CFGTOK_FORMATS},
       {"FEATURES", CFGTOK_FEATURES}, {"SYMBOLS", CFGTOK_SYMBOLS},
   };
   cfgtok_t BlockTok;
            ...
            if (S->Flags & SF_ALIGN) {
               ...
               if (S->Flags & SF_OVERWRITE) {
                  ...
                  if (NewAddr < Addr) {
                     ...
                     if (S->Flags & SF_OFFSET) {
                        CfgWarning(GetSourcePos(S->LI),
                                   "Segment '%s' offset is too small in '%s' "
                                   "by %lu byte%s",
                                   GetString(S->Name), GetString(M->Name),
                                   Addr - NewAddr,
                                   (Addr - NewAddr == 1) ? "" : "s");
                        ...
                        CfgWarning(GetSourcePos(S->LI),
                                   "Segment '%s' start address is too low in "
                                   "'%s' by %lu byte%s",
                                   GetString(S->Name), GetString(M->Name),
                                   Addr - NewAddr,
                                   (Addr - NewAddr == 1) ? "" : "s");
