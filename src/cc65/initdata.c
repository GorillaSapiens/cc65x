...
static unsigned ParseInitInternal(Type *T, int *Braces,
                                  int AllowFlexibleMembers);
...
{
   while (CurTok.Tok != TOK_CEOF && CurTok.Tok != TOK_SEMI &&
          BracesExpected >= 0) {
      switch (CurTok.Tok) {
         case TOK_RCURLY:
            --BracesExpected;
...
// Output a data definition for the given expression
{
   ...
   switch (ED_GetLoc(Expr)) {
      ...
      case E_LOC_NONE:
         ...
         // FIXME: float
         if (isfloat) {
            g_defdata_float(CF_IMM | CG_TypeOf(Expr->Type) | CF_CONST,
      ...
      case E_LOC_ABS:
         ...
         g_defdata(CF_ABSOLUTE | CG_TypeOf(Expr->Type) | CF_CONST, Expr->IVal,
                   0);
         break;

      case E_LOC_GLOBAL:
         if (isfloat) {
            printf("%s:%d FIXME: E_LOC_GLOBAL\n", __FILE__, __LINE__);
            exit(-1);
         }
         // Global variable
      ...
      case E_LOC_LITERAL:
         if (isfloat) {
            printf("%s:%d FIXME: E_LOC_LITERAL\n", __FILE__, __LINE__);
            exit(-1);
         }
      ...
      case E_LOC_REGISTER:
         if (isfloat) {
            printf("%s:%d FIXME: E_LOC_REGISTER\n", __FILE__, __LINE__);
            exit(-1);
         }
...
// Parse initializaton for arrays. Return the number of data bytes.
{
   ...
   if (IsDeclRankChar(ElementType) &&
      ...
      if (ElementCount != UNSPECIFIED && ElementCount != FLEXIBLE &&
...
// Parse initialization of a struct or union. Return the number of data bytes.
{
   ...
   while (CurTok.Tok != TOK_RCURLY) {
      ...
      if (SymIsBitField(TagSym)) {
         ...
         // Truncate the initializer value to the width of the bit-field and
         // check if we lost any useful bits.
         ...
         if (IsSignUnsigned(TagSym->Type)) {
            if (Field.IVal < 0 || (unsigned long)Field.IVal != Val) {
               Warning(IsSignUnsigned(Field.Type)
                           ? "Implicit truncation from '%s' to '%s : %u' in "
                       ...
                       GetFullTypeName(TagSym->Type), TagSym->Type->A.B.Width,
                       Field.IVal, Val);
            }
         }
         else {
            ...
            unsigned ShiftBits =
                sizeof(long) * CHAR_BIT - TagSym->Type->A.B.Width;
            ...
            if (Field.IVal != RestoredVal) {
               Warning(IsSignUnsigned(Field.Type)
                       ...
                       GetFullTypeName(Field.Type),
                       GetFullTypeName(TagSym->Type), TagSym->Type->A.B.Width,
                       Field.IVal, RestoredVal);
            }
         }

         // Add the value to the currently stored bit-field value
         Shift =
             (TagSym->V.Offs - SI.Offs) * CHAR_BITS + TagSym->Type->A.B.Offs;
         SI.BitVal |= (Val << Shift);
         ...
         SI.Offs +=
             ParseInitInternal(TagSym->Type, Braces,
                               AllowFlexibleMembers && TagSym->NextSym == 0);
      }
