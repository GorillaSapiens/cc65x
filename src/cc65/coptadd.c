...
// pla
{
   ...
   while (I < CS_GetEntryCount(S)) {
      ...
      if (L[0]->OPC == OP65_LDY && CE_IsConstImm(L[0]) &&
          !CS_RangeHasLabel(S, I + 1, 5) && CS_GetEntries(S, L + 1, I + 1, 5) &&
          CE_IsCallTo(L[1], "ldaxysp") && CE_IsCallTo(L[2], "pushax") &&
          L[3]->OPC == OP65_LDY && CE_IsConstImm(L[3]) &&
          CE_IsCallTo(L[4], "ldaxysp") && CE_IsCallTo(L[5], "tosaddax")) {
...
// provided that a/x is not used later.
{
   ...
   while (I < CS_GetEntryCount(S)) {
      ...
      if (L[0]->OPC == OP65_LDY && CE_IsConstImm(L[0]) &&
          ...
          CE_IsCallTo(L[1], "ldaxysp") && L[2]->OPC == OP65_LDY &&
          CE_IsConstImm(L[2]) && CE_IsCallTo(L[3], "addeqysp") &&
...
// L1:
{
   ...
   while (I < CS_GetEntryCount(S)) {
      ...
      /* Check for the sequence */
      if (CE_IsCallTo(L[0], "pushax") && CS_GetEntries(S, L + 1, I + 1, 4) &&
          !CS_RangeHasLabel(S, I + 1, 3) && L[1]->OPC == OP65_LDX &&
          CE_IsKnownImm(L[1], 0) && L[2]->OPC == OP65_LDA &&
...
// pla
{
   ...
   while (I < CS_GetEntryCount(S)) {
      ...
      if (CE_IsCallTo(L[0], "pushax") && CS_GetEntries(S, L + 1, I + 1, 3) &&
          !CS_RangeHasLabel(S, I + 1, 3) && L[1]->OPC == OP65_LDA &&
...
// is not used later.
{
   ...
   while (I < CS_GetEntryCount(S)) {
      ...
      /* Check for the sequence */
      if (E->OPC == OP65_JSR && strncmp(E->Arg, "incax", 5) == 0 &&
          IsDigit(E->Arg[5]) && E->Arg[6] == '\0' && !RegXUsed(S, I + 1)) {
...
// and remove the handling of the high byte if X is not used later.
{
   ...
   while (I < CS_GetEntryCount(S)) {
      ...
      /* Check for the sequence */
      if (E->OPC == OP65_ADC && CS_GetEntries(S, L, I + 1, 3) &&
          (L[0]->OPC == OP65_BCC || L[0]->OPC == OP65_JCC) &&
          L[0]->JumpTo != 0 && !CE_HasLabel(L[0]) && L[1]->OPC == OP65_INX &&
          !CE_HasLabel(L[1]) && L[0]->JumpTo->Owner == L[2] &&
          !RegXUsed(S, I + 3)) {
