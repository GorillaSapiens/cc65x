...
/* Compare the types regarding their qualifiers. Return via pointer *Result */
{
   ...
   if (IsTypeFunc(lhst)) {
      if ((LeftQual & T_QUAL_CCONV) == T_QUAL_NONE) {
         LeftQual |= (AutoCDecl || IsVariadicFunc(lhst)) ? T_QUAL_CDECL
                                                         : T_QUAL_FASTCALL;
      }
   }
   if (IsTypeFunc(rhst)) {
      if ((RightQual & T_QUAL_CCONV) == T_QUAL_NONE) {
         RightQual |= (AutoCDecl || IsVariadicFunc(rhst)) ? T_QUAL_CDECL
                                                          : T_QUAL_FASTCALL;
      }
   }
   ...
   if ((RightQual & T_QUAL_ADDRSIZE) == T_QUAL_NONE) {
      RightQual |= (IsTypeFunc(rhst) ? CodeAddrSizeQualifier()
                                     : DataAddrSizeQualifier());
   }
      ...
      switch (LeftRank) {
         case T_RANK_PTR:
            ...
            if (Result->Indirections == 1) {
               if ((GetUnderlyingTypeCode(lhs + 1) & T_MASK_RANK) ==
                   T_RANK_VOID) {
                  Result->F |= TCF_VOID_PTR_ON_LEFT;
               }
               if ((GetUnderlyingTypeCode(rhs + 1) & T_MASK_RANK) ==
                   T_RANK_VOID) {
