...
// Check if this is one of the allowed suboperation for OptPtrStore1
{
   ...
   if (L[0]->OPC == OP65_AND || L[0]->OPC == OP65_EOR ||
      ...
      return 1;
            ...
            L[1]->OPC == OP65_ADC && !CE_HasLabel(L[1])) {
      return 2;
   }
   else if (L[0]->OPC == OP65_SEC && (L[1] = CS_GetNextEntry(S, I)) != 0 &&
            L[1]->OPC == OP65_SBC && !CE_HasLabel(L[1])) {
      return 2;
   }
...
// return NULL.
{
   ...
   if (I >= 2 && CS_GetEntries(S, L, I - 2, 2) && L[0]->OPC == OP65_LDA &&
   ...
   while (I < CS_GetEntryCount(S)) {
      ...
      if (L[0]->OPC == OP65_CLC && CS_GetEntries(S, L + 1, I + 1, 8) &&
          ...
          (L[1]->AM == AM65_ABS || L[1]->AM == AM65_ZP ||
           L[1]->AM == AM65_IMM ||
           (L[1]->AM == AM65_ZP_INDY && RegValIsKnown(L[1]->RI->In.RegY))) &&
          (L[2]->OPC == OP65_BCC || L[2]->OPC == OP65_JCC) &&
          L[2]->JumpTo != 0 && L[2]->JumpTo->Owner == L[4] &&
          L[3]->OPC == OP65_INX && CE_IsCallTo(L[4], "pushax") &&
          L[5]->OPC == OP65_LDX && L[6]->OPC == OP65_LDA &&
          L[7]->OPC == OP65_LDY && CE_IsKnownImm(L[7], 0) &&
          CE_IsCallTo(L[8], "staspidx") && !CS_RangeHasLabel(S, I + 1, 3) &&
...
// depending on the code preceeding the sequence above.
{
   ...
   while (I < CS_GetEntryCount(S)) {
      ...
      if (L[0]->OPC == OP65_CLC && CS_GetEntries(S, L + 1, I + 1, 9) &&
          ...
          L[2]->JumpTo != 0 && L[2]->JumpTo->Owner == L[4] &&
          L[3]->OPC == OP65_INX && CE_IsCallTo(L[4], "pushax") &&
          L[5]->OPC == OP65_LDY && CE_IsConstImm(L[5]) &&
          L[6]->OPC == OP65_LDX && L[7]->OPC == OP65_LDA &&
          L[7]->AM == AM65_ZP_INDY && strcmp(L[7]->Arg, "c_sp") == 0 &&
          L[8]->OPC == OP65_LDY &&
          (L[8]->AM == AM65_ABS || L[8]->AM == AM65_ZP ||
           L[8]->AM == AM65_IMM) &&
          CE_IsCallTo(L[9], "staspidx") && !CS_RangeHasLabel(S, I + 1, 3) &&
...
// use the register bank instead of ptr1.
{
   ...
   while (I < CS_GetEntryCount(S)) {
      ...
      if (CE_IsCallTo(L[0], "pushax") && CS_GetEntries(S, L + 1, I + 1, 3) &&
          ...
          L[3 + K]->OPC == OP65_LDY && CE_IsConstImm(L[3 + K]) &&
          !CE_HasLabel(L[3 + K]) && CE_IsCallTo(L[4 + K], "staspidx") &&
          !CE_HasLabel(L[4 + K])) {
         ...
         if (I > 1) {
            ...
            if (P[0]->OPC == OP65_LDA && P[0]->AM == AM65_ZP &&
                P[1]->OPC == OP65_LDX && P[1]->AM == AM65_ZP &&
                !CE_HasLabel(P[1]) && strncmp(P[0]->Arg, "regbank+", 8) == 0) {
