...
// Remove repeated loads from one and the same memory location
{
   ...
   while (I < CS_GetEntryCount(S)) {
      ...
      if (E->Info & OF_LOAD) {
         ...
         if (Load != 0 && E->OPC == Load->OPC && E->AM == Load->AM &&
             ((E->Arg == 0 && Load->Arg == 0) ||
      ...
      switch (E->OPC) {
         ...
         case OP65_STY:
            ...
            if (RegValIsKnown(In->RegY) &&          // Value of Y is known
               ...
               else if (In->RegY == In->RegX && E->AM != AM65_ABSX &&
                        E->AM != AM65_ZPX) {
                  NewOPC = OP65_STX;
               }
            }
...
// Remove a store followed by a load from the same location.
{
   ...
   while (I < CS_GetEntryCount(S)) {
      ...
      if ((E->Info & OF_STORE) != 0 && (N = CS_GetNextEntry(S, I)) != 0 &&
          !CE_HasLabel(N) && E->AM == N->AM &&
...
// Remove an 8 bit load followed by a store into the same location.
{
   ...
   while (I < CS_GetEntryCount(S)) {
      ...
      // same address.
      if ((E->Info & OF_LOAD) != 0 && (N = CS_GetNextEntry(S, I)) != 0 &&
          !CE_HasLabel(N) && E->AM == N->AM &&
...
// and remove the useless load.
{
   ...
   while (I < CS_GetEntryCount(S)) {
      ...
      if ((L[0]->OPC == OP65_LDA || L[0]->OPC == OP65_LDX ||
          ...
          !CS_RangeHasLabel(S, I + 1, 3) && CS_GetEntries(S, L + 1, I + 1, 2) &&
          (L[1]->OPC == OP65_STA || L[1]->OPC == OP65_STX ||
           L[1]->OPC == OP65_STY) &&
      ...
      if ((E->Info & OF_XFR) != 0 && (N = CS_GetNextEntry(S, I)) != 0 &&
         ...
         if ((E->OPC == OP65_TAX && N->OPC == OP65_TXA &&
             ...
             (E->OPC == OP65_TAY && N->OPC == OP65_TYA &&
              !RegYUsed(S, I + 2)) ||
             (E->OPC == OP65_TXA && N->OPC == OP65_TAX &&
              !RegAUsed(S, I + 2)) ||
             (E->OPC == OP65_TYA && N->OPC == OP65_TAY &&
              !RegAUsed(S, I + 2))) {
      ...
      switch (State) {
         ...
         case FoundXfer:
            ...
            if (E->Info & OF_CBRA) {
               ...
               if ((E->Info & OF_STORE) != 0 && CS_IsBasicBlock(S, Xfer, I)) {
                  Store = I;
                  StoreEntry = E;
                  State = FoundStore;
               }
         ...
         case FoundStore:
            ...
            if ((GetRegInfo(S, I, XferEntry->Chg & REG_ALL) & XferEntry->Chg &
                 REG_ALL) == 0 &&
               ...
               switch (XferEntry->OPC) {
                  ...
                  case OP65_TXA:
                     X = NewCodeEntry(OP65_STX, StoreEntry->AM, StoreEntry->Arg,
                                      0, StoreEntry->LI);
                     break;

                  case OP65_TAX:
                     X = NewCodeEntry(OP65_STA, StoreEntry->AM, StoreEntry->Arg,
                                      0, StoreEntry->LI);
                     break;

                  case OP65_TYA:
                     X = NewCodeEntry(OP65_STY, StoreEntry->AM, StoreEntry->Arg,
                                      0, StoreEntry->LI);
                     break;
      ...
      switch (State) {
         ...
         case FoundLoad:
            ...
            if (E->Info & OF_CBRA) {
               ...
               // block, proceed. Otherwise restart
               if ((E->Info & OF_XFR) != 0 && CS_IsBasicBlock(S, Load, I)) {
                  Xfer = I;
                  XferEntry = E;
                  State = FoundXfer;
               }
         ...
         case FoundXfer:
            ...
            if ((GetRegInfo(S, I, LoadEntry->Chg & REG_ALL) & LoadEntry->Chg &
                 REG_ALL) == 0 &&
               ...
               switch (XferEntry->OPC) {
                  ...
                  case OP65_TXA:
                  case OP65_TYA:
                     X = NewCodeEntry(OP65_LDA, LoadEntry->AM, LoadEntry->Arg,
                                      0, LoadEntry->LI);
                     break;

                  case OP65_TAX:
                     X = NewCodeEntry(OP65_LDX, LoadEntry->AM, LoadEntry->Arg,
                                      0, LoadEntry->LI);
                     break;

                  case OP65_TAY:
                     X = NewCodeEntry(OP65_LDY, LoadEntry->AM, LoadEntry->Arg,
                                      0, LoadEntry->LI);
                     break;
   ...
   while (I < CS_GetEntryCount(S)) {
      ...
      switch (State) {
         ...
         case FoundPop:
            ...
            if (E->OPC == OP65_STA && (E->AM == AM65_ABS || E->AM == AM65_ZP) &&
                !CE_HasLabel(E) &&
      ...
      switch (E->OPC) {
         ...
         case OP65_SBC:
            if (CE_IsKnownImm(E, 0x00)) {
               ...
               int CondC =
                   PStatesAreKnown(In->PFlags, PSTATE_C) &&
                   ((E->OPC == OP65_ADC && (In->PFlags & PFVAL_C) == 0) ||
                    (E->OPC == OP65_SBC && (In->PFlags & PFVAL_C) != 0));
               int CondV = PStatesAreKnown(In->PFlags, PSTATE_V) &&
                           (In->PFlags & PFVAL_V) == 0;
               ...
               /* This is done last as it could change the info used by the two
                * above */
               ...
               // 0 + arg. In this case we need only care about the C/V flags
               // and let the load set the Z/N flags properly.
               int CondC = PStatesAreClear(In->PFlags, PSTATE_C);
               int CondV = PStatesAreClear(In->PFlags, PSTATE_V);
               unsigned R = (CondC ? 0 : REG_A | PSTATE_C) |
                            (CondC && CondV ? 0 : PSTATE_V);
               if (R) {
                  R = GetRegInfo(S, I + 1, R);
               }
               CondV = (CondC && CondV) || (R & PSTATE_V) == 0;
   ...
   while (I < CS_GetEntryCount(S)) {
      ...
      if (L[0]->OPC == OP65_LDA && CS_GetEntries(S, L + 1, I + 1, 4) &&
         ...
         if (L[3]->OPC == OP65_CPX && CE_IsConstImm(L[3]) &&
             (L[4]->Info & OF_CBRA) != 0 &&
             ((L[3]->Num == 0x00 && GetBranchCond(L[4]->OPC) == BC_PL) ||
