...
// and remove the useless load.
{
   ...
   while (I < CS_GetEntryCount(S)) {
      ...
      if (L[0]->OPC == OP65_LDY && CE_IsConstImm(L[0]) && L[0]->Num < 0xFF &&
          !CS_RangeHasLabel(S, I + 1, 3) && CS_GetEntries(S, L + 1, I + 1, 3) &&
          CE_IsCallTo(L[1], "staxysp") && L[2]->OPC == OP65_LDY &&
          CE_IsKnownImm(L[2], L[0]->Num + 1) && CE_IsCallTo(L[3], "ldaxysp")) {
...
// stack.
{
   ...
   while (I < CS_GetEntryCount(S)) {
      ...
      /* Check for the call */
      if (CE_IsCallTo(E, "staxysp") && RegValIsKnown(RI->In.RegA) &&
          RegValIsKnown(RI->In.RegX) && RegValIsKnown(RI->In.RegY) &&
          !RegAXUsed(S, I + 1)) {
...
// stack.
{
   ...
   while (I < CS_GetEntryCount(S)) {
      ...
      /* Check for the call */
      if (CE_IsCallTo(E, "steaxysp") && RegValIsKnown(RI->In.RegA) &&
          RegValIsKnown(RI->In.RegX) && RegValIsKnown(RI->In.RegY) &&
          RegValIsKnown(RI->In.SRegLo) && RegValIsKnown(RI->In.SRegHi) &&
          !RegEAXUsed(S, I + 1)) {
...
// from the load.
{
   ...
   while (I < CS_GetEntryCount(S)) {
      ...
      if (L[0]->OPC == OP65_STA &&
          ...
          !CS_RangeHasLabel(S, I + 1, 3) && CS_GetEntries(S, L + 1, I + 1, 4) &&
          L[1]->OPC == OP65_STX && L[1]->AM == L[0]->AM &&
          L[2]->OPC == OP65_LDA && L[2]->AM == L[0]->AM &&
          L[3]->OPC == OP65_LDX && L[3]->AM == L[1]->AM &&
          strcmp(L[0]->Arg, L[2]->Arg) == 0 &&
          strcmp(L[1]->Arg, L[3]->Arg) == 0 && !CE_UseLoadFlags(L[4])) {
...
// cycles or bytes; but, it keeps the old value of X, which may be reused later.
{
   ...
   while (I < CS_GetEntryCount(S)) {
      ...
      /* Check for the sequence */
      if (L[0]->OPC == OP65_LDA && !CS_RangeHasLabel(S, I + 1, 3) &&
          CS_GetEntries(S, L + 1, I + 1, 3) && L[1]->OPC == OP65_LDX &&
          L[2]->OPC == OP65_STA && L[3]->OPC == OP65_STX &&
          !RegXUsed(S, I + 4)) {
