   ...
   if (ED_IsLVal(Expr)) {
      ...
      if (IsClassFloat(OldType) && IsClassInt(NewType)) {
         ...
         if ((Expr->V.FVal.V != FP_D_FromInt(IVal).V) && !Explicit) {
            Warning("Floating point constant (%f) converted to integer loses "
                    "precision (%ld)",
                    Expr->V.FVal.V, IVal);
         }
         Expr->IVal = IVal;
      ...
      if (NewBits <= OldBits) {
         ...
         if ((OldVal != Expr->IVal) && IS_Get(&WarnConstOverflow) &&
             !Explicit) {
            Warning(
                "Implicit conversion of constant overflows %d-bit destination",
                NewBits);
         }
      }
         ...
         if (Result.C == TC_PTR_SIGN_DIFF) {
            ...
            if (IS_Get(&WarnPointerTypes)) {
               TypeCompatibilityDiagnostic(
                   NewType, Expr->Type, 0,
                   "Incompatible pointer conversion to '%s' from '%s'");
            }
         }
   ...
   if (HasError) {
      ...
      if (!IsIncompleteESUType(NewType) && !IsIncompleteESUType(Expr->Type)) {
         ...
         if (IsIncompleteESUType(NewType)) {
            Error("Conversion to incomplete type '%s'",
                  GetFullTypeName(NewType));
         }
         else {
            Error("Conversion from incomplete type '%s'",
...
/* Handle an explicit cast. */
{
   ...
   if (IsCastType(NewType)) {
      if (!IsIncompleteESUType(NewType)) {
         ...
         Error("Cast to incomplete type '%s'", GetFullTypeName(NewType));
      }
   }
   else {
