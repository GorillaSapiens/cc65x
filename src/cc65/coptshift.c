...
// L1:
{
   ...
   while (I < CS_GetEntryCount(S)) {
      ...
      if (E->OPC == OP65_JSR && (Shift = GetShift(E->Arg)) != SHIFT_NONE &&
         ...
         else if (E->RI->In.RegX == 0 && Count == 1 &&
...
// then their shifts are combined.
{
   ...
   while (I < CS_GetEntryCount(S)) {
      ...
      if ((L[0]->OPC == OP65_BPL || L[0]->OPC == OP65_BCC ||
           L[0]->OPC == OP65_JPL || L[0]->OPC == OP65_JCC) &&
          L[0]->JumpTo != 0 && CS_GetEntries(S, L + 1, I + 1, 3) &&
          L[1]->OPC == OP65_DEX && L[0]->JumpTo->Owner == L[2] &&
          !CS_RangeHasLabel(S, I, 2) && L[2]->OPC == OP65_JSR &&
         ...
         if (Count * 100 <= S->CodeSizeFactor && !RegXUsed(S, I + K)) {
...
// must be added.
{
   ...
   while (I < CS_GetEntryCount(S)) {
      ...
      if ((L[0]->OPC == OP65_BCC || L[0]->OPC == OP65_JCC) &&
          L[0]->JumpTo != 0 && L[0]->RI->In.RegX == 0 &&
          CS_GetEntries(S, L + 1, I + 1, 2) && L[1]->OPC == OP65_INX &&
          L[0]->JumpTo->Owner == L[2] && !CS_RangeHasLabel(S, I, 2) &&
          L[2]->OPC == OP65_JSR &&
...
// insns if the value of X is zero.
{
   ...
   while (I < CS_GetEntryCount(S)) {
      ...
      // Check for the sequence
      if (E->OPC == OP65_JSR && (Shift = GetShift(E->Arg)) != SHIFT_NONE &&
          SHIFT_DIR(Shift) == SHIFT_DIR_RIGHT && E->RI->In.RegX == 0) {
...
// or similar, provided that a/x is not used later
{
   ...
   while (I < CS_GetEntryCount(S)) {
      ...
      if (L[0]->OPC == OP65_LDA &&
          ...
          SHIFT_COUNT(ShiftType) == 1 && L[3]->OPC == OP65_STA &&
