...
// Print an error message about an invalid operand type
{
   ...
   if ((type & CF_TYPEMASK) == CF_FLOAT) {
      ...
      Internal("Invalid type in CF flags: %04X, type = %u", type,
               type & CF_TYPEMASK);
   }
}
...
static const char *GetLabelName(unsigned Flags, uintptr_t Label, long Offs) {
   ...
   switch (Flags & CF_ADDRMASK) {
      ...
      case CF_STATIC:
         ...
         if (Offs) {
            xsprintf(Buf, sizeof(Buf), "%s%+ld", LocalDataLabelName(Label),
                     Offs);
         }
         else {
      ...
      case CF_LITERAL:
         ...
         // Static memory cell
         if (Offs) {
            xsprintf(Buf, sizeof(Buf), "%s%+ld", PooledLiteralLabelName(Label),
                     Offs);
         }
         else {
      ...
      case CF_ABSOLUTE:
         // Absolute address
         xsprintf(Buf, sizeof(Buf), "$%04X",
                  (unsigned)((Label + Offs) & 0xFFFF));
         break;
...
// Generate the assembler code preamble
{
   ...
   switch (CPU) {
      case CPU_6502:
         AddTextLine("\t.setcpu\t\t\"6502\"");
         break;
      case CPU_6502X:
         AddTextLine("\t.setcpu\t\t\"6502X\"");
         break;
      case CPU_6502DTV:
         AddTextLine("\t.setcpu\t\t\"6502DTV\"");
         break;
      case CPU_65SC02:
         AddTextLine("\t.setcpu\t\t\"65SC02\"");
         break;
      case CPU_65C02:
         AddTextLine("\t.setcpu\t\t\"65C02\"");
         break;
      case CPU_W65C02:
         AddTextLine("\t.setcpu\t\t\"W65C02\"");
         break;
      case CPU_65CE02:
         AddTextLine("\t.setcpu\t\t\"65CE02\"");
         break;
      case CPU_65816:
         AddTextLine("\t.setcpu\t\t\"65816\"");
         break;
      case CPU_HUC6280:
         AddTextLine("\t.setcpu\t\t\"HUC6280\"");
         break;
      case CPU_4510:
         AddTextLine("\t.setcpu\t\t\"4510\"");
         break;
      case CPU_45GS02:
         AddTextLine("\t.setcpu\t\t\"45GS02\"");
         break;
      default:
         Internal("Unknown CPU: %d", CPU);
   }
   ...
   AddTextLine("\t.importzp\t"
               "c_sp, sreg, regsave, regbank");
...
// If debug info is enabled, place a file info into the source
{
   if (DebugInfo) {
      ...
      TS_AddLine(GS->Text, "\t.dbg\t\tfile, \"%s\", %lu, %lu", Name, Size,
                 MTime);
   }
}
...
// Define label as an alias for baselabel+offs
{
   ...
   SB_Terminate(&L);
   AddDataLine("%s\t:=\t%s+%ld", SB_GetConstBuf(&L),
               PooledLiteralLabelName(baselabel), offs);
   SB_Done(&L);
}
...
// Return new flags for integral promotions for types smaller than int.
{
   ...
   if ((flags & CF_TYPEMASK) == CF_CHAR) {
      /* int can represent all unsigned chars, so unsigned char is promoted to
       * int. */
      flags &= ~CF_TYPEMASK;
      flags &= ~CF_UNSIGNED;
      flags |= CF_INT;
      ...
      // int cannot represent all unsigned shorts, so unsigned short is promoted
      // to unsigned int.
      flags &= ~CF_TYPEMASK;
      flags |= CF_INT;
      return flags;
...
// in (e)ax. The return value is the flags value for the resulting type.
{
   ...
   // Before we apply the integral promotions, we check if both types are the
   // same character type. If so, we return that type, rather than int, which
   // would be returned by the standard rules.  This is only a performance
   // optimization allowing the use of unsigned and/or char operations; it does
   // not affect correctness, as the flags are only used for code generation,
   // and not to determine types of other expressions containing this one.  For
   // codgen, CF_CHAR means the operands are char and the result is int (unless
   // CF_FORCECHAR is also set, in which case the result is char).  This special
   // case part is not duplicated by ArithmeticConvert.
...
// Scale the value in the primary register by the given value. If val is
// positive, scale up, is val is negative, scale down. This function is used to
...
// Primary = TOS + Primary
{
   static const char *const ops[4] = {"tosaddax", "tosaddax", "tosaddeax",
                                      "tosaddeax"};
...
// Primary = TOS - Primary
{
   static const char *const ops[4] = {"tossubax", "tossubax", "tossubeax",
                                      "tossubeax"};
...
// Primary = Primary - TOS
{
   static const char *const ops[4] = {"tosrsubax", "tosrsubax", "tosrsubeax",
                                      "tosrsubeax"};
   oper(flags, val, ops);
}
...
// Primary = TOS * Primary
{
   static const char *const ops[4] = {"tosmulax", "tosumulax", "tosmuleax",
                                      "tosumuleax"};
   ...
   if (flags & CF_CONST) {
      ...
      if (p2 == 0 ||
          (p2 > 0 && IS_Get(&CodeSizeFactor) >= (Negation ? 100 : 0))) {
...
// Primary = TOS / Primary
{
   static const char *const ops[4] = {"tosdivax", "tosudivax", "tosdiveax",
                                      "tosudiveax"};
   ...
   if (flags & CF_CONST) {
      ...
      // cost of code size
      if (p2 == 0 ||
          (p2 > 0 && IS_Get(&CodeSizeFactor) >= (Negation ? 200 : 170))) {
...
// Primary = TOS % Primary
{
   static const char *const ops[4] = {"tosmodax", "tosumodax", "tosmodeax",
                                      "tosumodeax"};
   int p2;

   // Check if we can do some cost reduction
   if ((flags & CF_CONST) && (flags & CF_UNSIGNED) && val != 0xFFFFFFFF &&
       (p2 = PowerOf2(val)) >= 0) {
...
// Primary = TOS | Primary
{
   static const char *const ops[4] = {"tosorax", "tosorax", "tosoreax",
                                      "tosoreax"};
...
// Primary = TOS ^ Primary
{
   static const char *const ops[4] = {"tosxorax", "tosxorax", "tosxoreax",
                                      "tosxoreax"};
...
// Primary = TOS & Primary
{
   static const char *const ops[4] = {"tosandax", "tosandax", "tosandeax",
                                      "tosandeax"};
...
// Primary = TOS >> Primary
{
   static const char *const ops[4] = {"tosasrax", "tosshrax", "tosasreax",
                                      "tosshreax"};
      ...
      switch (flags & CF_TYPEMASK) {
         case CF_CHAR:
            if (flags & CF_FORCECHAR) {
               val &= 7;
               if ((flags & CF_UNSIGNED) != 0) {
                  // Instead of `val` right shifts, we can also do `9 - val`
                  // left rotates and a mask.  This saves 3 bytes and 8 cycles
                  // for `val == 7` and 1 byte and 4 cycles for `val == 6`.
                  if (val < 6) {
                     while (val--) {
                     ...
                     unsigned i;
                     // The first ROL shifts in garbage and sets carry to the
                     // high bit. The garbage is cleaned up by the mask.
                     for (i = val; i < 9; ++i) {
...
// Primary = TOS << Primary
{
   static const char *const ops[4] = {"tosaslax", "tosshlax", "tosasleax",
                                      "tosshleax"};
   ...
   if (flags & CF_CONST) {
      switch (flags & CF_TYPEMASK) {
         case CF_CHAR:
            if ((flags & CF_FORCECHAR) != 0) {
               val &= 7;
               // Large shifts are faster and smaller with ROR.  See g_asr for
               // detailed byte and cycle counts.
               if (val < 6) {
                  while (val--) {
                     AddCodeLine("asl a");
                  }
...
// Test for equal
{
   static const char *const ops[4] = {"toseqax", "toseqax", "toseqeax",
                                      "toseqeax"};
...
// Test for not equal
{
   static const char *const ops[4] = {"tosneax", "tosneax", "tosneeax",
                                      "tosneeax"};
...
// Test for less than
{
   static const char *const ops[4] = {"tosltax", "tosultax", "toslteax",
                                      "tosulteax"};
...
{
   static const char *const ops[4] = {"tosleax", "tosuleax", "tosleeax",
                                      "tosuleeax"};
...
{
   static const char *const ops[4] = {"tosgtax", "tosugtax", "tosgteax",
                                      "tosugteax"};
...
{
   static const char *const ops[4] = {"tosgeax", "tosugeax", "tosgeeax",
                                      "tosugeeax"};
...
void g_testbitfield(ATTR_UNUSED(unsigned Flags), unsigned BitOffs,
                    unsigned BitWidth)
// Test bit-field in primary.
{
   /* Since the end is inclusive and cannot be negative here, we subtract 1 from
    * the sum */
   ...
   /* We don't use these flags for now. Could CF_NOKEEP be potentially
    * interesting? */
   Flags &= ~CF_STYPEMASK;

   // If we need to do a test, then we avoid shifting (ASR only shifts one bit
   // at a time, so is slow) and just AND the head and tail bytes with the
   // appropriate mask, then OR the results with the rest bytes.
   if (Bytes == 1) {
   ...
   switch (BitOffs / CHAR_BITS) {
      ...
      case 2:
         ...
         break;
      case 3:
         /* In this case we'd have HeadMask == TailMask and only 1 byte, but
          * anyways... */
         if (HeadMask != 0xFF || TailMask == 0xFF) {
            AddCodeLine("lda sreg+1");
            UntestedBytes &= ~0x8;
   ...
   if (HeadMask != 0xFF) {
      ...
      /* Abuse the "Byte 0" flag so that this head content will be saved by the
       * routine */
      UntestedBytes |= 0x1;
   }
   ...
   if (TailMask != 0xFF) {
      // If we have to do any more masking operation, register A will be used
      // for that, and its current content in it must be saved.
...
// Extract bits from bit-field in primary.
{
   ...
   // To zero-extend, we will and by the width if the field doesn't end on a
   // char or int boundary.  If it does end on a boundary, then zeros will have
   // already been shifted in, but we need to clear the high byte for char.
   // g_and emits no code if the mask is all ones. This is here so the signed
   // and unsigned branches can use it.
   if (EndBit == CHAR_BITS) {
   ...
   if (IsSigned) {
      ...
      if (ZeroExtendMask != 0) {
         ...
         if (SignBitByte + 1U == sizeofarg(FullWidthFlags)) {
            /* We can just sign-extend on the high byte if it is the only
             * affected one */
            unsigned char SignBitMask = (1UL << SignBitPosInByte) & 0xFF;
            unsigned char Mask =
                ((2UL << (SignBitPos % CHAR_BITS)) - 1UL) & 0xFF;
         ...
         if (SignBitByte + 1U == sizeofarg(FullWidthFlags)) {
            ...
            switch (SignBitByte) {
               case 0:
                  ...
                  // We could jump over the following tya instead, but that
                  // wouldn't be faster than taking this extra tay and then the
                  // tya.
                  AddCodeLine("tay");
                  break;
               case 1:
            ...
            // Since we are going to get back .A later anyways, we may just do
            // the op on the higher bytes with whatever content currently in it.
            unsigned long Mask = ~((2UL << SignBitPos) - 1UL);
            g_or(FullWidthFlags | CF_CONST, Mask);
         }

         // Get back .A. We need to duplicate the TYA, rather than move it
         // before the branch to share with the other label, because TYA changes
