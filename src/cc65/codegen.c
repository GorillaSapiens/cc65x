...
// Print an error message about an invalid operand type
{
   ...
   if ((type & CF_TYPEMASK) == CF_FLOAT) {
      ...
      Internal("%s:%d Invalid type in CF flags: %04X, type = %u", func, line,
               type, type & CF_TYPEMASK);
   }
}
   ...
   if ((Flags & CF_CONST) != 0) {
      ...
      switch (Flags & CF_TYPEMASK) {
         ...
         case CF_FLOAT: // FIXME float - handle like long here
                        // CAUTION: make sure Val contains the float value in
                        // raw binary format fall through
         case CF_LONG:
...
// 4       --> Operate on floats (CAUTION: Val must be a float in raw binary
// format)
{
   int n = 0;
...
// Push the primary register or a constant value onto the stack
{
   ...
   switch (flags & CF_TYPEMASK) {
      ...
      case CF_FLOAT: // FIXME: float - handle like long here
         ...
         {
            g_getimmed(flags | CF_CONST, FP_D_As32bitRaw(FP_D_Make(val)),
                       0); // ?? FIXME
         }
         AddCodeLine("jsr pusheax");
         break;
...
{
   static const char *const ops[OPER_IDX_NUM] = {
       "tosrsubax", "tosrsubax", "tosrsubeax", "tosrsubeax", "ftosrsubeax"};
   oper(flags, val, ops);
}
...
{
   static const char *const ops[OPER_IDX_NUM] = {
       "tosmulax", "tosumulax", "tosmuleax", "tosumuleax", "ftosmuleax"};
   ...
   if (flags & CF_CONST) {
      ...
      switch (flags & CF_TYPEMASK) {
         ...
         case CF_FLOAT: // FIXME: float: is it the right thing here to do the
                        // same as LONG?
...
{
   static const char *const ops[OPER_IDX_NUM] = {
       "tosdivax", "tosudivax", "tosdiveax", "tosudiveax", "ftosdiveax"};
...
// Primary = TOS | Primary
{
   static const char *const ops[OPER_IDX_NUM] = {"tosorax", "tosorax",
                                                 "tosoreax", "tosoreax", NULL};
...
{
   static const char *const ops[OPER_IDX_NUM] = {
       "toseqax", "toseqax", "toseqeax", "toseqeax", "ftoseqeax"};
...
{
   static const char *const ops[OPER_IDX_NUM] = {
       "tosneax", "tosneax", "tosneeax", "tosneeax", "ftosneeax"};
...
{
   static const char *const ops[OPER_IDX_NUM] = {
       "tosltax", "tosultax", "toslteax", "tosulteax", "ftoslteax"};
...
{
   static const char *const ops[OPER_IDX_NUM] = {
       "tosleax", "tosuleax", "tosleeax", "tosuleeax", "ftosleeax"};
...
{
   static const char *const ops[OPER_IDX_NUM] = {
       "tosgtax", "tosugtax", "tosgteax", "tosugteax", "ftosgteax"};
...
{
   static const char *const ops[OPER_IDX_NUM] = {
       "tosgeax", "tosugeax", "tosgeeax", "tosugeeax", "ftosgeeax"};
      ...
      switch (flags & CF_TYPEMASK) {
         ...
         case CF_FLOAT:
            // FIXME: float
            {
               ...
               AddDataLine("\t.dword\t$%02x%02x%02x%02x\t; float", p[3], p[2],
                           p[1], p[0]);
            }
            break;
