               ...
               if (IsTypeVoid(Decl.Type)) {
                  ...
                  if (Sym->V.BssName != 0) {
                     if (strcmp(Sym->V.BssName, bssName) != 0) {
                        Error("Global variable '%s' already was defined in the "
                              "'%s' segment",
                  ...
                  // This is to make the automatical zeropage setting of the
                  // symbol work right.
                  g_usebss();
               }
            }
      ...
      for (Entry = GetGlobalSymTab()->SymHead; Entry; Entry = Entry->NextSym) {
         ...
         if ((Entry->Flags & (SC_TU_STORAGE | SC_DEF)) == SC_TU_STORAGE) {
            ...
            if ((Entry->Flags & SC_STORAGEMASK) == SC_STATIC &&
               ...
               Warning("Inline function '%s' %s but never defined", Entry->Name,
