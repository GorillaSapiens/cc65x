   ...
   while (Param) {
      ...
      if (RequireAll && IsIncompleteESUType(Param->Type)) {
         if (D->Flags & FD_UNNAMED_PARAMS) {
            Error("Parameter %u has incomplete type '%s'", D->ParamCount - I,
                  GetFullTypeName(Param->Type));
         }
         else {
            Error("Parameter '%s' has incomplete type '%s'", Param->Name,
                  GetFullTypeName(Param->Type));
         }
         ++IncompleteCount;
      }
...
/* Return true if this is an old style (K&R) function with an implicit int
   return */
{
...
/* Emit debug infos for the current function */
{
   if (DebugInfo) {
      ...
      AddTextLine("\t.dbg\tfunc, \"%s\", \"00\", %s, \"%s\"", Sym->Name,
                  (Sym->Flags & SC_EXTERN) ? "extern" : "static", Sym->AsmName);
   }
}
...
/* Parse argument declarations and function body. */
{
   ...
   if (D->ParamCount > 0 && IsFastcallFunc(Func->Type)) {
      ...
      if (IsClassStruct(D->LastParam->Type)) {
         Flags = CG_TypeOf(GetStructReplacementType(D->LastParam->Type)) |
                 CF_FORCECHAR;
      }
      else {
   ...
   if (ParamComplete) {
      ...
      while (Param && (Param->Flags & SC_PARAM) != 0) {
         ...
         if (IsClassStruct(RType)) {
            ...
            if (RType == Param->Type) {
               Error("Passing '%s' of this size (%d) by value is not supported",
                     GetFullTypeName(Param->Type), SizeOf(RType));
            }
         }
   ...
   if (!F_HasVoidReturn(CurrentFunc) && !F_HasReturn(CurrentFunc)) {
      ...
      if (F_IsMainFunc(CurrentFunc) && IS_Get(&Standard) >= STD_C99 &&
