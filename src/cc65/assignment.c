...
// Process inc/dec for bit-field
{
   ...
   // Determine the type to operate on the whole byte chunk containing the
   // bit-field
         ...
         if (Gen->Func == g_add) {
            ...
            if (!ED_IsUneval(Expr)) {
               if (Expr2.IVal == 0) {
                  ...
                  // If the shift count is greater than or equal to the width of
                  // the promoted left operand, the behaviour is undefined
                  // according to the standard.
                  if (Expr2.IVal < 0) {
                     Warning("Negative shift count %ld treated as %u for %s",
                             Expr2.IVal, (unsigned)Expr2.IVal & (ExprBits - 1),
                             GetBasicTypeName(CalType));
                  }
                  else if (Expr2.IVal >= (long)ExprBits) {
                     Warning("Shift count %ld >= width of %s treated as %u",
                             Expr2.IVal, GetBasicTypeName(CalType),
                  ...
                  if (Expr2.IVal >= (long)Expr->Type->A.B.Width) {
                     Warning("Shift count %ld >= width of bit-field",
                             Expr2.IVal);
                  }
               }
            }
            ...
            if (Gen->Func == g_div || Gen->Func == g_mod) {
               ...
               if (Expr->Type->A.B.Width < INT_BITS ||
                   IsSignSigned(Expr->Type)) {
                  AdjustedFlags = (Flags & ~CF_UNSIGNED) | CF_CONST;
                  AdjustedFlags = g_typeadjust(
                      AdjustedFlags, CG_TypeOf(Expr2.Type) | CF_CONST);
               }
               Gen->Func(g_typeadjust(Flags, AdjustedFlags) | CF_CONST,
                         Expr2.IVal);
            }
            else {
      ...
      if (Gen != 0) {
         ...
         if (Gen->Func == g_div || Gen->Func == g_mod) {
            ...
            if (Expr->Type->A.B.Width < INT_BITS || IsSignSigned(Expr->Type)) {
               AdjustedFlags = (Flags & ~CF_UNSIGNED) | CF_CONST;
               AdjustedFlags = g_typeadjust(AdjustedFlags,
                                            CG_TypeOf(Expr2.Type) | CF_CONST);
            }
...
static void OpAssignArithmetic(const GenDesc *Gen, ExprDesc *Expr,
                               const char *Op)
            ...
            if (!ED_IsUneval(Expr)) {
               if (Expr2.IVal == 0 && !ED_IsUneval(Expr)) {
                  ...
                  if (Expr2.IVal >= (long)BitSizeOf(Expr->Type)) {
                     Warning("Shift count %ld >= width of %s", Expr2.IVal,
                             GetBasicTypeName(Expr->Type));
                  }
               }
            }
   ...
   if (IsClassStruct(ltype) ? Gen != 0 : !IsScalarType(ltype)) {
      ...
      if (IsIncompleteESUType(ltype)) {
         ...
         if (IsTypeArray(ltype)) {
            ...
            Error("Function type '%s' is not assignable",
                  GetFullTypeName(ltype));
         }
         else {
      ...
      case E_LOC_CODE:
         ...
         if (IsClassFloat(Expr->Type)) {
            ...
            if (Gen->Tok == TOK_PLUS_ASSIGN) {
               g_addeqstatic(lflags, Expr->Name, Expr->IVal,
                             FP_D_As32bitRaw(Expr2.V.FVal));
            }
            else {
               g_subeqstatic(lflags, Expr->Name, Expr->IVal,
                             FP_D_As32bitRaw(Expr2.V.FVal));
            }
         }
         else {
