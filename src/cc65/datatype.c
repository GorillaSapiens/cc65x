...
// Return GetUnqualRawTypeCode (Type) if Type is not scalar.
{
   ...
   if (IsDeclTypeChar(Type)) {
      ...
      switch (TCode & T_MASK_SIZE) {
         case T_SIZE_INT:
            Underlying |= T_RANK_INT;
            break;
         case T_SIZE_LONG:
            Underlying |= T_RANK_LONG;
            break;
         case T_SIZE_SHORT:
            Underlying |= T_RANK_SHORT;
            break;
         case T_SIZE_CHAR:
            Underlying |= T_RANK_CHAR;
            break;
         case T_SIZE_LONGLONG:
            Underlying |= T_RANK_LONGLONG;
            break;
         default:
            Underlying |= T_RANK_INT;
            break;
...
// string may be T if there is no need to change it.
{
   ...
   if (IsTypeBitField(T)) {
      ...
      // - If an int can represent all values of the bit-field, the bit-field is
      // converted to an int;
      // - Otherwise, if an unsigned int can represent all values of the
      // bit-field, the bit-field is converted to an unsigned int;
      // - Otherwise, the bit-field will have its declared integer type.
      // These rules are borrowed from C++ and seem to be consistent with
      // GCC/Clang's.
      if (T->A.B.Width > INT_BITS) {
         return IsSignUnsigned(T) ? type_ulong : type_long;
      }
      return T->A.B.Width == INT_BITS && IsSignUnsigned(T) ? type_uint
                                                           : type_int;
   }
   else if (IsRankChar(T)) {
      // An integer can represent all values from either signed or unsigned
      // char, so convert chars to int.
      return type_int;
   }
   else if (IsRankShort(T)) {
      // An integer cannot represent all values from unsigned short, so convert
      ...
      // Otherwise, this is an incomplete enum, and there is expceted to be an
      // error already. Assume int to avoid further errors.
      return type_int;
   }
}
...
/* Perform the usual arithmetic conversions for binary operators. */
{
   ...
   // There are additional rules for floating point types that we don't bother
   // with, since floating point types are not (yet) supported. The integral
...
/* Get the underlying type of an enum or other integer class type */
{
   if (IsDeclTypeChar(Type)) {
      ...
      switch (GetMinimalTypeSizeByBitWidth(Type->A.B.Width)) {
         case SIZEOF_CHAR:
            Type = IsSignSigned(Type) ? type_schar : type_uchar;
            break;
         case SIZEOF_INT:
            Type = IsSignSigned(Type) ? type_int : type_uint;
            break;
         case SIZEOF_LONG:
            Type = IsSignSigned(Type) ? type_long : type_ulong;
            break;
         default:
            Type = IsSignSigned(Type) ? type_int : type_uint;
            break;
...
/* Get the type needed to operate on the byte chunk containing the bit-field */
{
   ...
   if (ChunkSize < SizeOf(Type + 1)) {
      ...
      switch (ChunkSize) {
         case SIZEOF_CHAR:
            return IsSignSigned(Type) ? type_schar : type_uchar;
         case SIZEOF_INT:
            return IsSignSigned(Type) ? type_int : type_uint;
         case SIZEOF_LONG:
            return IsSignSigned(Type) ? type_long : type_ulong;
         default:
            return IsSignSigned(Type) ? type_int : type_uint;
      }
   }
...
// Check fails if the type is not a function or a pointer to function.
{
   ...
   return !IsVariadicFunc(T) &&
          (AutoCDecl ? IsQualFastcall(T) : !IsQualCDecl(T));
}
...
/* Return a non-const pointer to the return type of a function or
...
static void GetParameterList(StrBuf *ParamList, StrBuf *Buf, const FuncDesc *D,
                             int Detailed) {
   /* First argument */
   ...
   if ((D->Flags & FD_OLDSTYLE) == 0) {
      ...
      for (I = 0; I < D->ParamCount; ++I) {
         ...
         SB_AppendStr(ParamList,
                      SB_GetConstBuf(GetFullTypeNameBuf(Buf, Param->Type)));
         SB_Clear(Buf);
         /* Next argument */
         Param = Param->NextSym;
      }
...
GetFullTypeNameWestEast(struct StrBuf *West, struct StrBuf *East, const Type *T)
...
// eastern part.
{
   ...
   if (IsTypeArray(T)) {
      ...
      if (SB_IsEmpty(West)) {
         ...
         SB_Printf(West, "%s(%s)", SB_GetConstBuf(&Buf),
                   SB_GetConstBuf(&ParamList));
      }
      else {
...
int GetQualifierTypeCodeNameBuf(struct StrBuf *S, TypeCode Qual,
                                TypeCode IgnoredQual)
...
/* Print a function signature */
{
   ...
   SB_Printf(&West, "%s%s (%s)", SB_GetConstBuf(&Buf), Name,
             SB_GetConstBuf(&ParamList));
