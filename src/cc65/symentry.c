...
/* Dump the given symbol table entry to the file in readable form */
{
   ...
   static SCFlagTable Types[] = {
       {"SC_NONE", SC_NONE},       {"SC_STRUCT", SC_STRUCT},
       {"SC_UNION", SC_UNION},     {"SC_ENUM", SC_ENUM},
       {"SC_LABEL", SC_LABEL},     {"SC_BITFIELD", SC_BITFIELD},
       {"SC_TYPEDEF", SC_TYPEDEF}, {"SC_ENUMERATOR", SC_ENUMERATOR},
       {"SC_FUNC", SC_FUNC},       {"SC_ARRAY", SC_ARRAY},
   };
   ...
   for (I = 0; I < sizeof(Properties) / sizeof(Properties[0]) && SymFlags != 0;
        ++I) {
...
/* Return true if this is a function that must be output */
{
   ...
   return IsTypeFunc(Sym->Type) && SymIsDef(Sym) &&
          ((Sym->Flags & SC_REF) || (Sym->Flags & SC_STORAGEMASK) != SC_STATIC);
}
