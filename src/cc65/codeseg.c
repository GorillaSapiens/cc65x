...
/* Pick a reference label and move it to index 0 in E. */
{
   ...
   unsigned LabelCount = CE_GetLabelCount(E);
   CHECK(LabelCount > 0);
   // Use either the first one as reference label, or a label with a Ref that
   // has no JumpTo. This is a hack to partially work around #1211.  Refs with
   // no JumpTo are labels used in data segments.  (They are not tracked.)  If a
   // data segment is the only reference, the label will be pruned away, but the
   // data reference will remain, causing linking to fail.
   ...
   for (I = 1; I < LabelCount; ++I) {
      ...
      for (J = 0; J < RefCount; ++J) {
         ...
         if (EJ->JumpTo == NULL) {
            /* Move it to the beginning since it's simpler to handle the removal
             * this way. */
...
static const char *ReadToken(const char *L, const char *Term, char *Buf,
                             unsigned BufSize)
...
// white space, for example.
{
   ...
   // function later.
   if ((E->Info & OF_CALL) == 0 && (E->ArgInfo & AIF_HAS_NAME) != 0) {
      ArgBase = E->ArgBase;
...
int CS_GetEntries(CodeSeg *S, struct CodeEntry **List, unsigned Start,
                  unsigned Count)
...
// one and adjust references accordingly.
{
   ...
   for (I = 0; I < CS_GetEntryCount(S); ++I) {
      ...
      // Pick a label to keep, all references will be moved to this one, and the
      // other labels will be deleted.  PickRefLab moves this to index 0.
      RefLab = PickRefLab(E);
...
/* emit an Error on anything other variable definition. */
{
      ...
      if (E->OPC == OP65_PHA) {
         ...
         static struct {
            ...
            bool CheckForSP;
             ...
             {OP65_PHA, AM65_IMP, false}, {OP65_LDA, AM65_ZP, true},
             {OP65_SEC, AM65_IMP, false}, {OP65_SBC, AM65_IMM, false},
             {OP65_STA, AM65_ZP, true},   {OP65_LDA, AM65_ZP, true},
             {OP65_SBC, AM65_IMM, false}, {OP65_STA, AM65_ZP, true},
             {OP65_PLA, AM65_IMP, false},
         };
         for (J = 0; J < sizeof(signature) / sizeof(signature[0]); ++J) {
            if (I + J <= Last) {
               CodeEntry *F = CS_GetEntry(S, I + J);
               if (F->OPC != signature[J].OPC || F->AM != signature[J].AM ||
   ...
   out:
      ...
      if (WantError) {
         Error("unreachable code at %s:%d not supported", E->LI->File->Name,
               E->LI->ActualLineNum);
         return;
      }
   }
...
void CS_CleanupSwitch(CodeSeg *S, unsigned location) {
   ...
   if (E0->OPC == OP65_JSR && E1->OPC == OP65_JSR &&
       !strcmp(E0->Arg, "saveeax") && !strcmp(E1->Arg, "resteax")) {
...
/* Generate register infos for all instructions */
{
   ...
   do {
      ...
      for (I = 0; I < CS_GetEntryCount(S); ++I) {
         ...
         if (LabelCount == 0 && (E->Info & OF_ZBRA) != 0 &&
             (P = CS_GetPrevEntry(S, I)) != 0) {
