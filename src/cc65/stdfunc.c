...
static struct StdFuncDesc {
   ...
   void (*Handler)(FuncDesc *, ExprDesc *);
    ...
    // BEGIN SORTED.SH
    {"memcpy", StdFunc_memcpy}, {"memset", StdFunc_memset},
    {"strcmp", StdFunc_strcmp}, {"strcpy", StdFunc_strcpy},
...
// Handle the memcpy function
{
   ...
   if (IS_Get(&InlineStdFuncs)) {
      ...
      if (ED_IsConstAbsInt(&Arg3.Expr) && Arg3.Expr.IVal <= 256 &&
          ED_IsConstAddr(&Arg2.Expr) && ED_IsStackAddr(&Arg1.Expr) &&
         ...
         if (Arg3.Expr.IVal <= 129 && !AllowOneIndex) {
            ...
            if (Offs == 0) {
               AddCodeLine("ldy #$%02X",
                           (unsigned char)(Offs + Arg3.Expr.IVal - 1));
               g_defcodelabel(Label);
...
// Handle the memset function
{
   ...
   if (IS_Get(&InlineStdFuncs)) {
      ...
      if (ED_IsConstAbsInt(&Arg3.Expr) && Arg3.Expr.IVal <= 256 &&
          ED_IsConstAbsInt(&Arg2.Expr) && ED_IsStackAddr(&Arg1.Expr) &&
          ED_GetStackOffs(&Arg1.Expr, Arg3.Expr.IVal) < 256) {
...
// Handle the strcmp function
{
   ...
   if (IS_Get(&InlineStdFuncs)) {
      ...
      // more efficient code.
      if (ED_IsLocLiteral(&Arg2.Expr) && IS_Get(&WritableStrings) == 0 &&
...
// Handle the strcpy function
{
   ...
   if (IS_Get(&InlineStdFuncs)) {
      ...
      if (ED_IsStackAddr(&Arg2.Expr) && StackPtr >= -255 &&
          ED_IsConstAddr(&Arg1.Expr)) {
      ...
      if (ED_IsConstAddr(&Arg2.Expr) && ED_IsStackAddr(&Arg1.Expr) &&
          StackPtr >= -255) {
...
// Handle the strlen function
{
   ...
   if (IS_Get(&Optimize)) {
      ...
      if (ED_IsLocLiteral(&Arg) && IS_Get(&WritableStrings) == 0) {
         ...
         size_t Len =
             strnlen(GetLiteralStr(Arg.V.LVal), GetLiteralSize(Arg.V.LVal) - 1);
   ...
   if (IS_Get(&InlineStdFuncs)) {
      ...
      if (ED_IsZPInd(&Arg) && IsPtr && IS_Get(&EagerlyInlineFuncs)) {
