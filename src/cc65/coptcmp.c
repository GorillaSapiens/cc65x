...
//
{
   return (L[0]->OPC == OP65_CPX && L[0]->AM == AM65_IMM &&
           CE_HasNumArg(L[0]) && !CE_HasLabel(L[0]) &&
           (L[1]->OPC == OP65_JNE || L[1]->OPC == OP65_BNE) &&
           L[1]->JumpTo != 0 && !CE_HasLabel(L[1]) && L[2]->OPC == OP65_CMP &&
           L[2]->AM == AM65_IMM && CE_HasNumArg(L[2]) &&
           (L[3]->Info & OF_CBRA) != 0 && L[3]->JumpTo != 0 &&
...
// and remove the cmp.
{
   ...
   while (I < CS_GetEntryCount(S)) {
      ...
      if ((L[0]->OPC == OP65_ADC || L[0]->OPC == OP65_AND ||
           ...
           L[0]->OPC == OP65_LDA || L[0]->OPC == OP65_LSR ||
           L[0]->OPC == OP65_ORA || L[0]->OPC == OP65_PLA ||
           L[0]->OPC == OP65_SBC || L[0]->OPC == OP65_TXA ||
           L[0]->OPC == OP65_TYA) &&
          !CS_RangeHasLabel(S, I + 1, 2) && CS_GetEntries(S, L + 1, I + 1, 2) &&
          L[1]->OPC == OP65_CMP && CE_IsKnownImm(L[1], 0)) {
         ...
         if (L[2]->OPC == OP65_JSR) {
            ...
            CodeEntry *E;
            CodeEntry *N;
            if ((E = CS_GetNextEntry(S, I + 2)) != 0 && L[2]->JumpTo != 0 &&
                (N = L[2]->JumpTo->Owner) != 0 && N->OPC != OP65_BCC &&
                N->OPC != OP65_BCS && N->OPC != OP65_JCC &&
                N->OPC != OP65_JCS &&
                (N->OPC != OP65_JSR || FindBoolCmpCond(N->Arg) == CMP_INV)) {
      ...
      if ((E->Info & OF_CMP) != 0 && (RegVal = GetCmpRegVal(E)) >= 0 &&
         ...
         while ((N = CS_GetNextEntry(S, I)) != 0 && // Followed by something..
            ...
            if (!Cond) {
               ...
               CodeEntry *X =
                   NewCodeEntry(OP65_JMP, AM65_BRA, LabelName, L, N->LI);
               CS_InsertEntry(S, X, I + 2);
               CS_DelEntry(S, I + 1);
...
// flag instead of the carry flag and remove the asl.
{
   ...
   while (I < CS_GetEntryCount(S)) {
      ...
      if (L[0]->OPC == OP65_SBC && CS_GetEntries(S, L + 1, I + 1, 4) &&
          ...
          L[1]->JumpTo != 0 && L[1]->JumpTo->Owner == L[3] &&
          L[2]->OPC == OP65_EOR && CE_IsKnownImm(L[2], 0x80) &&
          L[3]->OPC == OP65_ASL && L[3]->AM == AM65_ACC &&
          (L[4]->OPC == OP65_BCC || L[4]->OPC == OP65_BCS ||
