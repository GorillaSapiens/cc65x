...
void CS_MoveEntries(CodeSeg *S, unsigned Start, unsigned Count,
                    unsigned NewPos);
...
#else
#define CS_GetEntry(S, Index)                                                  \
   ((struct CodeEntry *)CollAt(&(S)->Entries, (Index)))
#endif
...
#define CS_ResetAllMarks(S)                                                    \
   ((CS_GetEntryCount(S) > 0) ? CS_ResetMarks(S, 0, CS_GetEntryCount(S))       \
                              : (void)0)
#endif
