...
// Return true if the token is a function specifier
{
   return (T->Tok == TOK_INLINE) || (T->Tok == TOK_FASTCALL) ||
          (T->Tok == TOK_CDECL) || (T->Tok == TOK_NEAR) || (T->Tok == TOK_FAR);
}
...
// Parse a numeric constant token
{
   ...
   if (SB_Peek(&Src) == '0') {
      ...
      else if (toupper(SB_Peek(&Src)) == 'B' && IS_Get(&Standard) >= STD_CC65 &&
   ...
   while ((C = SB_Peek(&Src)) != '\0' &&
          (Base <= 10 ? IsDigit(C) : IsXDigit(C))) {
      SB_Skip(&Src);
   }
   ...
   if (Overflow) {
      Error("Numerical constant \"%s\" too large for internal %d-bit "
            "representation",
   ...
   if (!IsFloat) {
      ...
      if (IVal > 0x7FFF) {
         ...
         if (IVal <= 0xFFFF && (Types & IT_UINT) == 0 &&
             (WarnTypes & IT_LONG) != 0) {
      ...
      if (SB_Peek(&Src) == '.') {
         ...
         Scale = ScaleDigit;
         while (IsXDigit(SB_Peek(&Src)) &&
                (DigitVal = HexVal(SB_Peek(&Src))) < Base) {
      ...
      if ((Base == 16 && toupper(SB_Peek(&Src)) == 'P') ||
         ...
         // Decimal exponents are base 10, hexadecimal exponents are base 2
         // (C99).
         if (Exp) {
            FVal = FP_D_Mul(FVal, FP_D_Make(pow((Base == 16) ? 2.0 : 10.0,
                                                (Sign ? -1.0 : 1.0) * Exp)));
         }
      }
...
// Consume an opening parenthesis/bracket/curly brace and remember that
{
   switch (Tok) {
      ...
      case TOK_LCURLY:
         ...
         break;
      default:
         Internal("Unexpected opening token: %02X", (unsigned)Tok);
   }
...
// If this exits at a non-EOF without consuming it.
{
   ...
   static const token_t TokenList[] = {TOK_COMMA,  TOK_SEMI,   TOK_LPAREN,
                                       TOK_RPAREN, TOK_LBRACK, TOK_RBRACK,
...
// is reached. Return 0 If this exits at an EOF. Otherwise return -1.
{
   ...
   // Some fix point tokens that are used for error recovery
   static const token_t TokenList[] = {TOK_LPAREN, TOK_RPAREN, TOK_LBRACK,
                                       TOK_RBRACK, TOK_LCURLY, TOK_RCURLY};
