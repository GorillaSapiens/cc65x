...
int ParseOpcArgStr(const char *Arg, unsigned short *ArgInfo,
                   struct StrBuf *Name, long *Offset)
// Break the opcode argument string into a symbol name/label part plus an
// offset. Both parts are optional, but if there are any characters in the
// string that can't be parsed, it's an failure. The caller is responsible for
// managing the StrBuf. Return whether parsing succeeds or not.
{
...
// Parse the argument string and memorize the result for the code entry
{
   ...
   if (ParseOpcArgStr(E->Arg, &E->ArgInfo, &B, &E->ArgOff)) {
      ...
      /* Parsing fails. Issue an error/warning so that this could be spotted and
       * fixed. */
      E->ArgBase = EmptyArg;
      SB_Done(&B);
      if (Debug) {
...
static void BranchDeduceOnProcessorFlag(RegContents *True, RegContents *False,
                                        unsigned short PTrueFlag)
...
// It simply returns TRUE in most cases for now.
{
   ...
   if (E->AM == AM65_ZP || E->AM == AM65_ABS ||
       (E->AM == AM65_ZPX && RegValIsKnown(In->RegX) &&
        (Index = In->RegX & 0xFF)) ||
       (E->AM == AM65_ZPY && RegValIsKnown(In->RegY) &&
        (Index = In->RegY & 0xFF)) ||
       (E->AM == AM65_ABSX && RegValIsKnown(In->RegX) &&
        (Index = In->RegX & 0xFF)) ||
       (E->AM == AM65_ABSY && RegValIsKnown(In->RegY) &&
        (Index = In->RegY & 0xFF))) {
      return 1;
   }
   else if ((E->AM == AM65_ZP_IND) ||
            (E->AM == AM65_ZPX_IND && RegValIsKnown(In->RegX) &&
             (Index = In->RegX & 0xFF)) ||
            (E->AM == AM65_ZP_INDY && RegValIsKnown(In->RegY) &&
             (Index = In->RegY & 0xFF))) {
      return 1;
   }
   return 1;
...
// overwritten.
{
   ...
   switch (E->OPC) {
      ...
      case OP65_ADC:
         ...
         Out->PFlags |= UNKNOWN_PFVAL_CZVN;
         Out->ZNRegs = ZNREG_A;
         if (PStatesAreKnown(In->PFlags, PSTATE_C) && RegValIsKnown(In->RegA)) {
            if (CE_IsConstImm(E)) {
      ...
      case OP65_BIT:
         ...
         // Note the V/N flags are not affected in imm addressing mode supported
         // by 65c02!
         if (E->AM == AM65_IMM) {
...
// Output the code entry to the output file
{
   ...
   if (Debug) {
      char Use[128];
      char Chg[128];
      WriteOutput("%*s; USE: %-15s CHG: %-15s SIZE: %u", (int)(30 - Chars), "",
                  RegInfoDesc(E->Use, Use), RegInfoDesc(E->Chg, Chg), E->Size);

      if (E->RI) {
         char RegIn[32];
         char RegOut[32];
         WriteOutput("   In %s   Out %s", RegContentDesc(&E->RI->In, RegIn),
