...
/* Prepare a LoadRegInfo struct for use */
{
   ...
   if ((LRI->Flags & (LI_CHECK_Y | LI_RELOAD_Y)) ==
           (LI_CHECK_Y | LI_RELOAD_Y) &&
       (LRI->LoadYEntry == 0 || (LRI->LoadYEntry->Use & REG_Y) == REG_Y)) {
      LRI->Flags &= ~LI_DIRECT;
   }
}
...
// If any part of the arg is used, it could be unsafe to add such a store before
// E. If any part of the arg is changed, it could be unsafe to add such a load
// after E.
{
   fncls_t fncls;
   unsigned int Use;
   ...
   if (E->AM == AM65_ACC || E->AM == AM65_BRA || E->AM == AM65_IMM ||
       E->AM == AM65_IMP) {
      goto L_Result;
   }
   CHECK((LRI->Flags & LI_CHECK_ARG) == 0 || LRI->LoadIndex < 0 ||
         LRI->LoadEntry != 0);
   CHECK((LRI->Flags & (LI_CHECK_Y | LI_RELOAD_Y)) == 0 ||
         LRI->LoadYIndex < 0 || LRI->LoadYEntry != 0);
      ...
      if ((E->Info & (OF_READ | OF_WRITE)) != 0) {
         ...
         if (E->AM == AM65_ABS || E->AM == AM65_ZP ||
            ...
            if ((LRI->Flags & LI_CHECK_Y) != 0) {
               ...
               if (YE == 0 || (YE->ArgOff == E->ArgOff &&
                               strcmp(YE->ArgBase, E->ArgBase) == 0)) {
...
L_Result:
   if ((LRI->Flags & LI_RELOAD_Y) != 0 && (E->Use & REG_Y) != 0) {
      Res |= LI_Y_USE;
   }
   if ((LRI->Flags & LI_CHECK_Y) != 0 && (E->Chg & REG_Y) != 0) {
      Res |= LI_Y_CHG;
   }
...
static void HonourUseAndChg(LoadRegInfo *LRI, unsigned Reg, const CodeEntry *E,
                            int I)
...
// Set the load src flags and remember to check for load src change if
// necessary. Note: this doesn't assume reloading Y.
{
   if (E->AM == AM65_IMM) {
   ...
   else if ((E->AM == AM65_ZP_INDY) && strcmp(E->Arg, "c_sp") == 0) {
   ...
   if (E->Info & OF_LOAD) {
      ...
      switch (E->OPC) {
         ...
         case OP65_TXS:
            return Used;
         default:
            Internal("Unknown XFR insn in TrackLoads");
      }
...
// OpIndex is adjusted according to the insertions.
{
         ...
         if ((R & REG_Y) != 0) {
            ...
            if ((R & PSTATE_ZN) != 0 && (R & ~(REG_Y | PSTATE_ZN)) == 0) {
               ...
               if ((N = CS_GetNextEntry(D->Code, I)) != 0 &&
                  ...
                  CodeEntry *X =
                      NewCodeEntry(OP65_LDY, AM65_IMM, Arg, 0, E->LI);
                  InsertEntry(D, X, I + 2);
                  ...
                  /* This could be suboptimal but it will always work (unless
                   * stack overflows) */
   ...
   if ((D->Rhs.A.Flags & (LI_RELOAD_Y | LI_SP | LI_CHECK_Y)) ==
       (LI_RELOAD_Y | LI_SP)) {
      D->Rhs.A.Offs -= Offs;
   }
   if ((D->Rhs.X.Flags & (LI_RELOAD_Y | LI_SP | LI_CHECK_Y)) ==
       (LI_RELOAD_Y | LI_SP)) {
      D->Rhs.X.Offs -= Offs;
   }
}
...
// If the value pushed is that of a zeropage variable that is unchanged until
// Op, replace ZPLo and ZPHi in the given StackOpData struct by the variable and
// return true. Otherwise leave D untouched and return false.
{
   ...
   if ((LI->A.Flags & LI_DIRECT) != 0) {
      ...
      if ((LI->A.Flags & LI_RELOAD_Y) == 0) {
         ...
         if ((LI->A.Flags & LI_CHECK_Y) == 0) {
            /* ldy #offs */
            X = NewCodeEntry(OP65_LDY, AM65_IMM, MakeHexArg(LI->A.Offs), 0,
                             D->OpEntry->LI);
         }
         else {
            /* ldy src */
            X = NewCodeEntry(OP65_LDY, LI->A.LoadYEntry->AM,
                             LI->A.LoadYEntry->Arg, 0, D->OpEntry->LI);
         }
         InsertEntry(D, X, D->IP++);
         ...
         if (LI->A.LoadEntry->OPC == OP65_JSR) {
            ...
            /* opc src,y */
            X = NewCodeEntry(OPC, LI->A.LoadEntry->AM, LI->A.LoadEntry->Arg, 0,
                             D->OpEntry->LI);
         }
         InsertEntry(D, X, D->IP++);
      }
   ...
   if ((LI->X.Flags & LI_DIRECT) != 0) {
      ...
      if ((LI->X.Flags & LI_RELOAD_Y) == 0) {
         ...
         if ((LI->X.Flags & LI_CHECK_Y) == 0) {
            /* ldy #const */
            X = NewCodeEntry(OP65_LDY, AM65_IMM, MakeHexArg(LI->X.Offs), 0,
                             D->OpEntry->LI);
         }
         else {
            /* ldy src */
            X = NewCodeEntry(OP65_LDY, LI->X.LoadYEntry->AM,
                             LI->X.LoadYEntry->Arg, 0, D->OpEntry->LI);
         }
         InsertEntry(D, X, D->IP++);
         ...
         if (LI->X.LoadEntry->OPC == OP65_JSR) {
            ...
            /* opc src,y */
            X = NewCodeEntry(OPC, LI->X.LoadEntry->AM, LI->X.LoadEntry->Arg, 0,
                             D->OpEntry->LI);
         }
         InsertEntry(D, X, D->IP++);
      }
...
/* Remove register load insns */
{
   ...
   if (LI->A.LoadEntry != 0 && (LI->A.Flags & LI_RELOAD_Y) != 0 &&
       LI->A.LoadYIndex >= 0) {
      /* If an entry is using Y and not removed, then its Y load mustn't be
       * removed */
   ...
   if (LI->X.LoadEntry != 0 && (LI->X.Flags & LI_RELOAD_Y) != 0 &&
       LI->X.LoadYIndex >= 0) {
      /* If an entry is using Y and not removed, then its Y load mustn't be
       * removed */
   ...
   if (GetFuncInfo(E->Arg, &Use, &Chg) == FNCLS_BUILTIN) {
      ...
      if ((Use & REG_SP) != 0 &&
          ((Use & (SLV_IND | SLV_TOP)) != SLV_IND ||
           RegValIsUnknown(E->RI->In.RegY) || E->RI->In.RegY < PushedBytes)) {
      ...
      void *R = bsearch(E->Arg, Tab, sizeof(Tab) / sizeof(Tab[0]),
                        sizeof(Tab[0]), CmpHarmless);
      return (R != 0);
   }
}
...
/* Remove flagged register load insns */
{
   if ((LRI->Flags & LI_REMOVE) == LI_REMOVE) {
      ...
      if (LRI->ChgIndex >= 0 && (LRI->ChgEntry->Flags & CEF_DONT_REMOVE) == 0) {
         DelEntryIdx(S, LRI->ChgIndex, Indices);
      }
   }

   if (LRI->LoadEntry != 0 && (LRI->Flags & LI_RELOAD_Y) != 0 &&
       LRI->LoadYIndex >= 0) {
      /* If an entry is using Y and not removed, then its Y load mustn't be
       * removed */
      LRI->LoadYEntry->Flags |= CEF_DONT_REMOVE;
...
static int BackupAAt(CodeSeg *S, BackupInfo *B, int Idx, Collection *Indices,
                     int After)
/* Backup the content of A Before or After the specified index Idx depending on
   the After param */
{
   CodeEntry *E;
   CodeEntry *X;
...
static int BackupXAt(CodeSeg *S, BackupInfo *B, int Idx, Collection *Indices,
                     int After)
/* Backup the content of X before or after the specified index Idx depending on
   the param After */
{
   CodeEntry *E;
   CodeEntry *X;
...
static int BackupYAt(CodeSeg *S, BackupInfo *B, int Idx, Collection *Indices,
                     int After)
/* Backup the content of Y before or after the specified index Idx depending on
   the param After */
{
   CodeEntry *E;
   CodeEntry *X;
...
static int BackupAXAt(CodeSeg *S, BackupInfo *B, int Idx, Collection *Indices,
                      int After)
/* Backup the content of AX Before or After the specified index Idx depending on
   the After param */
{
   CodeEntry *E;
   CodeEntry *X;
...
static int BackupAXYAt(CodeSeg *S, BackupInfo *B, int Idx, Collection *Indices,
                       int After)
// Backup the content of AXY before or after the specified index Idx depending
// on the param After. This doesn't allow separating the backup of Y from that
// of AX for now.
{
   CodeEntry *E;
   CodeEntry *X;
   ...
   if (E->RI != 0 && RegValIsKnown(E->RI->In.RegA) &&
       RegValIsKnown(E->RI->In.RegX) && RegValIsKnown(E->RI->In.RegY)) {
      /* Just memorize the value */
      ...
      switch (B->Type & BU_TYPE_MASK) {
         case BU_ZP:
            ...
            X = NewCodeEntry(OP65_STX, AM65_ZP, SB_GetConstBuf(&Arg), 0, E->LI);
            CS_InsertEntry(S, X, Idx++);
            X = NewCodeEntry(OP65_STY, AM65_ZP, GetZPName(B->Where & REG_TMP1),
                             0, E->LI);
            CS_InsertEntry(S, X, Idx++);
            break;
...
/* Restore the content of Y before the specified index Idx */
{
   ...
   switch (B->Type & BU_TYPE_MASK) {
      case BU_IMM:
         ...
         X = NewCodeEntry(OP65_LDA, AM65_IMM, MakeHexArg(B->Imm & 0xFF), 0,
                          E->LI);
         CS_InsertEntry(S, X, Idx++);
         break;
...
/* Restore the content of X before the specified index Idx */
{
   ...
   switch (B->Type & BU_TYPE_MASK) {
      case BU_IMM:
         /* Just use the memorized value */
         X = NewCodeEntry(OP65_LDX, AM65_IMM, MakeHexArg(B->Imm & 0xFF), 0,
                          E->LI);
         CS_InsertEntry(S, X, Idx++);
         break;
...
/* Restore the content of Y before the specified index Idx */
{
   ...
   switch (B->Type & BU_TYPE_MASK) {
      case BU_IMM:
         /* Just use the memorized value */
         X = NewCodeEntry(OP65_LDY, AM65_IMM, MakeHexArg(B->Imm & 0xFF), 0,
                          E->LI);
         CS_InsertEntry(S, X, Idx++);
         break;
   ...
   switch (B->Type & BU_TYPE_MASK) {
      case BU_REG:
         ...
         X = NewCodeEntry(OP65_LDA, AM65_IMM, MakeHexArg(B->Imm & 0xFF), 0,
                          E->LI);
         CS_InsertEntry(S, X, Idx++);
         X = NewCodeEntry(OP65_LDX, AM65_IMM, MakeHexArg((B->Imm >> 8) & 0xFF),
                          0, E->LI);
         CS_InsertEntry(S, X, Idx++);
         break;
   ...
   switch (B->Type & BU_TYPE_MASK) {
      case BU_IMM:
         ...
         X = NewCodeEntry(OP65_LDX, AM65_IMM, MakeHexArg((B->Imm >> 8) & 0xFF),
                          0, E->LI);
         CS_InsertEntry(S, X, Idx++);
         X = NewCodeEntry(OP65_LDY, AM65_IMM, MakeHexArg((B->Imm >> 16) & 0xFF),
                          0, E->LI);
         CS_InsertEntry(S, X, Idx++);
         break;
      ...
      case BU_ZP:
         ...
         X = NewCodeEntry(OP65_LDX, AM65_ZP, SB_GetConstBuf(&Arg), 0, E->LI);
         CS_InsertEntry(S, X, Idx++);
         X = NewCodeEntry(OP65_LDY, AM65_ZP, GetZPName(B->Where & REG_TMP1), 0,
                          E->LI);
         CS_InsertEntry(S, X, Idx++);
         break;
...
int BackupArgAfter(CodeSeg *S, BackupInfo *B, int Idx, const CodeEntry *E,
                   Collection *Indices)
// Backup the content of the opc arg of the entry E after the specified index
// Idx. Reg A/Y will be used to transfer the content from a memory location to
// another regardless of whether it is in use.
{
   CodeEntry *X;
   int OldIdx = Idx;
   unsigned ArgSize;
   ...
   /* We only recognize opc with an arg for now, as well as a special case for
    * ldaxysp */
      ...
      if (E->Size != 1 && E->AM != AM65_IMP) {
         ...
         switch (B->Type & BU_TYPE_MASK) {
            case BU_ZP:
               ...
               SB_Terminate(&DstArg);
               X = NewCodeEntry(OP65_STA, AM65_ZP, SB_GetConstBuf(&DstArg), 0,
                                E->LI);
               CS_InsertEntry(S, X, ++Idx);
               if (ArgSize == BU_B16) {
                  ...
                  SB_Terminate(&SrcArg);
                  X = NewCodeEntry(OP65_LDA, E->AM, SB_GetConstBuf(&SrcArg), 0,
                                   E->LI);
                  CS_InsertEntry(S, X, ++Idx);
                  SB_AppendStr(&DstArg, "+1");
                  SB_Terminate(&DstArg);
                  X = NewCodeEntry(OP65_STA, AM65_ZP, SB_GetConstBuf(&DstArg),
                                   0, E->LI);
                  CS_InsertEntry(S, X, ++Idx);
               }
               break;
            ...
            case BU_SP:
               ...
               if (ArgSize != BU_B16) {
                  ...
                  if ((B->ZPUsage & REG_X) == 0) {
                     if (E->AM == AM65_ZP) {
                        X = NewCodeEntry(OP65_LDX, E->AM,
                                         SB_GetConstBuf(&SrcArg), 0, E->LI);
                        CS_InsertEntry(S, X, ++Idx);
                        X = NewCodeEntry(OP65_JSR, AM65_ABS, "pushax", 0,
                                         E->LI);
                        CS_InsertEntry(S, X, ++Idx);
                     }
                     else {
                        X = NewCodeEntry(OP65_LDA, E->AM,
                                         SB_GetConstBuf(&SrcArg), 0, E->LI);
                        CS_InsertEntry(S, X, ++Idx);
                        X = NewCodeEntry(OP65_TAX, AM65_IMP, 0, 0, E->LI);
                        CS_InsertEntry(S, X, ++Idx);
                     ...
                     X = NewCodeEntry(OP65_JSR, AM65_ABS, "pusha", 0, E->LI);
                     CS_InsertEntry(S, X, ++Idx);
                     X = NewCodeEntry(OP65_LDA, AM65_ZP,
                                      SB_GetConstBuf(&DstArg), 0, E->LI);
                     CS_InsertEntry(S, X, ++Idx);
                     X = NewCodeEntry(OP65_JSR, AM65_ABS, "pusha", 0, E->LI);
                     CS_InsertEntry(S, X, ++Idx);
                  }
...
static int LoadAAt(CodeSeg *S, int Idx, const LoadRegInfo *LRI,
                   Collection *Indices, int After)
...
// depending on the After param.
{
   ...
   if ((E->OPC != OP65_JSR || strcmp(E->Arg, "ldaxysp") == 0) &&
      ...
      if (E->Size != 1 && E->AM != AM65_IMP) {
         ...
         /* FIXME: The load flags only reflect the situation by the time it
          * reaches the range end */
         if ((LRI->Flags & (LI_DIRECT | LI_CHECK_ARG | LI_CHECK_Y)) != 0) {
            if ((LRI->Flags & LI_RELOAD_Y) != 0) {
               if ((LRI->Flags & LI_CHECK_Y) == 0) {
                  X = NewCodeEntry(OP65_LDY, AM65_IMM, MakeHexArg(LRI->Offs), 0,
                                   E->LI);
               }
               else {
                  X = NewCodeEntry(OP65_LDY, LRI->LoadYEntry->AM,
                                   LRI->LoadYEntry->Arg, 0, E->LI);
               }
               CS_InsertEntry(S, X, Idx++);
            }
            X = NewCodeEntry(OP65_LDA, E->AM, E->Arg, 0, E->LI);
...
static int LoadXAt(CodeSeg *S, int Idx, const LoadRegInfo *LRI,
                   Collection *Indices, int After)
...
static int LoadYAt(CodeSeg *S, int Idx, const LoadRegInfo *LRI,
                   Collection *Indices, int After)
...
int LoadABefore(CodeSeg *S, int Idx, const LoadRegInfo *LRI,
                Collection *Indices)
...
int LoadXBefore(CodeSeg *S, int Idx, const LoadRegInfo *LRI,
                Collection *Indices)
...
int LoadYBefore(CodeSeg *S, int Idx, const LoadRegInfo *LRI,
                Collection *Indices)
...
unsigned GetRegUsageInOpenRange(CodeSeg *S, int First, int Last, unsigned *Use,
                                unsigned *Chg)
// Get what ZPs, registers or processor states are used or changed in the range
...
// Check if the loading the opc arg gives the same result everywhere between
// (First, Last). The code block in the range must be basic without any jump
// backwards. Note: this always checks Y if any of the LI_CHECK_Y / LI_RELOAD_Y
// flags is set.
{
   LoadRegInfo LRI;
   CodeEntry *X;
   ...
   if ((LRI.Flags & (LI_DIRECT | LI_CHECK_ARG | LI_CHECK_Y | LI_RELOAD_Y)) ==
       0) {
...
// Note: changes of Y are always ignored even if the LI_RELOAD_Y flag is not
// set.
{
   LoadRegInfo LRI;
   CodeEntry *X;
...
int FindArgLastUsageInOpenRange(CodeSeg *S, int First, int Last, CodeEntry *E,
                                int ReloadY)
// Find the last index where the arg of E might be used or changed in the range
// (First, Last). ReloadY indicates whether Y is supposed to be reloaded. The
// code block in the range must be basic without any jump backwards. Return the
// index of the found entry, or First if not found.
{
   LoadRegInfo LRI;
   CodeEntry *X;
...
int FindRegFirstChangeInOpenRange(CodeSeg *S, int First, int Last,
                                  unsigned what)
// Find the first possible spot where the queried ZPs, registers and/or
// processor states might be changed in the range (First, Last). The code block
// in the range must be basic without any jump backwards. Return the index of
// the found entry, or Last if not found.
{
   CodeEntry *X;
...
// Find the first possible spot where the queried ZPs, registers and/or
// processor states might be used in the range (First, Last). The code block in
// the range must be basic without any jump backwards. Return the index of the
// found entry, or Last if not found.
{
   CodeEntry *X;
