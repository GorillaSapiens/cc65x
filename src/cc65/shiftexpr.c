...
/* Parse the << and >> operators. */
{
   ...
   while (CurTok.Tok == TOK_SHL || CurTok.Tok == TOK_SHR) {
      ...
      if (!rconst) {
         ...
         if (!ED_IsUneval(Expr)) {
            if (Expr2.IVal < 0) {
               ...
               Warning("Shift count %ld >= width of %s treated as %u",
                       Expr2.IVal, GetBasicTypeName(ResultType),
         ...
         if (IsTypeBitField(Expr->Type) && Tok == TOK_SHR &&
             Expr2.IVal >= (long)Expr->Type->A.B.Width) {
            if (!ED_IsUneval(Expr)) {
               Warning("Right-shift count %ld >= width of bit-field",
                       Expr2.IVal);
            }
         }
         ...
         if (Tok == TOK_SHR && IsClassInt(Expr->Type) &&
             SizeOf(Expr->Type) == SIZEOF_INT && ED_IsLVal(Expr) &&
             ED_IsLocQuasiConst(Expr) && Expr2.IVal >= 8) {
      ...
      switch (Tok) {
         case TOK_SHL:
            g_asl(GenFlags, Expr2.IVal);
            break;
         case TOK_SHR:
            g_asr(GenFlags, Expr2.IVal);
            break;
         default:
            break;
      }
