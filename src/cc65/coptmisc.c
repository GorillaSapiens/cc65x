...
// provided that x and y are both zero.
{
   ...
   while (I < CS_GetEntryCount(S)) {
      ...
      if (E->OPC == OP65_LDA && E->AM == AM65_ZP_INDY && E->RI->In.RegY == 0 &&
          E->RI->In.RegX == 0) {
...
// provided that x and y are both zero.
{
   ...
   while (I < CS_GetEntryCount(S)) {
      ...
      /* Check if it's what we're looking for */
      if (E->OPC == OP65_LDA && E->AM == AM65_ZPX_IND && E->RI->In.RegY == 0 &&
          E->RI->In.RegX == 0) {
...
// known cases!
{
   ...
   while (I < CS_GetEntryCount(S)) {
      ...
      if (E->OPC == OP65_JSR && (Dec1 = IsDecSP(E)) > 0 &&
          (N = CS_GetNextEntry(S, I)) != 0 && (Dec2 = IsDecSP(N)) > 0 &&
          (Dec1 += Dec2) <= 255 && !CE_HasLabel(N)) {
...
/* Optimize SP adjustment for forward 'goto' */
{
   ...
   while (I < CS_GetEntryCount(S)) {
      ...
      if (L[0]->OPC == OP65_PHA && CS_GetEntries(S, L + 1, I + 1, 9) &&
          L[1]->OPC == OP65_LDA && L[1]->AM == AM65_ABS &&
          L[2]->OPC == OP65_CLC && L[3]->OPC == OP65_ADC &&
          strcmp(L[3]->Arg, "c_sp") == 0 && L[6]->OPC == OP65_ADC &&
          strcmp(L[6]->Arg, "c_sp+1") == 0 && L[9]->OPC == OP65_JMP) {
...
// a load of just the A register.
{
   ...
   while (I < CS_GetEntryCount(S)) {
      ...
      if (CE_IsCallTo(E, "ldaxysp") && RegValIsKnown(E->RI->In.RegY) &&
          !RegXUsed(S, I + 1)) {
...
/* Replace calls to ldaxysp by inline code */
{
   ...
   while (I < CS_GetEntryCount(S)) {
      ...
      if (CE_IsCallTo(L[0], "ldaxysp")) {
         ...
         if (CS_GetEntries(S, L + 1, I + 1, 2) && L[1]->OPC == OP65_STA &&
             L[2]->OPC == OP65_STX &&
             (L[1]->Arg == 0 || L[2]->Arg == 0 ||
              strcmp(L[1]->Arg, L[2]->Arg) != 0) &&
             !CS_RangeHasLabel(S, I + 1, 2) && !RegXUsed(S, I + 3)) {
