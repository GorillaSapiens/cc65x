...
// call to the function.
{
   ...
   switch (ED_GetLoc(Expr)) {
      ...
      default:
         Internal("Invalid location in ED_GetLabelName: 0x%04X",
                  ED_GetLoc(Expr));
   }
...
{
   return ED_IsLocRegister(Expr) || (ED_IsLocConst(Expr) && Expr->Sym != 0 &&
...
{
   return ED_NeedsPrimary(Expr) || ED_YetToTest(Expr) ||
...
// compile time.
{
   ...
   return (ED_IsConstAbsInt(Expr) && Expr->IVal != 0) || ED_IsEntityAddr(Expr);
}
...
// compile time.
{
   ...
   return (ED_IsConstAbsInt(Expr) && Expr->IVal == 0) || ED_IsNullPtr(Expr);
}
...
{
   return ED_IsConstAbs(Expr) && Expr->IVal == 0 &&
...
{
   return ED_IsAddrExpr(Expr) && Expr->Sym != 0 &&
...
// be an operand to a compare operation.
{
   ...
   return IsClassInt(Expr->Type) || IsClassFloat(Expr->Type) ||
          IsClassPtr(Expr->Type) || IsClassFunc(Expr->Type);
}
