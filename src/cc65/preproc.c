...
#define MSM_OP_HAS_C_ATTRIBUTE                                                 \
   0x40U                     /* Handle the "__has_c_attribute" operator */
#define MSM_TOK_HEADER 0x80U /* Support header tokens */

/* Macro expansion state flags */
#define MES_NONE 0x00U /* Nothing */
#define MES_FIRST_TOKEN                                                        \
   0x01U /* Mark for detecting pp-token count in the sequence */
#define MES_MULTIPLE_TOKEN                                                     \
   0x02U /* Multiple pp-tokens are detected in the sequence */
#define MES_BEGIN_WITH_IDENT                                                   \
   0x04U /* The first pp-token of the sequence is an identifier */
#define MES_HAS_REPLACEMENT                                                    \
   0x10U /* Macro argument has cached replacement result */
#define MES_NO_VA_COMMA                                                        \
   0x20U /* Variadic macro called w/o the ',' in front of variable argument */
...
static void PreprocessDirective(StrBuf *Source, StrBuf *Target,
                                unsigned ModeFlags);
...
static unsigned ReplaceMacros(StrBuf *Source, StrBuf *Target, MacroExp *E,
                              unsigned ModeFlags);
...
static const struct PPDType {
   ...
   ppdirective_t Type; /* Type */
    ...
    /* BEGIN SORTED.SH */
    {"define", PPD_DEFINE},   {"elif", PPD_ELIF},     {"else", PPD_ELSE},
    {"endif", PPD_ENDIF},     {"error", PPD_ERROR},   {"if", PPD_IF},
    {"ifdef", PPD_IFDEF},     {"ifndef", PPD_IFNDEF}, {"include", PPD_INCLUDE},
    {"line", PPD_LINE},       {"pragma", PPD_PRAGMA}, {"undef", PPD_UNDEF},
    {"warning", PPD_WARNING},
    /* END SORTED.SH */
};
...
static void ME_HideMacro(unsigned Idx, unsigned Count, MacroExp *E,
                         const Macro *M)
...
/* Return 1 if the macro can be expanded with the Idx'th identifier */
{
   if (E != 0) {
      ...
      if (MHS != 0) {
         ...
         HideRange *HS = MHS->HS;
         while (HS != 0) {
            // If the macro name overlaps with the range where the macro is
            // hidden, the macro cannot be expanded.
...
static void ME_HandleSemiNestedMacro(unsigned NameIdx, unsigned LastIdx,
                                     MacroExp *E)
...
/* Make a replaced version of the argument */
{
   ...
   if ((A->Flags & MES_HAS_REPLACEMENT) == 0) {
      ...
      A->Replaced->IdentCount = ReplaceMacros(
          &A->Tokens, &A->Replaced->Tokens, A->Replaced, MSM_IN_ARG_EXPANSION);
...
{
   return (M->Variadic && M->ParamCount == (int)CollCount(&E->Args) + 1);
}
...
static void PushRescanLine(RescanInputStack *RIS, StrBuf *L,
                           unsigned LastTokLen)
...
{
   if (Std >= STD_C99 && (M == 0 || !M->Variadic) &&
       strcmp(Ident, "__VA_ARGS__") == 0) {
      // __VA_ARGS__ cannot be used as a macro parameter name in post-C89
      // mode.
      PPWarning("__VA_ARGS__ can only appear in the expansion of a C99 "
                "variadic macro");
   }
}
...
// return 0 if nothing was skipped.
{
   ...
   while (1) {
      if (IsSpace(CurC)) {
         ...
         /* End of line */
         if (CurRescanStack != 0 && CollCount(&CurRescanStack->Lines) > 1 &&
             Line == CollLast(&CurRescanStack->Lines)) {

            unsigned LastTokLen =
                (unsigned)(uintptr_t)CollLast(&CurRescanStack->LastTokLens);

            /* Check for potentially merged tokens */
            if (Skipped == 0 && LastTokLen != 0) {
               /* Get the following input */
               StrBuf *Next =
                   CollAtUnchecked(&CurRescanStack->Lines,
...
// characters from Appended to form a new valid one.
{
   ...
   SB_CopyBuf(&Src, SB_GetConstBuf(Source) + SB_GetLen(Source) - TokLen,
              TokLen);
...
static int TryPastePPTok(StrBuf *Target, StrBuf *Appended, unsigned FirstTokLen,
...
// two pp-tokens pasted together. Return 1 if succeeds.
{
   ...
   if (FirstTokLen == 1) {
      ...
      if ((C == '.' && N == '.') || (C == '/' && (N == '/' || N == '*'))) {
         SB_AppendChar(Target, ' ');
         SB_Append(Target, Appended);
         PPWarning("Pasting formed \"%c%c\", an invalid preprocessing token", C,
                   N);
   ...
   SB_CopyBuf(&Src, SB_GetConstBuf(Target) + SB_GetLen(Target) - FirstTokLen,
              FirstTokLen);
   if (SecondTokLen == SB_GetLen(Appended) ||
       IsSpace(SB_LookAt(Appended, SecondTokLen))) {
...
// the following character.
{
   if (CurRescanStack->PrevTok != 0) {
      unsigned Len = SB_GetLen(CurRescanStack->PrevTok) -
...
// concatenation.
{
   ...
   /* Special check for .. */
   if (NewIndex > 0 && C == '.' && SB_AtUnchecked(Prev, NewIndex - 1) == '.') {
   ...
   if (CurRescanStack->PrevTok != 0) {
      ...
      if (OldLen == NewLen &&
          strncmp(SB_GetConstBuf(CurRescanStack->PrevTok) + OldIndex -
                      CheckEllipsis,
         ...
         Internal("Unchecked pp-token concatenation: \"%s\"",
                  SB_GetConstBuf(CurRescanStack->PrevTok) +
                      SB_GetIndex(CurRescanStack->PrevTok));
      }
   }
   else {
...
static unsigned ReadMacroArgs(unsigned NameIdx, MacroExp *E, const Macro *M,
                              int MultiLine)
...
// identifiers and right parentheses in the read argument list.
{
   ...
   while (1) {
      ...
      if (Parens == 0 &&
         ...
         if (CurC != ')' || CollCount(&E->Args) > 0 ||
             SB_NotEmpty(&Arg.Tokens) || M->ParamCount > 0) {
   ...
   if (CollCount(&E->Args) < (unsigned)M->ParamCount) {
      ...
      /* Too many arguments */
      PPError("Macro \"%s\" passed %u arguments, but takes just %u", M->Name,
              CollCount(&E->Args), (unsigned)M->ParamCount);
   }
...
static unsigned SubstMacroArgs(unsigned NameIdx, StrBuf *Target, MacroExp *E,
                               Macro *M, unsigned *IdentCount)
      ...
      if (IsSym(Ident)) {
         ...
         if ((ParamIdx = FindMacroParam(M, Ident)) >= 0) {
            ...
            if (CurC == '#' && NextC == '#') {
               ...
               /* Insert the range of identifiers to parent preceding this
                * argument */
               ME_OffsetHideSets(Idx, A->IdentCount, E);
...
static unsigned ExpandMacro(unsigned Idx, StrBuf *Target, MacroExp *E, Macro *M,
                            int MultiLine)
...
// right parentheses in the replacement result.
{
   ...
   while (CurC != '\0') {
      ...
      if (IsSym(Ident)) {
         ...
         if ((ModeFlags &
              (MSM_MULTILINE | MSM_IN_DIRECTIVE | MSM_IN_ARG_LIST)) != 0 &&
             (CollCount(&CurRescanStack->Lines) == 1 || CurC == '\0')) {
            CheckForBadIdent(Ident, IS_Get(&Standard), 0);
         }

         if ((ModeFlags & MSM_OP_DEFINED) != 0 &&
             strcmp(Ident, "defined") == 0) {
...
/* Get a decimal digit-sequence from the input. Return 0 on errors. */
{
   ...
   if (SB_GetLen(&Buf) > 0) {
      ...
      if (Str[0] == '\0') {
         ...
         if (Num > 2147483647) {
            PPError("#line directive requires an integer argument not greater "
                    "than 2147483647");
            ClearLine();
            Num = 0;
         }
...
/* Print a warning */
{
   ...
   if (CurC == '\0') {
      ...
      TranslationPhase3(Line, &MsgLine);
      PPWarning("#warning: %s",
                SB_GetConstBuf(&MsgLine) + SB_GetIndex(&MsgLine));
      SB_Done(&MsgLine);
   }
            ...
            case PPD_PRAGMA:
               ...
               if (!PPSkip) {
                  if ((ModeFlags & MSM_IN_ARG_LIST) == 0) {
                     ...
                     PPError("Embedded #pragma directive within macro "
