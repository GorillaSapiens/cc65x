...
// Return the addressing mode flags for the given expression
{
   switch (ED_GetLoc(Expr)) {
      ...
      case E_LOC_CODE:
         return CF_CODE;
      default:
         Internal("CG_AddrModeFlags: Invalid location flags value: 0x%04X",
                  Expr->Flags);
         // NOTREACHED
         return 0;
   }
...
// Beware: The function does only accept int types.
{
   ...
   if (ED_IsConstAbsInt(lhs) && ltype == CF_INT && lhs->IVal >= 0 &&
       lhs->IVal < 256) {
      // If the lhs is a int constant that fits in an unsigned char, use
      // unsigned char. g_typeadjust will either promote this to int or unsigned
      // int as appropriate based on the other operand.  See comment in
      // hie_internal.
   ...
   if (ED_IsConstAbsInt(rhs) && rtype == CF_INT && rhs->IVal >= 0 &&
       rhs->IVal < 256) {
...
// Limit the constant value of the expression to the range of its type
{
   switch (GetUnderlyingTypeCode(Expr->Type)) {
      ...
      case T_SHORT:
         if (WarnOverflow &&
             ((Expr->IVal < -0x8000) || (Expr->IVal > 0x7FFF))) {
...
// in hie10).
{
   ...
   return CurTok.Tok == TOK_LPAREN &&
          (TokIsType(&NextTok) || TokIsTypeQual(&NextTok) ||
           (NextTok.Tok == TOK_IDENT && (Entry = FindSym(NextTok.Ident)) != 0 &&
            SymIsTypeDef(Entry)));
}
...
// Specially checked SizeOf() used in 'sizeof' expressions
{
   ...
   if (Size == 0) {
      ...
      if (TagSym == 0 || !SymIsDef(TagSym)) {
         Error("Cannot apply 'sizeof' to incomplete type '%s'",
               GetFullTypeName(T));
      }
   }
   return Size;
...
// If the result of a comparison is constant, this is suspicious
{
   if (!ED_NeedsConst(Expr) && IS_Get(&WarnConstComparison) != 0) {
      Warning("Result of comparison is always %s",
              Expr->IVal != 0 ? "true" : "false");
   }
}
...
// Do increment
{
   ...
   // Get the flags
   Flags =
       CG_TypeOf(Expr->Type) | CG_AddrModeFlags(Expr) | CF_FORCECHAR | CF_CONST;
...
// Die off if check fails.
{
   if (GetDeferredOpCount() > 0) {
      Internal("Code generation messed up: missing operations past sequence "
               "points.");
   }
}
...
// The function returns the size of the arguments pushed in bytes.
{
   ...
   DoDeferred(IsFastcall && PushedCount > 0 ? SQP_KEEP_EAX : SQP_KEEP_NONE,
              &Expr);
...
// Perform a function call.
{
   ...
   if (ArgSize > 0xFF && (Func->Flags & FD_VARIADIC) != 0) {
      Error("Total size of all arguments passed to a variadic function cannot "
            "exceed 255 bytes");
   }
   ...
   if (IsFuncPtr) {
      ...
      if (Expr->Sym && Expr->Sym->V.F.WrappedCall) {
         ...
         g_call(CG_CallFlags(Expr->Type), Expr->Sym->V.F.WrappedCall->Name,
                ArgSize);
      }
      else {
   ...
   switch (CurTok.Tok) {
      ...
      case TOK_BOOL_AND:
         ...
         if (IS_Get(&Standard) >= STD_CC65) {
            ...
            Error("Computed gotos are a C extension, not supported with this "
                  "--standard");
      ...
      case TOK_IDENT:
         ...
         if (Sym) {
            ...
            if ((Sym->Flags & SC_CONST) == SC_CONST) {
               ...
               if ((Sym->Flags & SC_PARAM) == SC_PARAM &&
                   F_IsVariadic(CurrentFunc)) {
                  // Variadic parameter
            ...
            if (CurTok.Tok == TOK_LPAREN) {
               ...
               Sym =
                   AddGlobalSym(Ident, GetImplicitFuncType(), SC_REF | SC_FUNC);
               E->Type = Sym->Type;
      ...
      default:
         ...
         if (CurTok.Tok == TOK_LCURLY) {
            ...
            DeclSpec Spec;
            ParseDeclSpec(&Spec, TS_DEFAULT_TYPE_NONE | TS_FUNCTION_SPEC,
                          SC_AUTO);
...
// Process struct/union field after . or ->.
{
   ...
   if (Field.Type == 0) {
      Error("No field named '%s' found in '%s'", CurTok.Ident,
            GetFullTypeName(Expr->Type));
      // Make the expression an integer at address zero
      ED_MakeConstAbs(Expr, 0, type_int);
      return;
   ...
   else if (ED_IsRVal(Expr) && ED_IsLocPrimary(Expr) &&
   ...
   if (ED_IsRVal(Expr) && ED_IsLocPrimary(Expr) && !IsTypePtr(Expr->Type)) {
      ...
      switch (StructSize) {
         ...
         case 4:
            ...
            break;
         default:
            Internal("Invalid '%s' size: %u", GetFullTypeName(Expr->Type),
                     StructSize);
            break;
      }
      ...
      switch (CurTok.Tok) {
         ...
         case TOK_PTR_REF:
            ...
            if (!IsClassPtr(Expr->Type) ||
                !IsClassStruct(Indirect(Expr->Type))) {
...
static void hie_internal(const GenDesc *Ops, // List of generators
                         ExprDesc *Expr, void (*hienext)(ExprDesc *),
                         int *UsedGen)
// Helper function
{
   CodeMark Mark1;
   ...
   while ((Gen = FindGen(CurTok.Tok, Ops)) != 0) {
      ...
      *UsedGen = 1;
      ...
      if (lconst && rconst) {
...
#endif
         if ((ltype == CF_FLOAT) || (Expr2.Type == type_float)) {
         ...
         // of int, which would be returned without this modification).  This
         // allows more efficient operations, but does not affect correctness
         // for the same reasons explained in g_typeadjust.
...
// Helper function for the compare operators
{
   ...
   while ((Gen = FindGen(CurTok.Tok, Ops)) != 0) {
      ...
      if (CG_TypeOf(Expr2.Type) == CF_FLOAT) {
         if (ltype != CF_FLOAT) {
            // left hand is NOT a float (but a constant), right IS a float (but
      ...
      if (IsClassInt(Expr->Type)) {
         if (!IsClassInt(Expr2.Type) && !IsClassFloat(Expr2.Type) &&
            ...
            if (IsClassPtr(Expr2.Type)) {
               TypeCompatibilityDiagnostic(
                   Expr->Type, PtrConversion(Expr2.Type), 0,
         ...
         if (IsClassPtr(Expr2.Type)) {
            ...
            if (TypeCmp(Expr->Type, Expr2.Type).C < TC_VOID_PTR) {
               ...
               TypeCompatibilityDiagnostic(
                   PtrConversion(Expr->Type), PtrConversion(Expr2.Type), 0,
            ...
            if (IsClassInt(Expr2.Type)) {
               TypeCompatibilityDiagnostic(
                   PtrConversion(Expr->Type), Expr2.Type, 0,
      ...
      else if (ED_IsEntityAddr(Expr) && ED_IsEntityAddr(&Expr2) &&
               Expr->Sym == Expr2.Sym) {
...
// plus operation. Otherwise, this evaluates the subscript operation.
{
   ...
   if (ED_IsQuasiConst(Expr)) {
      ...
      if (ED_IsQuasiConst(&Expr2)) {
         ...
         if (AddDone) {
            ...
            if (!ED_IsAbs(&Expr2) && rscale > 1) {
               ...
               if (ED_IsAbs(Expr)) {
                  ...
                  // Static address
                  g_addaddr_static(flags | CG_AddrModeFlags(Expr), Expr->Name,
                                   Expr->IVal);
               }
            }
            else {
               ...
               if (ED_IsAbs(&Expr2)) {
                  ...
                  // Static address
                  g_addaddr_static(flags | CG_AddrModeFlags(&Expr2), Expr2.Name,
                                   Expr2.IVal);
               }
            }
         ...
         if (!AddDone) {
            if (ED_IsLocQuasiConst(&Expr2) && !IsTypeBitField(Expr2.Type) &&
                rscale == 1 && CheckedSizeOf(rhst) == SIZEOF_CHAR) {
               ...
               if (CG_TypeOf(Expr->Type) == CF_FLOAT) {
                  ...
                  if (CG_TypeOf(Expr2.Type) == CF_FLOAT) {
                     // lhs = int, rhs float
                     g_inc(flags,
                           FP_D_As32bitRaw(FP_D_FromInt(Expr->IVal * lscale)));
      ...
      if (ED_IsConstAbs(&Expr2) && ED_CodeRangeIsEmpty(&Expr2)) {
         ...
         if (flags & CF_FLOAT) {
            // FIXME: float - what to do here exactly?
            if (IsClassInt(rhst)) {
               g_inc(flags | CF_CONST,
                     FP_D_As32bitRaw(FP_D_FromInt(Expr2.IVal)));
            }
            else {
         ...
         // Lhs and rhs are not so "numeric constant". Check for pointer
         // arithmetic.
         if (IsClassPtr(lhst) && IsClassInt(rhst)) {
            // Left is pointer, right is int, must scale rhs
            rscale = CheckedPSizeOf(lhst);
   ...
   if (DoArrayRef) {
      ...
      // referenced in the primary, unless it is once again an array. We can
      // just assume the usual case first, and change it later if necessary.
      ED_IndExpr(Expr);
      Expr->Type = Indirect(Expr->Type);
      ...
      // virtually an rvalue address, otherwise it's an lvalue reference. (A
      // function would also be an rvalue address, but an array cannot contain
      // functions).
      if (IsTypeArray(Expr->Type)) {
         ED_AddrExpr(Expr);
      }
...
// result of the expression on return.
{
   ...
   if (IsClassPtr(lhst)) {
      ...
      if (rscale == 0 && (IsClassPtr(rhst) || IsClassInt(rhst))) {
         TypeCompatibilityDiagnostic(
             lhst, rhst, 1,
             "Invalid pointer types in subtraction: '%s' and '%s'");
         // Avoid further errors
         rscale = 1;
      }
   ...
   if (IsClassPtr(lhst) && IsClassPtr(rhst)) {
      ...
      if (Cmp.C < TC_STRICT_COMPATIBLE) {
         TypeCompatibilityDiagnostic(
             lhst, rhst, 1,
             "Incompatible pointer types in subtraction: '%s' and '%s'");
      }
      ...
      if (ED_IsQuasiConst(&Expr2) && ED_CodeRangeIsEmpty(&Expr2)) {
         ...
         if (ED_IsQuasiConst(Expr)) {
            ...
            if (ED_GetLoc(Expr) == ED_GetLoc(&Expr2) &&
                Expr->Sym == Expr2.Sym) {
         ...
         if (ED_IsConstAbs(&Expr2)) {
            ...
            if (IsClassInt(lhst)) {
               ...
               if (IsClassFloat(rhst)) {
                  ...
                  g_dec(flags | CF_CONST,
                        FP_D_As32bitRaw(FP_D_FromInt(Expr2.IVal * rscale)));
               }
            }
            else {
               // Do the subtraction
...
// Handle == and !=
{
   static const GenDesc hie5_ops[] = {{TOK_EQ, GEN_NOPUSH, g_eq},
...
// Handle & (bitwise and)
{
   static const GenDesc hie4_ops[] = {{TOK_AND, GEN_NOPUSH | GEN_COMM, g_and},
                                      {TOK_INVALID, 0, 0}};
   int UsedGen;
...
{
   static const GenDesc hie3_ops[] = {{TOK_XOR, GEN_NOPUSH | GEN_COMM, g_xor},
                                      {TOK_INVALID, 0, 0}};
   int UsedGen;
...
// Handle | (bitwise or)
{
   static const GenDesc hie2_ops[] = {{TOK_OR, GEN_NOPUSH | GEN_COMM, g_or},
                                      {TOK_INVALID, 0, 0}};
   int UsedGen;
...
// Parse the ternary operator
{
   ...
   if (CurTok.Tok == TOK_QUEST) {
      ...
      if (!IsTypeVoid(Expr2.Type) && ED_YetToLoad(&Expr2) &&
      ...
      if (!IsTypeVoid(Expr3.Type) && ED_YetToLoad(&Expr3) &&
      ...
      if (IsClassInt(Expr2.Type) && IsClassInt(Expr3.Type)) {
         ...
         if (IsTypeVoid(Indirect(Expr2.Type))) {
            ...
            if (TypeCmp(Expr2.Type, Expr3.Type).C < TC_VOID_PTR) {
               TypeCompatibilityDiagnostic(
                   Expr2.Type, Expr3.Type, 1,
                   "Incompatible pointer types in ternary: '%s' and '%s'");
               // Avoid further errors
...
// Parse comma operator.
{
   ...
   while (CurTok.Tok == TOK_COMMA) {
      ...
      if (CodeRangeIsEmpty(&Start, &End) ||
         ...
         if (!ED_MayHaveNoEffect(Expr) && IS_Get(&WarnNoEffect) &&
