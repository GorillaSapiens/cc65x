...
extern unsigned
    ErrorCount; // Errors occurred in parser and later translation phases
extern unsigned
    WarningCount; // Warnings occurred in parser and later translation phases
...
extern IntStack WarnPointerSign; /* - pointer conversion to pointer differing in
                                    signedness */
extern IntStack
    WarnPointerTypes; // - pointer conversion to incompatible pointer type
...
extern IntStack
    WarnConstOverflow; // - overflow conversion of numerical constants
...
void LIError_(const char *file, int line, errcat_t EC, LineInfo *LI,
              const char *Format, ...) attribute((format(printf, 5, 6)));
