...
typedef enum {
   ...
   OP_LHS_SAME_BY_OP =
       0x08, /* Load result of LHS must be the same if relocated to op */
   OP_LHS_LOAD_DIRECT = 0x0C, /* Must have direct load insn for LHS */
   OP_RHS_LOAD = 0x10,        /* Must have load insns for RHS */
   OP_RHS_SAME_BY_OP =
       0x20, /* Load result of RHS must be the same if relocated to op */
   ...
   OP_LHS_SAME_BY_PUSH =
       0x0100, /* LHS must load the same content if relocated to push */
   OP_RHS_SAME_BY_PUSH =
       0x0200, /* RHS must load the same content if relocated to push */
   OP_LHS_SAME_BY_RHS =
       0x0400, /* LHS must load the same content if relocated to RHS */
   OP_RHS_SAME_BY_LHS =
       0x0800, /* RHS must load the same content if relocated to LHS */
   OP_LHS_REMOVE =
       0x1000, /* LHS must be removable or RHS may use ZP store/load */
   OP_LHS_REMOVE_DIRECT = 0x3000, /* LHS must be directly removable */
   OP_RHS_REMOVE =
       0x4000, /* RHS must be removable or LHS may use ZP store/load */
...
/* Check if Rhs Reg A == Lhs Reg A */
{
   ...
   if (RRI == 0 ||
       (D->Rhs.A.ChgIndex >= 0 && D->Rhs.A.ChgIndex == D->Lhs.A.ChgIndex) ||
       (LRI != 0 && RegValIsKnown(LRI->Out.RegA) &&
...
/* Check if Rhs Reg X == Lhs Reg X */
{
   ...
   if (RRI == 0 ||
       (D->Rhs.X.ChgIndex >= 0 && D->Rhs.X.ChgIndex == D->Lhs.X.ChgIndex) ||
       (LRI != 0 && RegValIsKnown(LRI->Out.RegX) &&
...
/* Optimize the tosaddax sequence */
{
   ...
   // alltogether.
   if (D->OpEntry->RI->In.RegX == 0 && D->NextEntry->OPC == OP65_LDY &&
       CE_IsKnownImm(D->NextEntry, 0) && !CE_HasLabel(D->NextEntry) &&
       (N = CS_GetNextEntry(D->Code, D->OpIndex + 1)) != 0 &&
       (CE_IsCallTo(N, "ldauidx") || CE_IsCallTo(N, "ldaidx"))) {
      ...
      if (D->PushEntry->RI->In.RegX == 0) {
         ...
         if (RegValIsKnown(D->PushEntry->RI->In.RegX)) {
            ...
            if (D->Lhs.X.Flags & LI_DIRECT) {
               CodeEntry *LoadX = D->Lhs.X.LoadEntry;
               X = NewCodeEntry(OP65_LDX, LoadX->AM, LoadX->Arg, 0,
                                D->OpEntry->LI);
            }
            else {
               X = NewCodeEntry(OP65_LDX, AM65_ZP, D->ZPHi, 0, D->OpEntry->LI);
...
/* Optimize the tosxorax sequence */
{
   ...
   if (RegValIsKnown(D->PushEntry->RI->In.RegX) &&
      ...
      const char *Arg =
          MakeHexArg(D->PushEntry->RI->In.RegX ^ D->OpEntry->RI->In.RegX);
      X = NewCodeEntry(OP65_LDX, AM65_IMM, Arg, 0, D->OpEntry->LI);
      InsertEntry(D, X, D->IP++);
   }
...
/* Optimize the tosandax/tosorax/tosxorax sequence. */
{
   ...
   if ((D->Rhs.A.Flags & LI_DIRECT) == 0) {
      if ((D->Lhs.A.Flags & (LI_DIRECT | LI_RELOAD_Y)) == LI_DIRECT) {
         /* Just reload lhs */
         X = NewCodeEntry(OPC, D->Lhs.A.LoadEntry->AM, D->Lhs.A.LoadEntry->Arg,
                          0, D->OpEntry->LI);
         InsertEntry(D, X, D->IP++);
      }
      else {
      ...
      /* Add code for low operand */
      X = NewCodeEntry(OPC, D->Rhs.A.LoadEntry->AM, D->Rhs.A.LoadEntry->Arg, 0,
                       D->OpEntry->LI);
      InsertEntry(D, X, D->IP++);
   ...
   if ((GetRegInfo(D->Code, D->IP, REG_X) & REG_X) != 0) {
      ...
      if (OPC == OP65_EOR) {
         X = NewCodeEntry(OP65_LDX, AM65_IMM, MakeHexArg(0), 0,
                          D->Rhs.X.ChgEntry->LI);
         InsertEntry(D, X, D->IP++);
         D->Rhs.X.Flags |= LI_REMOVE;
      }
...
/* Optimize the TOS compare sequence with a bool transformer */
{
   ...
   if (!D->RhsMultiChg && (D->Rhs.A.Flags & LI_DIRECT) != 0 &&
      ...
      // If the lhs is direct (but not stack relative), encode compares with
      // lhs, effectively reversing the order (which doesn't matter for == and
      // !=).
      ...
      AddStoreLhsA(D);
      /* AddStoreLhsA may have moved the OpIndex, recalculate insertion point to
       * prevent label migration. */
      D->IP = D->OpIndex + 1;
...
/* Replace tosicmp with CMP */
{
   ...
   if (!SameRegAValue(D)) {
      ...
      if ((D->Rhs.A.Flags & LI_DIRECT) == 0) {
         ...
         /* RHS insertion may have moved the OpIndex, recalculate insertion
          * point to prevent label migration. */
         D->IP = D->OpIndex + 1;
         ...
         if ((D->Rhs.A.Flags & LI_RELOAD_Y) == 0) {
            ...
            X = NewCodeEntry(OP65_CMP, AM65_ZP, D->Rhs.A.LoadEntry->Arg, 0,
                             D->OpEntry->LI);
            InsertEntry(D, X, D->IP++);
         }
         else {
            ...
            if ((D->Rhs.A.Flags & LI_CHECK_Y) == 0) {
               X = NewCodeEntry(OP65_LDY, AM65_IMM, MakeHexArg(D->Rhs.A.Offs),
                                0, D->OpEntry->LI);
            }
            else {
               X = NewCodeEntry(OP65_LDY, D->Rhs.A.LoadYEntry->AM,
                                D->Rhs.A.LoadYEntry->Arg, 0, D->OpEntry->LI);
            }
            InsertEntry(D, X, D->IP++);
            ...
            if (D->Rhs.A.LoadEntry->OPC == OP65_JSR) {
               /* opc (c_sp),y */
               X = NewCodeEntry(OP65_CMP, AM65_ZP_INDY, "c_sp", 0,
                                D->OpEntry->LI);
            }
            else {
               /* opc src,y */
               X = NewCodeEntry(OP65_CMP, D->Rhs.A.LoadEntry->AM,
                                D->Rhs.A.LoadEntry->Arg, 0, D->OpEntry->LI);
            }
            InsertEntry(D, X, D->IP++);
         }
      ...
      if ((RI = GetLastChangedRegInfo(D, &D->Lhs.A)) != 0 &&
          RegValIsKnown(RI->Out.RegA) && (RI->Out.RegA & 0xFF) == 0) {
...
static const OptFuncDesc FuncTable[] = {
    ...
    {"toseqax", Opt_toseqax, REG_NONE,
     OP_LR_INTERCHANGE | OP_RHS_REMOVE_DIRECT},
    {"tosgeax", Opt_tosgeax, REG_NONE,
     OP_RHS_REMOVE_DIRECT | OP_RHS_LOAD_DIRECT},
    {"tosltax", Opt_tosltax, REG_NONE,
     OP_RHS_REMOVE_DIRECT | OP_RHS_LOAD_DIRECT},
    {"tosneax", Opt_tosneax, REG_NONE,
     OP_LR_INTERCHANGE | OP_RHS_REMOVE_DIRECT},
    {"tosorax", Opt_tosorax, REG_NONE, OP_NONE},
    {"tosshlax", Opt_tosshlax, REG_NONE, OP_NONE},
    {"tosshrax", Opt_tosshrax, REG_NONE, OP_NONE},
    {"tossubax", Opt_tossubax, REG_NONE,
     OP_RHS_REMOVE_DIRECT | OP_RHS_LOAD_DIRECT},
    {"tosugeax", Opt_tosugeax, REG_NONE,
     OP_RHS_REMOVE_DIRECT | OP_RHS_LOAD_DIRECT},
    {"tosugtax", Opt_tosugtax, REG_NONE,
     OP_RHS_REMOVE_DIRECT | OP_RHS_LOAD_DIRECT},
    {"tosuleax", Opt_tosuleax, REG_NONE,
     OP_RHS_REMOVE_DIRECT | OP_RHS_LOAD_DIRECT},
    {"tosultax", Opt_tosultax, REG_NONE,
     OP_RHS_REMOVE_DIRECT | OP_RHS_LOAD_DIRECT},
...
static const OptFuncDesc FuncRegATable[] = {
    /* BEGIN SORTED.SH */
    {"tosandax", Opt_a_tosand, REG_NONE,
     OP_RHS_REMOVE_DIRECT | OP_RHS_LOAD_DIRECT},
    ...
    {"tosorax", Opt_a_tosor, REG_NONE,
     OP_RHS_REMOVE_DIRECT | OP_RHS_LOAD_DIRECT},
    {"tossubax", Opt_a_tossub, REG_NONE,
     OP_RHS_REMOVE_DIRECT | OP_RHS_LOAD_DIRECT},
    ...
    {"tosxorax", Opt_a_tosxor, REG_NONE,
     OP_RHS_REMOVE_DIRECT | OP_RHS_LOAD_DIRECT},
    /* END SORTED.SH */
};
...
static const OptFuncDesc *FindFunc(const OptFuncDesc FuncTable[], size_t Count,
                                   const char *Name)
...
// register to use for temporary storage.
{
   ...
   for (I = (Desc->Flags & OP_AX_INTERCHANGE ? 0 : 1); !Passed && I < 2; ++I) {
      ...
      do {
         if ((Desc->Flags & OP_A_KNOWN) != 0 && RegValIsUnknown(LoVal)) {
            /* Cannot optimize */
            break;
         }
         if ((Desc->Flags & OP_X_ZERO) != 0 && HiVal != 0) {
            /* Cannot optimize */
            break;
         }
         Passed = 1;
   ...
   Lhs = &D->Lhs;
   Rhs = &D->Rhs;
   /* Check normally first, then interchange LHS/RHS and check again if
    * necessary */
   for (I = (Desc->Flags & OP_LR_INTERCHANGE ? 0 : 1); !Passed && I < 2; ++I) {
      ...
      do {
         ...
         RhsLo = &Rhs->A;
         RhsHi = &Rhs->X;
         // Currently we have only LHS/RHS checks with identical requirements
         // for A/X, so we don't need to check twice for now.
         ...
         if ((Desc->Flags & OP_LHS_REMOVE) != 0) {
            ...
            if ((LhsLo->LoadEntry != 0 &&
                 (LhsLo->LoadEntry->Flags & CEF_DONT_REMOVE) != 0) ||
                (LhsHi->LoadEntry != 0 &&
                 (LhsHi->LoadEntry->Flags & CEF_DONT_REMOVE) != 0)) {
         ...
         if ((Desc->Flags & OP_RHS_REMOVE) != 0) {
            if ((RhsLo->LoadEntry != 0 &&
                 (RhsLo->LoadEntry->Flags & CEF_DONT_REMOVE) != 0) ||
                (RhsHi->LoadEntry != 0 &&
                 (RhsHi->LoadEntry->Flags & CEF_DONT_REMOVE) != 0)) {
...
// register to use for temporary storage.
{
   ...
   for (I = (Desc->Flags & OP_AX_INTERCHANGE ? 0 : 1); !Passed && I < 2; ++I) {
      ...
      do {
         ...
         if ((Desc->Flags & OP_A_KNOWN) != 0 && RegValIsUnknown(LhsLoVal)) {
            /* Cannot optimize */
            break;
         }
         if ((Desc->Flags & OP_X_ZERO) != 0 && LhsHiVal != 0) {
            /* Cannot optimize */
            break;
         }
         Passed = 1;
...
/* Optimize operations that take operands via the stack */
{
   ...
   enum { Initialize, Search, FoundPush, FoundOp } State = Initialize;
         ...
         case FoundPush:
            ...
            if (E->OPC == OP65_JSR) {
               ...
               if (SameRegXValue(&Data)) {
                  Data.OptFunc = FindFunc(FuncRegATable,
                                          FUNC_COUNT(FuncRegATable), E->Arg);
                  IsRegAOptFunc = 1;
               }
               if (Data.OptFunc == 0) {
                  Data.OptFunc =
                      FindFunc(FuncTable, FUNC_COUNT(FuncTable), E->Arg);
                  IsRegAOptFunc = 0;
               }
               if (Data.OptFunc) {
               ...
               if (E->AM == AM65_ZPX_IND ||
                   ...
                   (RegValIsUnknown(E->RI->In.RegY) || E->RI->In.RegY < 2)) {
                  I = Data.PushIndex;
                  State = Initialize;
                  break;
               }
