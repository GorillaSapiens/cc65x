...
/* Check a symbol table for open references, unused symbols ... */
{
   ...
   while (Entry) {
      ...
      if (!SymIsTypeDef(Entry)) {
         ...
         /* Check if the symbol has non-external linkage and is defined but not
          * used */
         if (!SymIsGlobal(Entry) || (Flags & SC_STORAGEMASK) == SC_STATIC) {
            if (SymIsDef(Entry) && !SymIsRef(Entry) &&
                !SymHasAttr(Entry, atUnused)) {
               if (Flags & SC_PARAM) {
                  if (IS_Get(&WarnUnusedParam) && !IsAnonName(Entry->Name)) {
                     Warning("Parameter '%s' is never used", Entry->Name);
                  }
               }
               else if ((Flags & SC_TYPEMASK) == SC_FUNC) {
                  if (IS_Get(&WarnUnusedFunc)) {
                     Warning("Function '%s' is defined but never used",
                             Entry->Name);
                  }
               }
               else if ((Flags & SC_TYPEMASK) == SC_NONE) {
                  if (IS_Get(&WarnUnusedVar) && !IsAnonName(Entry->Name) &&
                      (Flags & SC_CONST) != SC_CONST) {
                     Warning("Variable '%s' is defined but never used",
                             Entry->Name);
                  }
               }
            }
...
/* Enter the program global lexical level */
{
   ...
   if (OutName) {
      ...
      if (DebugTableFile == 0) {
         Error("Cannot create table dump file '%s': %s", OutName,
               strerror(errno));
      }
   }
   else if (Debug) {
...
/* Leave the program global lexical level */
{
   ...
   if (DebugTableFile) {
      ...
      if (Entry) {
         ...
         do {
            if (!((Entry->Flags & SC_STRUCT) || (Entry->Flags & SC_UNION)) ||
                !Entry->V.S.SymTab) {
               continue;
            }
            ...
            PrintSymTable(Entry->V.S.SymTab, DebugTableFile,
                          SB_GetConstBuf(Header));
         } while ((Entry = Entry->NextSym));
      }

      /* Close the file */
      if (DebugTableFile != stdout && fclose(DebugTableFile) != 0) {
         Error("Error closing table dump file '%s': %s",
               SB_GetConstBuf(&DebugTableName), strerror(errno));
      }
   }
...
static SymEntry *FindSymInTable(const SymTable *T, const char *Name,
                                unsigned Hash)
...
static int IsDistinctRedef(const Type *lhst, const Type *rhst,
                           typecmpcode_t Code, typecmpflag_t Flags)
...
// Return true if there *is* an error.
{
   ...
   if (E_SCType == SC_TYPEDEF) {
      ...
      if (SCType == SC_TYPEDEF) {
         ...
         Error("Redefinition of typedef '%s' as different kind of symbol",
               Sym->Name);
         Sym = 0;
      }
   }
      ...
      if (IsTypeFunc(T)) {
         ...
         if (SymIsDef(Sym) && (Flags & SC_DEF) == SC_DEF) {
            Error("Body for function '%s' has already been defined", Sym->Name);
            Sym = 0;
         }
         else {
         ...
         Error("Redefinition of function '%s' as different kind of symbol",
               Sym->Name);
         Sym = 0;
      }
   }
...
SymEntry *AddEnumSym(const char *Name, unsigned Flags, const Type *Type,
                     SymTable *Tab, unsigned *DSFlags)
...
SymEntry *AddStructSym(const char *Name, unsigned Flags, unsigned Size,
                       SymTable *Tab, unsigned *DSFlags)
...
/* Add a bit field to the local symbol table and return the symbol entry */
{
   ...
   if (Entry) {
      ...
      if (!SignednessSpecified) {
         // int is treated as signed int everywhere except bit-fields; switch it
         // to unsigned, since this is allowed for bit-fields and avoids
         // sign-extension, so is much faster. enums set SignednessSpecified to
         // 1 to avoid this adjustment.  Character types actually distinguish 3
         // types of char; char may either be signed or unsigned, which is
         // controlled by `--signed-chars`.  In bit-fields, however, we perform
         // the same `char -> unsigned char` adjustment that is performed with
         ...
         if ((DOR->Flags & SC_DEF) && (Flags & SC_REF) &&
            ...
            if ((size_t)CollLast(AIC) != DOR->LocalsBlockId &&
               ...
               Warning("Goto at line %d to label %s jumps into a block with "
                       "initialization of an object that has automatic storage "
                       "duration",
                       GetCurrentLineNum(), Name);
            }
         }

         if ((DOR->Flags & SC_REF) && (DOR->Flags & (SC_GOTO | SC_GOTO_IND)) &&
             (Flags & SC_DEF)) {
            ...
            if (DOR->Flags & SC_GOTO) {
               ...
               // Optimizer will need the information about the value of SP
               // adjustment later, so let's preserve it.
      ...
      if (HandleSymRedefinition(Entry, T, Flags)) {
         ...
         if (SymIsDef(Entry) && (Flags & SC_DEF) == SC_DEF) {
            ...
            if ((Flags & SC_STORAGEMASK) == SC_AUTO ||
               ...
               Error("Declaration of '%s' with no linkage follows extern "
                     "declaration",
                     Name);
               Entry = 0;
            }
...
/* Add an external or global symbol to the symbol table and return the entry */
{
   ...
   if (IsTypeFunc(Entry->Type)) {
      ...
      if (SymTab == SymTab0 &&
          ((Flags & SC_STORAGEMASK) == SC_EXTERN || (Flags & SC_INLINE) == 0)) {
         Entry->Flags |= SC_NOINLINEDEF;
      }
   }

   /* Add an alias of the global symbol to the local symbol table */
   if (Tab == SymTab0 && SymTab != SymTab0 && Entry->Owner != SymTab &&
       Alias == 0) {
...
/* Write import/export statements for external symbols */
{
   ...
   while (Entry) {
      unsigned Flags = Entry->Flags;
      if (Entry->Owner == SymTab0 && (Flags & SC_STORAGEMASK) != SC_STATIC &&
          ((Flags & SC_TYPEMASK) == SC_FUNC ||
           (Flags & SC_TYPEMASK) == SC_NONE)) {
         ...
         if (SymIsRef(Entry) && !SymIsDef(Entry)) {
            ...
            g_defimport(Entry->Name, Flags & SC_ZEROPAGE);
                  ...
                  ((Flags & SC_NOINLINEDEF) || (Flags & SC_INLINE) == 0)) {
            /* An export */
      ...
      while (Sym) {
         if ((Sym->Flags & SC_TYPEMASK) == 0) {
            if ((Sym->Flags & SC_STORAGEMASK) == SC_AUTO) {
               AddTextLine("%s, \"%s\", \"00\", auto, %d", Head, Sym->Name,
                           Sym->V.Offs);
            }
               ...
               AddTextLine("%s, \"%s\", \"00\", %s, \"%s\"", Head, Sym->Name,
