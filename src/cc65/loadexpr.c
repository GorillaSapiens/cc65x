   ...
   if (!ED_IsAddrExpr(Expr)) {
      ...
      if ((Flags & CF_TYPEMASK) == 0) {
         if (IsTypeFragBitField(Expr->Type)) {
            ...
            // Flags we need operate on the whole chunk containing the
            // bit-field.
            Flags |= CG_TypeOf(GetBitFieldChunkType(Expr->Type));

            // If we're adjusting, then only load a char (not an int) and do
            // only char ops; We will clear the high byte in the adjustment.
            // CF_FORCECHAR does nothing if the type is not CF_CHAR; If
            // adjusting, then we're sign extending manually, so do everything
            // unsigned to make shifts faster.
      ...
      if (ED_YetToTest(Expr)) {
         ...
         // Setting CF_TEST will cause the load to perform optimizations and not
         // actually load all bits of the bit-field, instead just computing the
         // condition codes.  Therefore, if adjustment is required, we do not
         // set CF_TEST here, but handle it below.
         if (!AdjustBitField) {
            Flags |= CF_TEST;
         }
      }
      ...
      switch (ED_GetLoc(Expr)) {
         ...
         case E_LOC_GLOBAL:
            // if (CG_TypeOf (Expr->Type) == CF_FLOAT) { printf("%s:%d FIXME:
            // E_LOC_GLOBAL\n", __FILE__, __LINE__); exit(-1); } Global
            // variable, offset in IVal
            g_getstatic(Flags | CF_EXTERNAL, Expr->Name, Expr->IVal);
            break;

         case E_LOC_STATIC:
            // if (CG_TypeOf (Expr->Type) == CF_FLOAT) { printf("%s:%d FIXME:
            // E_LOC_STATIC\n", __FILE__, __LINE__); exit(-1); } Static
            // variable, offset in IVal
         ...
         case E_LOC_LITERAL:
            // if (CG_TypeOf (Expr->Type) == CF_FLOAT) { printf("%s:%d FIXME:
            // E_LOC_LITERAL\n", __FILE__, __LINE__); exit(-1); } Literal in the
            // literal pool, offset in IVal
            g_getstatic(Flags | CF_LITERAL, Expr->Name, Expr->IVal);
            break;

         case E_LOC_REGISTER:
            // if (CG_TypeOf (Expr->Type) == CF_FLOAT) { printf("%s:%d FIXME:
            // E_LOC_REGISTER\n", __FILE__, __LINE__); exit(-1); } Register
            // variable, offset in IVal
         ...
         case E_LOC_CODE:
            // if (CG_TypeOf (Expr->Type) == CF_FLOAT) { printf("%s:%d FIXME:
            // E_LOC_CODE\n", __FILE__, __LINE__); exit(-1); } Code label
            // location, offset in IVal
            g_getstatic(Flags | CF_CODE, Expr->Name, Expr->IVal);
            break;

         case E_LOC_STACK:
            // if (CG_TypeOf (Expr->Type) == CF_FLOAT) { printf("%s:%d FIXME:
            // E_LOC_STACK\n", __FILE__, __LINE__); exit(-1); } Fetch value on
            // the stack (with offset in IVal)
      ...
      // Handle bit fields if necessary. The actual type may have been casted or
      // converted, so be sure to always use unsigned ints for the operations.
      if (AdjustBitField) {
         // We always need to do something with the low byte, so there is no
         // opportunity for optimization by skipping it.
         ...
         if (ED_YetToTest(Expr)) {
            ...
            g_extractbitfield(Flags, BitFieldFullWidthFlags,
                              IsSignSigned(Expr->Type), Expr->Type->A.B.Offs,
                              Expr->Type->A.B.Width);
         }
      }
