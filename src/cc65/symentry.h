...
#define SC_ASSIGN_INIT                                                         \
   0x2000U /* Symbol is to be initialized with assignment code */
...
#define SC_NOINLINEDEF                                                         \
   0x800000U /* Symbol may never have an inline definition */
...
{
   return ((Sym->Flags & (SC_STORAGEMASK | SC_TYPEMASK)) ==
           (SC_REGISTER | SC_NONE));
}
#else
#define SymIsRegVar(Sym)                                                       \
   (((Sym)->Flags & (SC_STORAGEMASK | SC_TYPEMASK)) == (SC_REGISTER | SC_NONE))
#endif
...
#else
#define SymHasFlexibleArrayMember(Sym)                                         \
   (((Sym)->Flags & SC_HAVEFAM) == SC_HAVEFAM)
#endif
