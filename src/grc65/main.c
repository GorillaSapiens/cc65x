...
// - err, maybe free allocated memory, huh? (who cares, it's just a little
// prog...)
...
const char *hdrFields[] = {"author", "info",      "date", "dostype",
                           "mode",   "structure", "icon", ""};
...
const char *memFields[] = {"stacksize", "overlaysize", "overlaynums",
                           "backbuffer", ""};

const int BSWTab[] = {
    0,     0x005, 0x007, 0x00b, 0x011, 0x017, 0x01d, 0x023, 0x025, 0x029, 0x02d,
    0x033, 0x039, 0x03c, 0x041, 0x043, 0x04a, 0x04f, 0x052, 0x056, 0x05a, 0x05f,
    0x063, 0x068, 0x06d, 0x072, 0x077, 0x079, 0x07c, 0x080, 0x084, 0x088, 0x08e,
    0x094, 0x09a, 0x09f, 0x0a4, 0x0a9, 0x0ad, 0x0b1, 0x0b6, 0x0bc, 0x0be, 0x0c2,
    0x0c8, 0x0cc, 0x0d4, 0x0da, 0x0e0, 0x0e5, 0x0eb, 0x0f0, 0x0f5, 0x0f9, 0x0fe,
    0x104, 0x10c, 0x112, 0x118, 0x11e, 0x121, 0x129, 0x12c, 0x132, 0x13a, 0x13e,
    0x143, 0x148, 0x14d, 0x152, 0x157, 0x15a, 0x15f, 0x164, 0x166, 0x168, 0x16d,
    0x16f, 0x177, 0x17c, 0x182, 0x187, 0x18c, 0x18f, 0x193, 0x196, 0x19b, 0x1a1,
...
static int findToken(const char *const *tokenTbl, const char *token) {
   /* takes as input table of tokens and token, returns position in table or -1
    * if not found */
   int i;
...
static char *nextPhrase(void) { return strtok(NULL, "\""); }

static char *nextWord(void) { return strtok(NULL, " "); }
...
static void DoMenu(void) {
   ...
   if (item == 0)
      AbEnd("Menu '%s' has 0 items!", myMenu.name);
   if (item > 31)
      AbEnd("Menu '%s' has too many items!", myMenu.name);
   ...
   for (a = 0; a != item; a++) {
       ...
       */
      if ((strstr(curItem->type, "MENU_ACTION") != NULL) ||
          (strstr(curItem->type, "DYN_SUB_MENU") != NULL)) {
   ...
   fprintf(outputCFile,
           ...
           "\t(int)%i, (int)%i,\n"
           "\t(char)(%i | %s),\n",
           myMenu.name, myMenu.top, myMenu.bot, myMenu.left, myMenu.right, item,
           myMenu.type);

   curItem = myMenu.item;
   for (a = 0; a != item; a++) {
      fprintf(outputCFile, "\t%s, (char)%s, (int)", curItem->name,
              curItem->type);
      if ((strstr(curItem->type, "SUB_MENU") != NULL) &&
          (strstr(curItem->type, "DYN_SUB_MENU") == NULL)) {
         fprintf(outputCFile, "&");
      }
...
static void DoHeader(void) {
   ...
   if (apple == 1) {
      ...
      fprintf(outputSFile, "\t.byte %i << 4 | %u\n", myHead.structure + 2,
              (unsigned)strlen(myHead.dosname));
      ...
      fprintf(outputSFile,
              ...
              myHead.geostype,
              myHead.structure == 0
                  ? "__VLIR0_LAST__ - __VLIR0_START__ - __BSS_SIZE__"
                  : "0",
      ...
      fprintf(outputSFile,
              ...
              myHead.structure, myHead.geostype, myHead.year, myHead.month,
              myHead.day, myHead.hour, myHead.min);
   }

   fprintf(outputSFile, "\t\t.segment \"FILEINFO\"\n\n"
                        "\t.import __VLIR0_START__, __STARTUP_RUN__\n\n"
                        "\t.byte 3, 21, 63 | $80\n");

   if (myHead.icon != NULL) {
      fprintf(outputSFile, "\t.incbin \"%s\", 0, 63\n", myHead.icon);
   }
   else {
      for (i = 0; i != 63; i = i + 3) {
         fprintf(outputSFile, "\t.byte %%%s, %%%s, %%%s\n",
                 bintos(icon1[i], i1), bintos(icon1[i + 1], i2),
                 bintos(icon1[i + 2], i3));
      }
   }
...
static void DoMemory(void) {
   ...
   if (lastnumber != -1) {
         ...
         for (number = 0; number <= lastnumber; number++) {
            if (overlaytable[number] == 1) {
               fprintf(
                   ...
                   number, number, number, number == 0 ? ", __BSS_SIZE__" : "",
                   number, number, number == 0 ? " - __BSS_SIZE__" : "", number,
                   number, number == 0 ? " - __BSS_SIZE__" : "");
            }
         }
      }
      else {
      ...
      fprintf(outputCFile, "extern void _OVERLAYADDR__[];\n"
   ...
   if (stacksize != -1) {
      fprintf(outputSFile, "\t.export __STACKSIZE__ : absolute = $%04x\n\n",
              stacksize);
   }

   if (overlaysize != -1) {
      fprintf(outputSFile, "\t.export __OVERLAYSIZE__ : absolute = $%04x\n\n",
              overlaysize);
   }

   if (backbuffer != -1) {
      fprintf(outputSFile, "\t.export __BACKBUFSIZE__ : absolute = $%04x\n\n",
...
static char *filterInput(FILE *F, char *tbl) {
   ...
   while (1) {
      if (i >= BLOODY_BIG_BUFFER) {
         AbEnd("File too large for internal parsing buffer (%d bytes)",
               BLOODY_BIG_BUFFER);
      }
      if (((a == '\n') || (a == '\015')) || (a == ',' && quote)) {
         a = ' ';
      ...
      if (IsSpace(a)) {
         ...
         if (a == ';' && quote) {
            ...
            // which is invalid for text mode files, and was unreliable across
            // platforms.
            continue;
         }
         else {
   ...
   if (bracket != 0)
      AbEnd("There are unclosed brackets!");
