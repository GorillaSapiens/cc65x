...
static const CmdEntry MainCmds[] = {
    {"exit", 0, 1, CmdQuit},
    {"help", "Show available commands", 1, CmdHelp},
    {"load", "Load a debug info file", 2, CmdLoad},
    {"quit", "Terminate the shell", 1, CmdQuit},
    {"show", "Show items from the info file", -2, CmdShow},
    {"unload", "Unload a debug info file", 1, CmdUnload},
};

/* Table with show commands */
static const CmdEntry ShowCmds[] = {
    {"childscopes", "Show child scopes of other scopes.", -2,
     CmdShowChildScopes},
    {"csym", 0, -1, CmdShowCSymbol},
    {"csymbol", "Show c symbols.", -1, CmdShowCSymbol},
    {"func", 0, -2, CmdShowFunction},
    {"function", "Show c functions.", -2, CmdShowFunction},
    {"help", "Show available subcommands.", 1, CmdShowHelp},
    {"line", "Show line info. May be followed by one or more line ids.", -2,
     CmdShowLine},
    {"library", "Show libraries. May be followed by one or more library ids.",
     -1, CmdShowLibrary},
    {"module", "Show modules. May be followed by one or more module ids.", -1,
     CmdShowModule},
    {"scope", "Show scopes. May be followed by one or more segment ids.", -1,
     CmdShowScope},
    {"segment", "Show segments. May be followed by one or more segment ids.",
     -1, CmdShowSegment},
    {"source", "Show sources. May be followed by one or more source file ids.",
     -1, CmdShowSource},
    {"span", "Show spans. May be followed by one or more span ids.", -1,
     CmdShowSpan},
    {"symbol",
     "Show symbols. May be followed by one or more symbol or scope ids.", -2,
     CmdShowSymbol},
    {"symdef",
     "Show where a symbol was defined. May be followed by one or more symbol "
     "ids.",
     -2, CmdShowSymDef},
    {"symref",
     "Show where a symbol was referenced. May be followed by one or more "
     "symbol ids.",
     -2, CmdShowSymRef},
    {"type", "Show type information. May be followed by one or more type ids.",
     -2, CmdShowType},
};
...
{
   /* Output a message */
   PrintLine("%s:%s(%lu): %s", Info->type ? "Error" : "Warning", Info->name,
             (unsigned long)Info->line, Info->errormsg);
...
static const CmdEntry *FindCmd(const char *Cmd, const CmdEntry *Tab,
                               unsigned Count)
...
// diagnostic.
{
   ...
   if (E->ArgCount > 0 && (int)CollCount(Args) != E->ArgCount) {
      ...
      switch (E->ArgCount) {
         ...
         default:
            PrintLine("Command requires at least %d arguments",
                      E->ArgCount - 1);
            return;
      }
   }
...
/* Find an id type by its name. Returns the type or InvalidId. */
{
   static const struct {
      ...
      unsigned Type;
       ...
       {"l", LineId},        {"lib", LibraryId}, {"library", LibraryId},
       {"line", LineId},     {"m", ModuleId},    {"mod", ModuleId},
       {"module", ModuleId}, {"s", SymbolId},    {"sc", ScopeId},
       {"scope", ScopeId},   {"seg", SegmentId}, {"segment", SegmentId},
       {"source", SourceId}, {"src", SourceId},  {"c_sp", SpanId},
       {"span", SpanId},     {"sym", SymbolId},  {"symbol", SymbolId},
...
{
   /* Header */
   PrintLine("  id  name                        type  kind   sc   offs  symbol "
             "scope");
   PrintSeparator();
}
...
/* Output a header for a list of scopes */
{
   /* Header */
   PrintLine(
       "  id    name                    type    size   parent  symbol  module");
   PrintSeparator();
}
...
/* Output a header for a list of segments */
{
   /* Header */
   PrintLine(
       "  id    name            address  size   output file     offs   bank");
   PrintSeparator();
}
...
/* Output a header for a list of source files */
{
   /* Header */
   PrintLine(
       "  id    name                            size   modification time");
   PrintSeparator();
}
...
{
   /* Header */
   PrintLine("  id  name                    type  size  value  export  seg  "
             "scope parent");
   PrintSeparator();
}
