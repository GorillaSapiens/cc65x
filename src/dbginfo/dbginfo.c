...
// Prepare for insertion of the data at a given position in the collection
{
   ...
   if (C->Count != Index) {
      memmove(C->Items + Index + 1, C->Items + Index,
              (C->Count - Index) * sizeof(void *));
   }
   ++C->Count;
}
...
// Dump a list of file infos
{
   ...
   for (I = 0; I < CollCount(FileInfos); ++I) {
      ...
      printf("File info %u:\n"
             ...
             "  Size:   %lu\n"
             "  MTime:  %lu\n",
             FI->Id, FI->Name, (unsigned long)FI->Size,
...
// Dump one line info entry
{
   printf("  Index:  %u\n"
          ...
          "    Type:   %u\n"
          "    Count:  %u\n",
          Num, LI->File.Info->Name, (unsigned long)LI->Line,
          (unsigned long)LI->Start, (unsigned long)LI->End, LI->Type,
          LI->Count);
}
...
{
   cc65_csyminfo *S =
       xmalloc(sizeof(*S) - sizeof(S->data[0]) + Count * sizeof(S->data[0]));
   S->count = Count;
   return S;
}
...
// Helper function to sort file infos in a collection by name
{
   ...
   // together.
   int Res = strcmp(((const FileInfo *)L)->Name, ((const FileInfo *)R)->Name);
   if (Res != 0) {
      return Res;
   }
...
{
   cc65_libraryinfo *L =
       xmalloc(sizeof(*L) - sizeof(L->data[0]) + Count * sizeof(L->data[0]));
   L->count = Count;
   return L;
}
...
{
   cc65_moduleinfo *M =
       xmalloc(sizeof(*M) - sizeof(M->data[0]) + Count * sizeof(M->data[0]));
   M->count = Count;
   return M;
}
...
static SegInfo *NewSegInfo(const StrBuf *Name, unsigned Id, cc65_addr Start,
                           cc65_addr Size, const StrBuf *OutputName,
                           unsigned long OutputOffs, unsigned Bank)
...
{
   // Sort by file name
   return strcmp(((const SegInfo *)L)->Name, ((const SegInfo *)R)->Name);
}
...
{
   // Sort by symbol name
   return strcmp(((const SymInfo *)L)->Name, ((const SymInfo *)R)->Name);
}
   ...
   switch (B) {
      ...
      default:
         if (GT_GET_TYPE(B) == GT_TYPE_ARRAY) {
            ...
            if (!P->Error) {
               Data->size =
                   Data->data.array.ele_count * Data->data.array.ele_type->size;
            }
         }
         else {
            // OOPS!
...
// Read the next token from the input stream
{
   ...
   if (D->C == '_' || isalpha(D->C)) {
      ...
      Entry = bsearch(SB_GetConstBuf(&D->SVal), KeywordTable,
...
// Parse a CSYM line
{
   ...
   while (1) {
      ...
      // Something we know?
      if (D->Tok != TOK_ID && D->Tok != TOK_NAME && D->Tok != TOK_OFFS &&
          D->Tok != TOK_SC && D->Tok != TOK_SCOPE && D->Tok != TOK_SYM &&
          D->Tok != TOK_TYPE) {
   ...
   if (((InfoBits & ibSymId) != 0) && (SC == CC65_CSYM_AUTO)) {
      ParseError(D, CC65_ERROR,
                 "Only non auto symbols can have a symbol attached");
      goto ErrorExit;
   }
...
// Parse a FILE line
{
   ...
   while (1) {
      ...
      // Something we know?
      if (D->Tok != TOK_ID && D->Tok != TOK_MODULE && D->Tok != TOK_MTIME &&
...
// Parse an INFO line
{
   ...
   while (1) {
      ...
      // Something we know?
      if (D->Tok != TOK_CSYM && D->Tok != TOK_FILE && D->Tok != TOK_LIBRARY &&
          D->Tok != TOK_LINE && D->Tok != TOK_MODULE && D->Tok != TOK_SCOPE &&
          D->Tok != TOK_SEGMENT && D->Tok != TOK_SPAN && D->Tok != TOK_SYM &&
          D->Tok != TOK_TYPE) {
...
// Parse a LINE line
{
   ...
   while (1) {
      ...
      // Something we know?
      if (D->Tok != TOK_COUNT && D->Tok != TOK_FILE && D->Tok != TOK_ID &&
          D->Tok != TOK_LINE && D->Tok != TOK_SPAN && D->Tok != TOK_TYPE) {
...
// Parse a MODULE line
{
   ...
   while (1) {
      ...
      // Something we know?
      if (D->Tok != TOK_FILE && D->Tok != TOK_ID && D->Tok != TOK_NAME &&
          D->Tok != TOK_LIBRARY) {
...
// Parse a SCOPE line
{
   ...
   while (1) {
      ...
      // Something we know?
      if (D->Tok != TOK_ID && D->Tok != TOK_MODULE && D->Tok != TOK_NAME &&
          D->Tok != TOK_PARENT && D->Tok != TOK_SIZE && D->Tok != TOK_SPAN &&
      ...
      switch (Tok) {
         ...
         case TOK_TYPE:
            switch (D->Tok) {
               case TOK_GLOBAL:
                  Type = CC65_SCOPE_GLOBAL;
                  break;
               case TOK_FILE:
                  Type = CC65_SCOPE_MODULE;
                  break;
               case TOK_SCOPE:
                  Type = CC65_SCOPE_SCOPE;
                  break;
               case TOK_STRUCT:
                  Type = CC65_SCOPE_STRUCT;
                  break;
               case TOK_ENUM:
                  Type = CC65_SCOPE_ENUM;
                  break;
               default:
                  ParseError(D, CC65_ERROR,
...
// Parse a SEGMENT line
{
   ...
   while (1) {
      ...
      // Something we know?
      if (D->Tok != TOK_ADDRSIZE && D->Tok != TOK_BANK && D->Tok != TOK_ID &&
...
// Parse a SPAN line
{
   ...
   while (1) {
      ...
      // Something we know?
      if (D->Tok != TOK_ID && D->Tok != TOK_SEGMENT && D->Tok != TOK_SIZE &&
          D->Tok != TOK_START && D->Tok != TOK_TYPE) {
...
// Parse a SYM line
{
   ...
   while (1) {
      ...
      // Something we know?
      if (D->Tok != TOK_ADDRSIZE && D->Tok != TOK_DEF && D->Tok != TOK_EXPORT &&
          D->Tok != TOK_FILE && D->Tok != TOK_ID && D->Tok != TOK_NAME &&
          D->Tok != TOK_PARENT && D->Tok != TOK_REF && D->Tok != TOK_SCOPE &&
          D->Tok != TOK_SEGMENT && D->Tok != TOK_SIZE && D->Tok != TOK_TYPE &&
          D->Tok != TOK_VALUE) {
   ...
   if ((InfoBits & (ibScopeId | ibParentId)) == 0 ||
      ...
      ParseError(D, CC65_ERROR,
                 "Only one of \"parent\", \"scope\" must be specified");
      goto ErrorExit;
   }
...
static SpanInfoListEntry *FindSpanInfoByAddr(const SpanInfoList *L,
                                             cc65_addr Addr)
...
// Postprocess c symbol infos
{
   ...
   for (I = 0; I < CollCount(&D->Info->CSymInfoById); ++I) {
      ...
      if (S->Sym.Id == CC65_INV_ID) {
         ...
         ParseError(D, CC65_ERROR,
                    "Invalid symbol id %u for c symbol with id %u", S->Sym.Id,
                    S->Id);
         S->Sym.Info = 0;
      }
      else {
      ...
      if (S->Type.Id >= CollCount(&D->Info->TypeInfoById)) {
         ParseError(D, CC65_ERROR, "Invalid type id %u for c symbol with id %u",
                    S->Type.Id, S->Id);
         S->Type.Info = 0;
      }
      else {
      ...
      if (S->Scope.Id >= CollCount(&D->Info->ScopeInfoById)) {
         ParseError(D, CC65_ERROR,
                    "Invalid scope id %u for c symbol with id %u", S->Scope.Id,
                    S->Id);
         S->Scope.Info = 0;
      }
      else {
   ...
   for (I = 0; I < CollCount(&D->Info->FileInfoById); ++I) {
      ...
      for (J = 0; J < CollCount(&F->ModInfoByName); ++J) {
         ...
         if (ModId >= CollCount(&D->Info->ModInfoById)) {
            ParseError(D, CC65_ERROR,
                       "Invalid module id %u for file with id %u", ModId,
                       F->Id);
...
// Postprocess line infos
{
   ...
   for (I = 0; I < CollCount(LineInfos); ++I) {
      ...
      if (L->File.Id >= CollCount(FileInfos)) {
         ParseError(D, CC65_ERROR, "Invalid file id %u for line with id %u",
                    L->File.Id, L->Id);
         L->File.Info = 0;
      }
      else {
      ...
      for (J = 0; J < CollCount(&L->SpanInfoList); ++J) {
         ...
         if (SpanId >= CollCount(&D->Info->SpanInfoById)) {
            ParseError(D, CC65_ERROR, "Invalid span id %u for line with id %u",
                       SpanId, L->Id);
...
// Postprocess module infos
{
   ...
   for (I = 0; I < CollCount(&D->Info->ModInfoById); ++I) {
      ...
      if (M->File.Id >= CollCount(&D->Info->FileInfoById)) {
         ParseError(D, CC65_ERROR, "Invalid file id %u for module with id %u",
                    M->File.Id, M->Id);
         M->File.Info = 0;
      }
      else {
      ...
      if (M->Lib.Id == CC65_INV_ID) {
         ...
         ParseError(D, CC65_ERROR,
                    "Invalid library id %u for module with id %u", M->Lib.Id,
                    M->Id);
         M->Lib.Info = 0;
      }
      else {
...
// Postprocess scope infos
{
   ...
   for (I = 0; I < CollCount(&D->Info->ScopeInfoById); ++I) {
      ...
      if (S->Mod.Id >= CollCount(&D->Info->ModInfoById)) {
         ParseError(D, CC65_ERROR, "Invalid module id %u for scope with id %u",
                    S->Mod.Id, S->Id);
         S->Mod.Info = 0;
      }
      else {
      ...
      if (S->Label.Id == CC65_INV_ID) {
         ...
         ParseError(D, CC65_ERROR, "Invalid label id %u for scope with id %u",
                    S->Label.Id, S->Id);
         S->Label.Info = 0;
      }
      else {
      ...
      for (J = 0; J < CollCount(&S->SpanInfoList); ++J) {
         ...
         if (SpanId >= CollCount(&D->Info->SpanInfoById)) {
            ParseError(D, CC65_ERROR, "Invalid span id %u for scope with id %u",
                       SpanId, S->Id);
   ...
   for (I = 0; I < CollCount(&D->Info->ModInfoById); ++I) {
      ...
      if (M->MainScope == 0) {
         ParseError(D, CC65_ERROR, "Module with id %u has no main scope",
                    M->Id);
      }
...
// Postprocess span infos
{
   ...
   for (I = 0; I < CollCount(&D->Info->SpanInfoById); ++I) {
      ...
      if (S->Seg.Id >= CollCount(&D->Info->SegInfoById)) {
         ParseError(D, CC65_ERROR, "Invalid segment id %u for span with id %u",
                    S->Seg.Id, S->Id);
         S->Seg.Info = 0;
      }
      else {
      ...
      if (S->Type.Id == CC65_INV_ID) {
         ...
         ParseError(D, CC65_ERROR, "Invalid type id %u for span with id %u",
                    S->Type.Id, S->Id);
         S->Type.Info = 0;
      }
      else {
...
// Postprocess symbol infos
{
   ...
   for (I = 0; I < CollCount(&D->Info->SymInfoById); ++I) {
      ...
      if (S->Exp.Id == CC65_INV_ID) {
         ...
         ParseError(D, CC65_ERROR, "Invalid export id %u for symbol with id %u",
                    S->Exp.Id, S->Id);
         S->Exp.Info = 0;
      }
      else {
      ...
      if (S->Seg.Id == CC65_INV_ID) {
         ...
         ParseError(D, CC65_ERROR,
                    "Invalid segment id %u for symbol with id %u", S->Seg.Id,
                    S->Id);
         S->Seg.Info = 0;
      }
      else {
      ...
      if (S->Scope.Id == CC65_INV_ID) {
         ...
         ParseError(D, CC65_ERROR, "Invalid scope id %u for symbol with id %u",
                    S->Scope.Id, S->Id);
         S->Scope.Info = 0;
      }
      else {
      ...
      if (S->Parent.Id == CC65_INV_ID) {
         ...
         ParseError(D, CC65_ERROR, "Invalid parent id %u for symbol with id %u",
                    S->Parent.Id, S->Id);
         S->Parent.Info = 0;
      }
      else {
      ...
      for (J = 0; J < CollCount(&S->DefLineInfoList); ++J) {
         ...
         if (LineId >= CollCount(&D->Info->LineInfoById)) {
            ParseError(D, CC65_ERROR,
                       "Invalid line id %u for symbol with id %u", LineId,
                       S->Id);
   ...
   for (I = 0; I < CollCount(&D->Info->SymInfoById); ++I) {
      ...
      if (S->Scope.Info == 0) {
         ...
         if (S->Parent.Info == 0) {
            ParseError(D, CC65_ERROR,
                       "Symbol with id %u has no parent and no scope", S->Id);
         }
         else if (S->Parent.Info->Scope.Info == 0) {
            ParseError(D, CC65_ERROR,
                       "Symbol with id %u has parent %u without a scope", S->Id,
                       S->Parent.Info->Id);
         }
         else {
...
// read successfully, NULL is returned.
{
   ...
   if (D.F == 0) {
      // Cannot open
      ParseError(&D, CC65_ERROR, "Cannot open input file \"%s\": %s", FileName,
                 strerror(errno));
      return 0;
   }
   ...
   if (D.Info->MajorVersion < VER_MAJOR) {
      ParseError(&D, CC65_ERROR,
                 ...
                 "= %u.%u",
                 D.Info->MajorVersion, D.Info->MinorVersion, VER_MAJOR,
                 VER_MINOR);
      goto CloseAndExit;
   }
      ...
      ParseError(
          ...
          D.Info->MajorVersion, D.Info->MinorVersion, VER_MAJOR, VER_MINOR);
   }
   else if (D.Info->MajorVersion > VER_MAJOR) {
