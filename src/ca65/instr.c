...
// Emit a TAMn instruction (HuC6280). Because this is a two-byte instruction
// with implicit addressing mode, the opcode byte in the table is actually the
...
// Emit a TMAn instruction (HuC6280). Because this is a two-byte instruction
// with implicit addressing mode, the opcode byte in the table is actually the
...
// Note that JMP [abs] and JML [abs] are always both permitted for instruction
// $DC, because the [] notation for long indirection makes the generated
...
// CAUTION: in the pdf $1a is dec a, and $3a is inc a - if that is really the
// case, the table below (and the handler) should be fixed and this notice
// removed
...
static const struct {
   unsigned Count;
   InsDesc Ins[106];
} InsTabm740 = {sizeof(InsTabm740.Ins) / sizeof(InsTabm740.Ins[0]),
                {
                    // BEGIN SORTED.SH
...
static const InsTable *InsTabs[CPU_COUNT] = {
    (const InsTable *)&InsTabNone,    (const InsTable *)&InsTab6502,
    (const InsTable *)&InsTab6502X,   (const InsTable *)&InsTab6502DTV,
    (const InsTable *)&InsTab65SC02, // original CMOS
    (const InsTable *)&InsTab65C02,  // CMOS with Rockwell extensions
    (const InsTable *)&InsTab65816,   (const InsTable *)&InsTabSweet16,
    (const InsTable *)&InsTabHuC6280,
    (const InsTable *)&InsTabm740, // Mitsubishi 740
    (const InsTable *)&InsTab4510,    (const InsTable *)&InsTab45GS02,
...
static unsigned char EATab[16][AM65I_COUNT] = {
    {// Table 0 (sec, sed, seo, set, slw, sta, stp, tax, tay, tsx, txa, txs,
     // tya)
     0x00, 0x00, 0x05, 0x0D, 0x0F, 0x15, 0x1D, 0x1F, 0x00, 0x19, 0x12,
     0x00, 0x07, 0x11, 0x17, 0x01, 0x00, 0x00, 0x00, 0x03, 0x13, 0x09,
     0x00, 0x09, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x12, 0x00},
    {// Table 1 (rol, ror, stx, sty, tst, cpx, cpy)
     0x08, 0x08, 0x04, 0x0C, 0x00, 0x14, 0x1C, 0x00, 0x14, 0x1C, 0x00,
     0x80, 0x00, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
     0x00, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00},
    {// Table 2 (bit)
     0x00, 0x00, 0x24, 0x2C, 0x0F, 0x34, 0x3C, 0x00, 0x00, 0x00, 0x00,
     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x89,
     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
    {// Table 3 (dec, dea)
     0x3A, 0x3A, 0xC6, 0xCE, 0x00, 0xD6, 0xDE, 0x00, 0x00, 0x00, 0x00,
     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
    {// Table 4 (inc)
     0x1A, 0x1A, 0xE6, 0xEE, 0x00, 0xF6, 0xFE, 0x00, 0x00, 0x00, 0x00,
     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
    {// Table 5 (stz)
     0x00, 0x00, 0x60, 0x98, 0x00, 0x70, 0x9E, 0x00, 0x00, 0x00, 0x00,
     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
    {// Table 6 (jmp, rrf)
     0x00, 0x00, 0x00, 0x00, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
     0x20, 0x00, 0x00, 0x00, 0x00, 0x30, 0x00, 0x00, 0x00, 0x00, 0x00,
     0x00, 0x00, 0x00, 0x00, 0x90, 0x00, 0x00, 0x00, 0x00, 0x00},
    {// Table 7 (jsr)
     0x00, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
     0x02, 0x00, 0x00, 0x00, 0x00, 0xDC, 0x00, 0x00, 0x00, 0x00, 0x00,
     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
    {// Table 8
     0x00, 0x40, 0x01, 0x41, 0x00, 0x09, 0x49, 0x00, 0x00, 0x00, 0x00,
     0x51, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x40,
     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
    {// Table 9 (dew, inw)
     0x00, 0x00, 0x00, 0x10, 0x00, 0x20, 0x30, 0x00, 0x00, 0x00, 0x00,
     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
    {// Table 10 (NOPs, clbX, sebX)
     0xea, 0x00, 0x04, 0x0c, 0x00, 0x14, 0x1c, 0x00, 0x00, 0x00, 0x00,
     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
     0x00, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
    {// Table 11 (LAX)
     0x08, 0x08, 0x04, 0x0C, 0x00, 0x14, 0x1C, 0x00, 0x14, 0x1C, 0x00,
     0x80, 0x00, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
     0x00, 0x08, 0x00, 0x00, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00},
    {// Table 12 (m740: JMP)
     0x00, 0x00, 0x00, 0x4c, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xb2,
     0x6c, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
    {// Table 13 (Q)
     0x00, 0x00, 0x05, 0x0D, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x12,
    ...
    {// Table 14 (45GS02: ASLQ)
     0x0a, 0x0a, 0x06, 0x0e, 0x00, 0x16, 0x1e, 0x00, 0x00, 0x00, 0x12,
     0x00, 0x12, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0a},
    {// Table 15 (45GS02: ASRQ)
     0x03, 0x03, 0x04, 0x0c, 0x00, 0x14, 0x00, 0x00, 0x00, 0x00, 0x00,
     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
// this function. The function returns true on success and false on errors.
{
   ...
   if (A->AddrMode == AM65I_IMM_ACCU || A->AddrMode == AM65I_IMM_INDEX ||
       ...
       A->AddrMode == AM65I_IMM_IMPLICIT_WORD) {
...
// Output code for the data in A
{
   ...
   switch (ExtBytes[A->AddrMode]) {
      ...
      case 2:
         if (CPU == CPU_65816 &&
             (A->AddrModeBit &
              (AM65_ABS | AM65_ABS_X | AM65_ABS_Y | AM65_ABS_X_IND))) {
...
// Handle a generic sweet16 instruction
{
   ...
   switch (Sweet16ExtBytes[A.AddrMode]) {
      ...
      default:
         Internal("Invalid operand byte count: %u",
                  Sweet16ExtBytes[A.AddrMode]);
   }
}
