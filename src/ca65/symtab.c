...
SymTable *SymFindScope(SymTable *Parent, const StrBuf *Name,
                       SymFindAction Action)
...
SymEntry *SymFindLocal(SymEntry *Parent, const StrBuf *Name,
                       SymFindAction Action)
...
// scope.
{
   ...
   do {
      ...
      // scopes.
      if (SymSearchTree(Scope->Table[Hash % Scope->TableSlots], Name, &Sym) ==
          0) {
...
/* Handle an undefined symbol */
{
   ...
   if ((S->Flags & SF_FIXED) == 0) {
      ...
      while (Tab) {
         Sym = SymFind(Tab, GetStrBuf(S->Name),
                       SYM_FIND_EXISTING | SYM_CHECK_ONLY);
   ...
   if (Sym) {
      ...
      if (S->Flags & SF_EXPORT) {
         if (Sym->Flags & SF_IMPORT) {
            ...
            LIError(&S->RefLines, "Symbol '%s' is already an import",
                    GetString(Sym->Name));
         }
         if ((Sym->Flags & SF_EXPORT) == 0) {
            ...
            if (Sym->AddrSize > Sym->ExportSize) {
               ...
               LIWarning(&Sym->DefLines, 1,
                         "Symbol '%m%p' is %s but exported %s", GetSymName(Sym),
      ...
      if (S->Flags & SF_EXPORT) {
         ...
         LIError(&S->RefLines, "Exported symbol '%m%p' was never defined",
                 GetSymName(S));
      }
      else {
         if (AutoImport) {
            ...
            /* Error */
            LIError(&S->RefLines, "Symbol '%m%p' is undefined", GetSymName(S));
         }
      }
   }
}
...
/* Run through all symbols and check for anomalies and errors */
{
   ...
   if (CurrentScope->Parent != 0) {
      if (CurrentScope->Label) {
         ...
         // scope has no label to track a line number, uses end-of-document line
         // instead. Anonymous scopes will reveal their internal automatic name.
   ...
   while (S) {
      if ((S->Flags & SF_UNUSED) == 0 &&
         ...
         else if ((S->Flags & SF_DEFINED) != 0 &&
                  (S->Flags & SF_REFERENCED) == 0) {
            LIWarning(&S->DefLines, 2,
                      "Symbol '%m%p' is defined but never used", GetSymName(S));
         }
         ...
         if (S->AddrSize != ADDR_SIZE_DEFAULT) {
            ...
            if (S->AddrSize <=
                sizeof(S->GuessedUse) / sizeof(S->GuessedUse[0])) {
               /* Get the file position where the symbol was used */
               const FilePos *P = S->GuessedUse[S->AddrSize - 1];
               if (P) {
                  PWarning(P, 0, "Didn't use %s addressing for '%m%p'",
                           AddrSizeToStr(S->AddrSize), GetSymName(S));
               }
            }
         }
      }
