...
static void ED_MergeAddrSizeAND(ExprNode *Expr, ExprDesc *ED,
                                const ExprDesc *Right)
/* Merge the address sizes of two expressions into ED, special case for AND
   operator */
{
   int ConstL, ConstR;
   int Size, ConstSize;
   ...
   if (!ConstL && !ConstR) {
      /* If neither part of the expression is constant, the above is all we can
       * do */
      return;
   }
...
/* Study an EXPR_SWAP expression node */
{
   ...
   /* We can handle only const expressions */
   if (ED_IsConst(D)) {
      D->Val = (D->Val & ~0xFFFFUL) | ((D->Val >> 8) & 0xFF) |
               ((D->Val << 8) & 0xFF00);
   }
   else {
      ED_Invalidate(D);
