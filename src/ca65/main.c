...
/* Assemble one line */
{
   ...
   // namespace)
   if (CurTok.Tok == TOK_LOCAL_IDENT || CurTok.Tok == TOK_NAMESPACE ||
   ...
   if (Sym) {
      ...
      // Suppress .size Symbol if this Symbol already has a multiply-defined
      // error, as it will only create its own additional unnecessary error.
