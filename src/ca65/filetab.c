...
static HashTable HashTab =
    STATIC_HASHTABLE_INITIALIZER(HASHTAB_COUNT, &HashFunc);
...
static FileEntry *NewFileEntry(unsigned Name, FileType Type, unsigned long Size,
                               unsigned long MTime)
...
{
   static const StrBuf ErrorMsg =
       LIT_STRBUF_INITIALIZER("(outside file scope)");
...
/* Create dependency files requested by the user */
{
   if (SB_NotEmpty(&DepName)) {
      CreateDepFile(SB_GetConstBuf(&DepName), FT_MAIN | FT_INCLUDE | FT_BINARY);
   }
