...
static void VPrintMsg(const FilePos *Pos, const char *Desc, const char *Format,
                      va_list ap)
/* Format and output an error/warning message. */
{
   ...
   SB_Printf(&S, "%s:%u: %s: ", SB_GetConstBuf(GetFileName(Pos->Name)),
             Pos->Line, Desc);
...
static void PrintMsg(const FilePos *Pos, const char *Desc, const char *Format,
                     ...)
/* Format and output an error/warning message. */
{
   va_list ap;
...
/* Output additional notifications for an error or warning */
{
   ...
   if (Skipped > 0) {
      ...
      PrintMsg(GetSourcePos(LI), "Note", "Dropping %u additional line infos",
               Skipped);
   }
}
...
static void WarningMsg(const Collection *LineInfos, const char *Format,
                       va_list ap)
