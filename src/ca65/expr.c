...
// Handle the .ISMNEMONIC, .ISMNEM builtin function
{
   ...
   if (CurTok.Tok == TOK_IDENT) {
      if (UbiquitousIdents) {
         ...
         /* Macros and symbols may NOT use the names of instructions, so just
...
// Force the given expression into a specific size of ForceRange is true
{
   if (ForceRange) {
      switch (Size) {
         case 1:
            Expr = GenByteExpr(Expr);
            break;
         case 2:
            Expr = GenWordExpr(Expr);
            break;
         case 3:
            Expr = GenFarAddrExpr(Expr);
            break;
         case 4:
            Expr = GenDWordExpr(Expr);
            break;
         default:
            Internal("Invalid size in BoundedExpr: %u", Size);
      }
   }
   return Expr;
}
