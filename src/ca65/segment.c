...
SegDef ZeropageSegDef =
    STATIC_SEGDEF_INITIALIZER(SEGNAME_ZEROPAGE, ADDR_SIZE_ZP);
...
// the given value.
{
   ...
   if (CombinedAlignment > MAX_ALIGNMENT) {
      ...
      if (CombinedAlignment >= LARGE_ALIGNMENT &&
          CombinedAlignment > ActiveSeg->Align &&
          CombinedAlignment > Alignment && !LargeAlignment) {
            ...
            if (ED_IsConst(&ED)) {
               ...
               if (F->Type == FRAG_SEXPR) {
                  ...
                  if (ED.Val > Hi || ED.Val < Lo) {
                     LIError(&F->LI, "Range error (%ld not in [%ld..%ld])",
                             ED.Val, Lo, Hi);
                  }
               }
               else {
                  if (((unsigned long)ED.Val) > U_Hi[F->Len - 1]) {
                     LIError(&F->LI, "Range error (%lu not in [0..%lu])",
               ...
               if ((F->Len == 1 && ED.AddrSize > ADDR_SIZE_ZP) ||
                  ...
                  LIError(&F->LI,
                          "Range error (Address size %u does not match "
                          "fragment size %u)",
                          ED.AddrSize, F->Len);
               }
            }
   ...
   while (Frag) {
      ...
      switch (Frag->Type) {
         ...
         case FRAG_EXPR:
            switch (Frag->Len) {
               case 1:
                  ObjWrite8(FRAG_EXPR8);
                  break;
               case 2:
                  ObjWrite8(FRAG_EXPR16);
                  break;
               case 3:
                  ObjWrite8(FRAG_EXPR24);
                  break;
               case 4:
                  ObjWrite8(FRAG_EXPR32);
                  break;
               default:
                  Internal("Invalid fragment size: %u", Frag->Len);
            }
         ...
         case FRAG_SEXPR:
            switch (Frag->Len) {
               case 1:
                  ObjWrite8(FRAG_SEXPR8);
                  break;
               case 2:
                  ObjWrite8(FRAG_SEXPR16);
                  break;
               case 3:
                  ObjWrite8(FRAG_SEXPR24);
                  break;
               case 4:
                  ObjWrite8(FRAG_SEXPR32);
                  break;
               default:
                  Internal("Invalid fragment size: %u", Frag->Len);
            }
