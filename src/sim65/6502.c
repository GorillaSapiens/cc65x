...
// 0x  BRK  ORA  ---  SLO  NOP  ORA  ASL  SLO  PHP  ORA  ASL  ANC  NOP  ORA  ASL
// SLO inx       inx  zp   zp   zp   zp        imm  acc  imm  abs  abs  abs  abs
//
// 1x  BPL  ORA  ---  SLO  NOP  ORA  ASL  SLO  CLC  ORA  NOP  SLO  NOP  ORA  ASL
// SLO rel  iny       iny  zpx  zpx  zpx  zpy       aby       aby  abx  abx  abx
// abx
//
// 2x  JSR  AND  ---  RLA  BIT  AND  ROL  RLA  PLP  AND  ROL  ANC  BIT  AND  ROL
// RLA abs  inx       inx  zp   zp   zp   zp        imm  acc  imm  abs  abs  abs
// abs
//
// 3x  BMI  AND  ---  RLA  NOP  AND  ROL  RLA  SEC  AND  NOP  RLA  NOP  AND  ROL
// RLA rel  iny       iny  zpx  zpx  zpx  zpy       aby       aby  abx  abx  abx
// abx
//
// 4x  RTI  EOR  ---  SRE  NOP  EOR  LSR  SRE  PHA  EOR  LSR  ASR  JMP  EOR  LSR
// SRE inx       inx  zp   zp   zp   zp        imm  acc  imm  abs  abs  abs  abs
//
// 5x  BVC  EOR  ---  SRE  NOP  EOR  LSR  SRE  CLI  EOR  NOP  SRE  NOP  EOR  LSR
// SRE rel  iny       iny  zpx  zpx  zpx  zpx       aby       aby  abx  abx  abx
// abx
//
// 6x  RTS  ADC  ---  RRA  NOP  ADC  ROR  RRA  PLA  ADC  ROR  ARR  JMP  ADC  ROR
// RRA inx       inx  zp   zp   zp   zp        imm  acc  imm  ind  abs  abs  abs
//
// 7x  BVS  ADC  ---  RRA  NOP  ADC  ROR  RRA  SEI  ADC  NOP  RRA  NOP  ADC  ROR
// RRA rel  iny       iny  zpx  zpx  zpx  zpx       aby       aby  abx  abx  abx
// abx
//
// 8x  NOP  STA  NOP  SAX  STY  STA  STX  SAX  DEY  NOP  TXA  ANE  STY  STA  STX
// SAX imm  inx  imm  inx  zp   zp   zp   zp        imm       imm  abs  abs  abs
// abs
//
// 9x  BCC  STA  ---  SHA  STY  STA  STX  SAX  TYA  STA  TXS  TAS  SHY  STA  SHX
// SHA rel  iny       iny  zpx  zpx  zpy  zpy       aby       aby  abx  abx  aby
// aby
//
// Ax  LDY  LDA  LDX  LAX  LDY  LDA  LDX  LAX  TAY  LDA  TAX  LXA  LDY  LDA  LDX
// LAX imm  inx  imm  inx  zp   zp   zp   zp        imm       imm  abs  abs  abs
// abs
//
// Bx  BCS  LDA  ---  LAX  LDY  LDA  LDX  LAX  CLV  LDA  TSX  LAS  LDY  LDA  LDX
// LAX rel  iny       iny  zpx  zpx  zpy  zpy       aby       aby  abx  abx  aby
// aby
//
// Cx  CPY  CMP  NOP  DCP  CPY  CMP  DEC  DCP  INY  CMP  DEX  SBX  CPY  CMP  DEC
// DCP imm  inx  imm  inx  zp   zp   zp   zp        imm       imm  abs  abs  abs
// abs
//
// Dx  BNE  CMP  ---  DCP  NOP  CMP  DEC  DCP  CLD  CMP  NOP  DCP  NOP  CMP  DEC
// DCP rel  iny       iny  zpx  zpx  zpx  zpx       aby  zpx  aby  abx  abx  abx
// abx
//
// Ex  CPX  SBC  NOP  ISC  CPX  SBC  INC  ISC  INX  SBC  NOP  SBC  CPX  SBC  INC
// ISC imm  inx  imm  inx  zp   zp   zp   zp        imm       imm  abs  abs  abs
// abs
//
// Fx  BEQ  SBC  ---  ISC  NOP  SBC  INC  ISC  SED  SBC  NOP  ISC  NOP  SBC  INC
// ISC rel  iny       iny  zpx  zpx  zpx  zpx       aby  zpx  aby  abx  abx  abx
// abx
//
// --- = CPU JAM/HALT
//
...
// -------------+--------+--------+
//
// RLA: rotate left the contents of a memory location and then AND the result
...
// -------------+--------+--------+
//
// LAX: loads both the accumulator and the X register with the content of a
// memory location.
//
// Address mode | opcode | cycles |            N V B D I Z C
// -------------+--------+--------+    FLAGS:  X . . . . X .
...
// -------------+--------+--------+
//
// DCP: decrements the contents of a memory location and then compares the
...
// -------------+--------+--------+
//
// ISC: increments the contents of a memory location and then subtract with
// carry the result from the accumulator.
//
// Address mode | opcode | cycles |            N V B D I Z C
// -------------+--------+--------+    FLAGS:  X X . . . X X
// ISC abs      |   EFh  |  6     |
...
// -------------+--------+--------+
//
// ASR: calculates the AND between the accumulator and an immediate value and
// then shift right the result.
//
// Address mode | opcode | cycles |            N V B D I Z C
// -------------+--------+--------+    FLAGS:  X . . . . X X
// ASR #imm     |   4Bh  |  2     |
// -------------+--------+--------+
//
// ARR: calculates the AND between the accumulator and an immediate value and
// then rotate right the result.
//
// Address mode | opcode | cycles |            N V B D I Z C
// -------------+--------+--------+    FLAGS:  X . . . . X X
// ARR #imm     |   6Bh  |  2     |
// -------------+--------+--------+
//
// ANE: calculates the OR of the accumulator with an unstable constant, then it
// does an AND with the X register and an immediate value. The unstable constant
// varies with temperature, the production batch and maybe other factors.
// Experimental measures assume its value to 0xEF.
//
// Address mode | opcode | cycles |            N V B D I Z C
// -------------+--------+--------+    FLAGS:  X . . . . X .
// ANE #imm     |   8Bh  |  2     |
// -------------+--------+--------+
//
// LXA: calculates the OR of the accumulator with an unstable constant, then it
// does an AND with an immediate value. The result is copied into the X register
// and the accumulator. The unstable constant varies with temperature, the
// production batch and maybe other factors. Experimental measures assume its
// value to 0xEE.
//
// Address mode | opcode | cycles |            N V B D I Z C
// -------------+--------+--------+    FLAGS:  X . . . . X .
...
// -------------+--------+--------+
//
// SBX: calculates the AND of the accumulator with the X register and the
...
// -------------+--------+--------+
//
// TAS: calculates the AND of the accumulator with the X register and stores the
// result into the stack pointer. Then, it calculates the AND of the result with
// the high byte of the memory pointer plus 1 and it stores the final result in
// memory.
//
// Address mode | opcode | cycles |            N V B D I Z C
// -------------+--------+--------+    FLAGS:  . . . . . . .
...
// -------------+--------+--------+
//
// SHY: calculates the AND of the Y register with the high byte of the memory
// pointer plus 1 and it stores the final result in memory.
//
// Address mode | opcode | cycles |            N V B D I Z C
// -------------+--------+--------+    FLAGS:  . . . . . . .
// SHY abs,x    |   9Ch  |  5     |
// -------------+--------+--------+
//
// SHX: calculates the AND of the X register with the high byte of the memory
// pointer plus 1 and it stores the final result in memory.
//
// Address mode | opcode | cycles |            N V B D I Z C
// -------------+--------+--------+    FLAGS:  . . . . . . .
...
// -------------+--------+--------+
//
// SHA: calculates the AND of the accumulator with the X register with the high
// byte of the memory pointer plus 1 and it stores the final result in memory.
//
// Address mode | opcode | cycles |            N V B D I Z C
// -------------+--------+--------+    FLAGS:  . . . . . . .
// SHX abs,y    |   9Fh  |  5     |
...
// Set the flags. The parameter is a boolean flag that says if the flag should
// be set or reset.
#define SET_CF(f)                                                              \
   do {                                                                        \
      if (f) {                                                                 \
...
#define ADC_DECIMAL_MODE_65C02(v)                                              \
   do {                                                                        \
      ...
      const bool NewOF =                                                       \
          ((OldAC >= 0x80) ^ PrematureSF) & ((op >= 0x80) ^ PrematureSF);      \
      SET_SF(NewSF);                                                           \
      SET_OF(NewOF);                                                           \
      SET_ZF(NewZF);                                                           \
      SET_CF(carry);                                                           \
...
#define CPX(operand) COMPARE(Regs.XR, operand)

#define CPY(operand) COMPARE(Regs.YR, operand)

#define CMP(operand) COMPARE(Regs.AC, operand)
...
#define BITIMM(Val) SET_ZF((Val & Regs.AC) == 0)
...
// SHA
#define SHA(Val) Val = Regs.AC & Regs.XR & ((address >> 8) + 1)
...
static void OPC_Illegal(void) {
   Error("Illegal opcode $%02X at address $%04X", MemReadByte(Regs.PC),
         Regs.PC);
}
...
// Opcode $20: JSR
{
   // The obvious way to implement JSR for the 6502 is to (a) read the target
   // address, and then (b) push the return address minus one. Or do (b) first,
   // then (a).
   //
   // However, there is a non-obvious case where this conflicts with the actual
   // order of operations that the 6502 does, which is:
   //
   ...
   //
   // This can make a difference in a pretty esoteric case, if the JSR target is
   // located, wholly or in part, inside the stack page (!). This won't happen
   // in normal code but it can happen in specifically constructed examples.
   //
   // To deal with this, we load the LSB and MSB of the target address
   // separately, with the pushing of the return address sandwiched in between,
   // to mimic the order of the bus operations on a real 6502.
...
// Opcode $6C: JMP (ind)
{
   ...
   if (Hi != Lo + 1) {
      Warning("6502 indirect jump bug triggered at $%04X, ind addr = $%04X", PC,
              Lo);
   }
...
static const OPFunc OP6502Table[256] = {
    OPC_6502_00, OPC_6502_01, OPC_Illegal, OPC_Illegal, OPC_Illegal,
    OPC_6502_05, OPC_6502_06, OPC_Illegal, OPC_6502_08, OPC_6502_09,
    OPC_6502_0A, OPC_Illegal, OPC_Illegal, OPC_6502_0D, OPC_6502_0E,
    OPC_Illegal, OPC_6502_10, OPC_6502_11, OPC_Illegal, OPC_Illegal,
    OPC_Illegal, OPC_6502_15, OPC_6502_16, OPC_Illegal, OPC_6502_18,
    OPC_6502_19, OPC_Illegal, OPC_Illegal, OPC_Illegal, OPC_6502_1D,
    OPC_6502_1E, OPC_Illegal, OPC_6502_20, OPC_6502_21, OPC_Illegal,
    OPC_Illegal, OPC_6502_24, OPC_6502_25, OPC_6502_26, OPC_Illegal,
    OPC_6502_28, OPC_6502_29, OPC_6502_2A, OPC_Illegal, OPC_6502_2C,
    OPC_6502_2D, OPC_6502_2E, OPC_Illegal, OPC_6502_30, OPC_6502_31,
    OPC_Illegal, OPC_Illegal, OPC_Illegal, OPC_6502_35, OPC_6502_36,
    OPC_Illegal, OPC_6502_38, OPC_6502_39, OPC_Illegal, OPC_Illegal,
    OPC_Illegal, OPC_6502_3D, OPC_6502_3E, OPC_Illegal, OPC_6502_40,
    OPC_6502_41, OPC_Illegal, OPC_Illegal, OPC_Illegal, OPC_6502_45,
    OPC_6502_46, OPC_Illegal, OPC_6502_48, OPC_6502_49, OPC_6502_4A,
    OPC_Illegal, OPC_6502_4C, OPC_6502_4D, OPC_6502_4E, OPC_Illegal,
    OPC_6502_50, OPC_6502_51, OPC_Illegal, OPC_Illegal, OPC_Illegal,
    OPC_6502_55, OPC_6502_56, OPC_Illegal, OPC_6502_58, OPC_6502_59,
    OPC_Illegal, OPC_Illegal, OPC_Illegal, OPC_6502_5D, OPC_6502_5E,
    OPC_Illegal, OPC_6502_60, OPC_6502_61, OPC_Illegal, OPC_Illegal,
    OPC_Illegal, OPC_6502_65, OPC_6502_66, OPC_Illegal, OPC_6502_68,
    OPC_6502_69, OPC_6502_6A, OPC_Illegal, OPC_6502_6C, OPC_6502_6D,
    OPC_6502_6E, OPC_Illegal, OPC_6502_70, OPC_6502_71, OPC_Illegal,
    OPC_Illegal, OPC_Illegal, OPC_6502_75, OPC_6502_76, OPC_Illegal,
    OPC_6502_78, OPC_6502_79, OPC_Illegal, OPC_Illegal, OPC_Illegal,
    OPC_6502_7D, OPC_6502_7E, OPC_Illegal, OPC_Illegal, OPC_6502_81,
    OPC_Illegal, OPC_Illegal, OPC_6502_84, OPC_6502_85, OPC_6502_86,
    OPC_Illegal, OPC_6502_88, OPC_Illegal, OPC_6502_8A, OPC_Illegal,
    OPC_6502_8C, OPC_6502_8D, OPC_6502_8E, OPC_Illegal, OPC_6502_90,
    OPC_6502_91, OPC_Illegal, OPC_Illegal, OPC_6502_94, OPC_6502_95,
    OPC_6502_96, OPC_Illegal, OPC_6502_98, OPC_6502_99, OPC_6502_9A,
    OPC_Illegal, OPC_Illegal, OPC_6502_9D, OPC_Illegal, OPC_Illegal,
    OPC_6502_A0, OPC_6502_A1, OPC_6502_A2, OPC_Illegal, OPC_6502_A4,
    OPC_6502_A5, OPC_6502_A6, OPC_Illegal, OPC_6502_A8, OPC_6502_A9,
    OPC_6502_AA, OPC_Illegal, OPC_6502_AC, OPC_6502_AD, OPC_6502_AE,
    OPC_Illegal, OPC_6502_B0, OPC_6502_B1, OPC_Illegal, OPC_Illegal,
    OPC_6502_B4, OPC_6502_B5, OPC_6502_B6, OPC_Illegal, OPC_6502_B8,
    OPC_6502_B9, OPC_6502_BA, OPC_Illegal, OPC_6502_BC, OPC_6502_BD,
    OPC_6502_BE, OPC_Illegal, OPC_6502_C0, OPC_6502_C1, OPC_Illegal,
    OPC_Illegal, OPC_6502_C4, OPC_6502_C5, OPC_6502_C6, OPC_Illegal,
    OPC_6502_C8, OPC_6502_C9, OPC_6502_CA, OPC_Illegal, OPC_6502_CC,
    OPC_6502_CD, OPC_6502_CE, OPC_Illegal, OPC_6502_D0, OPC_6502_D1,
    OPC_Illegal, OPC_Illegal, OPC_Illegal, OPC_6502_D5, OPC_6502_D6,
    OPC_Illegal, OPC_6502_D8, OPC_6502_D9, OPC_Illegal, OPC_Illegal,
    OPC_Illegal, OPC_6502_DD, OPC_6502_DE, OPC_Illegal, OPC_6502_E0,
    OPC_6502_E1, OPC_Illegal, OPC_Illegal, OPC_6502_E4, OPC_6502_E5,
    OPC_6502_E6, OPC_Illegal, OPC_6502_E8, OPC_6502_E9, OPC_6502_EA,
    OPC_Illegal, OPC_6502_EC, OPC_6502_ED, OPC_6502_EE, OPC_Illegal,
    OPC_6502_F0, OPC_6502_F1, OPC_Illegal, OPC_Illegal, OPC_Illegal,
    OPC_6502_F5, OPC_6502_F6, OPC_Illegal, OPC_6502_F8, OPC_6502_F9,
    OPC_Illegal, OPC_Illegal, OPC_Illegal, OPC_6502_FD, OPC_6502_FE,
    OPC_Illegal,
};

// Opcode handler table for the 6502X
static const OPFunc OP6502XTable[256] = {
    OPC_6502_00,  OPC_6502_01,  OPC_Illegal,  OPC_6502X_03, OPC_6502X_04,
    OPC_6502_05,  OPC_6502_06,  OPC_6502X_07, OPC_6502_08,  OPC_6502_09,
    OPC_6502_0A,  OPC_6502X_0B, OPC_6502X_0C, OPC_6502_0D,  OPC_6502_0E,
    OPC_6502X_0F, OPC_6502_10,  OPC_6502_11,  OPC_Illegal,  OPC_6502X_13,
    OPC_6502X_14, OPC_6502_15,  OPC_6502_16,  OPC_6502X_17, OPC_6502_18,
    OPC_6502_19,  OPC_6502X_1A, OPC_6502X_1B, OPC_6502X_1C, OPC_6502_1D,
    OPC_6502_1E,  OPC_6502X_1F, OPC_6502_20,  OPC_6502_21,  OPC_Illegal,
    OPC_6502X_23, OPC_6502_24,  OPC_6502_25,  OPC_6502_26,  OPC_6502X_27,
    OPC_6502_28,  OPC_6502_29,  OPC_6502_2A,  OPC_6502X_2B, OPC_6502_2C,
    OPC_6502_2D,  OPC_6502_2E,  OPC_6502X_2F, OPC_6502_30,  OPC_6502_31,
    OPC_Illegal,  OPC_6502X_33, OPC_6502X_34, OPC_6502_35,  OPC_6502_36,
    OPC_6502X_37, OPC_6502_38,  OPC_6502_39,  OPC_6502X_3A, OPC_6502X_3B,
    OPC_6502X_3C, OPC_6502_3D,  OPC_6502_3E,  OPC_6502X_3F, OPC_6502_40,
    OPC_6502_41,  OPC_Illegal,  OPC_6502X_43, OPC_6502X_44, OPC_6502_45,
    OPC_6502_46,  OPC_6502X_47, OPC_6502_48,  OPC_6502_49,  OPC_6502_4A,
    OPC_6502X_4B, OPC_6502_4C,  OPC_6502_4D,  OPC_6502_4E,  OPC_6502X_4F,
    OPC_6502_50,  OPC_6502_51,  OPC_Illegal,  OPC_6502X_53, OPC_6502X_54,
    OPC_6502_55,  OPC_6502_56,  OPC_6502X_57, OPC_6502_58,  OPC_6502_59,
    OPC_6502X_5A, OPC_6502X_5B, OPC_6502X_5C, OPC_6502_5D,  OPC_6502_5E,
    OPC_6502X_5F, OPC_6502_60,  OPC_6502_61,  OPC_Illegal,  OPC_6502X_63,
    OPC_6502X_64, OPC_6502_65,  OPC_6502_66,  OPC_6502X_67, OPC_6502_68,
    OPC_6502_69,  OPC_6502_6A,  OPC_6502X_6B, OPC_6502_6C,  OPC_6502_6D,
    OPC_6502_6E,  OPC_6502X_6F, OPC_6502_70,  OPC_6502_71,  OPC_Illegal,
    OPC_6502X_73, OPC_6502X_74, OPC_6502_75,  OPC_6502_76,  OPC_6502X_77,
    OPC_6502_78,  OPC_6502_79,  OPC_6502X_7A, OPC_6502X_7B, OPC_6502X_7C,
    OPC_6502_7D,  OPC_6502_7E,  OPC_6502X_7F, OPC_6502X_80, OPC_6502_81,
    OPC_6502X_82, OPC_6502X_83, OPC_6502_84,  OPC_6502_85,  OPC_6502_86,
    OPC_6502X_87, OPC_6502_88,  OPC_6502X_89, OPC_6502_8A,  OPC_6502X_8B,
    OPC_6502_8C,  OPC_6502_8D,  OPC_6502_8E,  OPC_6502X_8F, OPC_6502_90,
    OPC_6502_91,  OPC_Illegal,  OPC_6502X_93, OPC_6502_94,  OPC_6502_95,
    OPC_6502_96,  OPC_6502X_97, OPC_6502_98,  OPC_6502_99,  OPC_6502_9A,
    OPC_6502X_9B, OPC_6502X_9C, OPC_6502_9D,  OPC_6502X_9E, OPC_6502X_9F,
    OPC_6502_A0,  OPC_6502_A1,  OPC_6502_A2,  OPC_6502X_A3, OPC_6502_A4,
    OPC_6502_A5,  OPC_6502_A6,  OPC_6502X_A7, OPC_6502_A8,  OPC_6502_A9,
    OPC_6502_AA,  OPC_6502X_AB, OPC_6502_AC,  OPC_6502_AD,  OPC_6502_AE,
    OPC_6502X_AF, OPC_6502_B0,  OPC_6502_B1,  OPC_Illegal,  OPC_6502X_B3,
    OPC_6502_B4,  OPC_6502_B5,  OPC_6502_B6,  OPC_6502X_B7, OPC_6502_B8,
    OPC_6502_B9,  OPC_6502_BA,  OPC_6502X_BB, OPC_6502_BC,  OPC_6502_BD,
    OPC_6502_BE,  OPC_6502X_BF, OPC_6502_C0,  OPC_6502_C1,  OPC_6502X_C2,
    OPC_6502X_C3, OPC_6502_C4,  OPC_6502_C5,  OPC_6502_C6,  OPC_6502X_C7,
    OPC_6502_C8,  OPC_6502_C9,  OPC_6502_CA,  OPC_6502X_CB, OPC_6502_CC,
    OPC_6502_CD,  OPC_6502_CE,  OPC_6502X_CF, OPC_6502_D0,  OPC_6502_D1,
    OPC_Illegal,  OPC_6502X_D3, OPC_6502X_D4, OPC_6502_D5,  OPC_6502_D6,
    OPC_6502X_D7, OPC_6502_D8,  OPC_6502_D9,  OPC_6502X_DA, OPC_6502X_DB,
    OPC_6502X_DC, OPC_6502_DD,  OPC_6502_DE,  OPC_6502X_DF, OPC_6502_E0,
    OPC_6502_E1,  OPC_6502X_E2, OPC_6502X_E3, OPC_6502_E4,  OPC_6502_E5,
    OPC_6502_E6,  OPC_6502X_E7, OPC_6502_E8,  OPC_6502_E9,  OPC_6502_EA,
    OPC_6502X_EB, OPC_6502_EC,  OPC_6502_ED,  OPC_6502_EE,  OPC_6502X_EF,
    OPC_6502_F0,  OPC_6502_F1,  OPC_Illegal,  OPC_6502X_F3, OPC_6502X_F4,
    OPC_6502_F5,  OPC_6502_F6,  OPC_6502X_F7, OPC_6502_F8,  OPC_6502_F9,
    OPC_6502X_FA, OPC_6502X_FB, OPC_6502X_FC, OPC_6502_FD,  OPC_6502_FE,
    OPC_6502X_FF};

// Opcode handler table for the 65C02
static const OPFunc OP65C02Table[256] = {
    OPC_6502_00,     OPC_6502_01,
    OPC_65C02_NOP22, // $02
    OPC_65C02_NOP11, // $03
    OPC_65C02_04,    OPC_6502_05,  OPC_6502_06,  OPC_65C02_07, OPC_6502_08,
    OPC_6502_09,     OPC_6502_0A,
    OPC_65C02_NOP11, // $0B
    OPC_65C02_0C,    OPC_6502_0D,  OPC_6502_0E,  OPC_65C02_0F, OPC_6502_10,
    OPC_6502_11,     OPC_65C02_12,
    OPC_65C02_NOP11, // $13
    OPC_65C02_14,    OPC_6502_15,  OPC_6502_16,  OPC_65C02_17, OPC_6502_18,
    OPC_6502_19,     OPC_65C02_1A,
    OPC_65C02_NOP11, // $1B
    OPC_65C02_1C,    OPC_6502_1D,  OPC_65C02_1E, OPC_65C02_1F, OPC_6502_20,
    OPC_6502_21,
    OPC_65C02_NOP22, // $22
    OPC_65C02_NOP11, // $23
    OPC_6502_24,     OPC_6502_25,  OPC_6502_26,  OPC_65C02_27, OPC_6502_28,
    OPC_6502_29,     OPC_6502_2A,
    OPC_65C02_NOP11, // $2B
    OPC_6502_2C,     OPC_6502_2D,  OPC_6502_2E,  OPC_65C02_2F, OPC_6502_30,
    OPC_6502_31,     OPC_65C02_32,
    OPC_65C02_NOP11, // $33
    OPC_65C02_34,    OPC_6502_35,  OPC_6502_36,  OPC_65C02_37, OPC_6502_38,
    OPC_6502_39,     OPC_65C02_3A,
    OPC_65C02_NOP11, // $3B
    OPC_65C02_3C,    OPC_6502_3D,  OPC_65C02_3E, OPC_65C02_3F, OPC_6502_40,
    OPC_6502_41,
    OPC_65C02_NOP22, // $42
    OPC_65C02_NOP11, // $43
    OPC_6502X_44,    // $44
    OPC_6502_45,     OPC_6502_46,  OPC_65C02_47, OPC_6502_48,  OPC_6502_49,
    OPC_6502_4A,
    OPC_65C02_NOP11, // $4B
    OPC_6502_4C,     OPC_6502_4D,  OPC_6502_4E,  OPC_65C02_4F, OPC_6502_50,
    OPC_6502_51,     OPC_65C02_52,
    OPC_65C02_NOP11, // $53
    OPC_65C02_NOP24, // $54
    OPC_6502_55,     OPC_6502_56,  OPC_65C02_57, OPC_6502_58,  OPC_6502_59,
    OPC_65C02_5A,
    OPC_65C02_NOP11, // $5B
    OPC_65C02_5C,    OPC_6502_5D,  OPC_65C02_5E, OPC_65C02_5F, OPC_6502_60,
    OPC_65C02_61,
    OPC_65C02_NOP22, // $62
    OPC_65C02_NOP11, // $63
    OPC_65C02_64,    OPC_65C02_65, OPC_6502_66,  OPC_65C02_67, OPC_6502_68,
    OPC_65C02_69,    OPC_6502_6A,
    OPC_65C02_NOP11, // $6B
    OPC_65C02_6C,    OPC_65C02_6D, OPC_6502_6E,  OPC_65C02_6F, OPC_6502_70,
    OPC_65C02_71,    OPC_65C02_72,
    OPC_65C02_NOP11, // $73
    OPC_65C02_74,    OPC_65C02_75, OPC_6502_76,  OPC_65C02_77, OPC_6502_78,
    OPC_65C02_79,    OPC_65C02_7A,
    OPC_65C02_NOP11, // $7B
    OPC_65C02_7C,    OPC_65C02_7D, OPC_65C02_7E, OPC_65C02_7F, OPC_65C02_80,
    OPC_6502_81,
    OPC_65C02_NOP22, // $82
    OPC_65C02_NOP11, // $83
    OPC_6502_84,     OPC_6502_85,  OPC_6502_86,  OPC_65C02_87, OPC_6502_88,
    OPC_65C02_89,    OPC_6502_8A,
    OPC_65C02_NOP11, // $8B
    OPC_6502_8C,     OPC_6502_8D,  OPC_6502_8E,  OPC_65C02_8F, OPC_6502_90,
    OPC_6502_91,     OPC_65C02_92,
    OPC_65C02_NOP11, // $93
    OPC_6502_94,     OPC_6502_95,  OPC_6502_96,  OPC_65C02_97, OPC_6502_98,
    OPC_6502_99,     OPC_6502_9A,
    OPC_65C02_NOP11, // $9B
    OPC_65C02_9C,    OPC_6502_9D,  OPC_65C02_9E, OPC_65C02_9F, OPC_6502_A0,
    OPC_6502_A1,     OPC_6502_A2,
    OPC_65C02_NOP11, // $A3
    OPC_6502_A4,     OPC_6502_A5,  OPC_6502_A6,  OPC_65C02_A7, OPC_6502_A8,
    OPC_6502_A9,     OPC_6502_AA,
    OPC_65C02_NOP11, // $AB
    OPC_6502_AC,     OPC_6502_AD,  OPC_6502_AE,  OPC_65C02_AF, OPC_6502_B0,
    OPC_6502_B1,     OPC_65C02_B2,
    OPC_65C02_NOP11, // $B3
    OPC_6502_B4,     OPC_6502_B5,  OPC_6502_B6,  OPC_65C02_B7, OPC_6502_B8,
    OPC_6502_B9,     OPC_6502_BA,
    OPC_65C02_NOP11, // $BB
    OPC_6502_BC,     OPC_6502_BD,  OPC_6502_BE,  OPC_65C02_BF, OPC_6502_C0,
    OPC_6502_C1,
    OPC_65C02_NOP22, // $C2
    OPC_65C02_NOP11, // $C3
    OPC_6502_C4,     OPC_6502_C5,  OPC_6502_C6,  OPC_65C02_C7, OPC_6502_C8,
    OPC_6502_C9,     OPC_6502_CA,
    OPC_Illegal, // $CB: WAI currently unsupported
    OPC_6502_CC,     OPC_6502_CD,  OPC_6502_CE,  OPC_65C02_CF, OPC_6502_D0,
    OPC_6502_D1,     OPC_65C02_D2,
    OPC_65C02_NOP11, // $D3
    OPC_65C02_NOP24, // $D4
    OPC_6502_D5,     OPC_6502_D6,  OPC_65C02_D7, OPC_6502_D8,  OPC_6502_D9,
    OPC_65C02_DA,
    OPC_Illegal,     // $DB: STP currently unsupported
    OPC_65C02_NOP34, // $DC
    OPC_6502_DD,     OPC_6502_DE,  OPC_65C02_DF, OPC_6502_E0,  OPC_65C02_E1,
    OPC_65C02_NOP22, // $E2
    OPC_65C02_NOP11, // $E3
    OPC_6502_E4,     OPC_65C02_E5, OPC_6502_E6,  OPC_65C02_E7, OPC_6502_E8,
    OPC_65C02_E9,    OPC_6502_EA,
    OPC_65C02_NOP11, // $EB
    OPC_6502_EC,     OPC_65C02_ED, OPC_6502_EE,  OPC_65C02_EF, OPC_6502_F0,
    OPC_65C02_F1,    OPC_65C02_F2,
    OPC_65C02_NOP11, // $F3
    OPC_65C02_NOP24, // $F4
    OPC_65C02_F5,    OPC_6502_F6,  OPC_65C02_F7, OPC_6502_F8,  OPC_65C02_F9,
    OPC_65C02_FA,
    OPC_65C02_NOP11, // $FB
    OPC_65C02_NOP34, // $FC
    OPC_65C02_FD,    OPC_6502_FE,  OPC_65C02_FF};

// Tables with opcode handlers
static const OPFunc *Handlers[3] = {OP6502Table, OP65C02Table, OP6502XTable};
...
// Execute one CPU instruction
{
   ...
   // Increment the 64-bit clock cycle counter with the cycle count for the
   // instruction that we just executed.
