...
// Get the wallclock time with nanosecond resolution.
{
   ...
   // When using the MinGW64 compiler, neither timespec_get() nor
   // clock_gettime() are available; using either of them makes the Linux PR
   // build workflow build fail. The gettimeofday() function does work, so use
   // that; its microsecond resolution is fine for most applications.
   struct timeval tv;
   ...
   // Note: we test for MinGW32 after the test for MinGW64, as the __MINGW32__
   // symbol is also defined in MinGW64. This allows us to distinguish MinGW32
   // and MinGW64 build.
   //
   // When using the MinGW32 compiler, neither timespec_get() nor
   // clock_gettime() are available; using either of them makes the Linux
   // snapshot workflow build fail. The gettimeofday() function does work, so
   // use that; its microsecond resolution is fine for most applications.
   struct timeval tv;
   time_valid = (gettimeofday(&tv, NULL) == 0);
   if (time_valid) {
      ts->tv_sec = tv.tv_sec;
   ...
   switch (Addr) {
      ...
      case PERIPHERALS_COUNTER_ADDRESS_OFFSET_LATCH: {
         ...
         if (time_valid) {
            ...
            // 1-1-1970, low word is number of nanoseconds since the start of
            // that second.
            Peripherals.Counter.LatchedWallclockTimeSplit =
      ...
      case PERIPHERALS_COUNTER_ADDRESS_OFFSET_VALUE + 7: {
         ...
         switch (Peripherals.Counter.LatchedValueSelected) {
            case PERIPHERALS_COUNTER_SELECT_CLOCKCYCLE_COUNTER:
               Value = Peripherals.Counter.LatchedClockCycles;
               break;
            case PERIPHERALS_COUNTER_SELECT_INSTRUCTION_COUNTER:
               Value = Peripherals.Counter.LatchedCpuInstructions;
               break;
            case PERIPHERALS_COUNTER_SELECT_IRQ_COUNTER:
               Value = Peripherals.Counter.LatchedIrqEvents;
               break;
            case PERIPHERALS_COUNTER_SELECT_NMI_COUNTER:
               Value = Peripherals.Counter.LatchedNmiEvents;
               break;
            case PERIPHERALS_COUNTER_SELECT_WALLCLOCK_TIME:
               Value = Peripherals.Counter.LatchedWallclockTime;
               break;
            case PERIPHERALS_COUNTER_SELECT_WALLCLOCK_TIME_SPLIT:
               Value = Peripherals.Counter.LatchedWallclockTimeSplit;
               break;
            default:
