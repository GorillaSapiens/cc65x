...
/* The library header */
static LibHeader Header = {LIB_MAGIC, LIB_VERSION, 0, 0};
...
// is created.
{
   ...
   if (Lib == 0) {
      ...
      if (MustExist) {
         ...
         Print(stdout, 1, "%s: Library '%s' will be created.\n", ProgName,
               Name);
      }
   }
   else {
   ...
   if (NeedTemp) {
      ...
      NewLibName = xmalloc(strlen(Name) + (sizeof(".temp-") - 1) +
...
// filename
{
   ...
   if (NewLib) {
      ...
      if (Lib == 0) {
         Error("Cannot open library '%s' for writing: %s", LibName,
               strerror(errno));
      }
   ...
   if (NewLib && fclose(NewLib) != 0) {
      Error("Problem closing temporary library file '%s': %s", NewLibName,
            strerror(errno));
   }
   if (NewLibName && remove(NewLibName) != 0) {
      Error("Problem deleting temporary library file '%s': %s", NewLibName,
            strerror(errno));
   }
}
