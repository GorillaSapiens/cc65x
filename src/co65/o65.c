...
// header) from the o65 file.
{
   ...
   switch (H->mode & O65_SIZE_MASK) {
      ...
      case O65_SIZE_16BIT:
         ...
         break;
      default:
         Internal("Invalid size field value in o65 header");
   }
   return Size;
}
...
// something is wrong.
{
   ...
   if (H->marker[0] != O65_MARKER_0 || H->marker[1] != O65_MARKER_1) {
      Error("Not an o65 object file: Invalid marker %02X %02X", H->marker[0],
            H->marker[1]);
   }
   ...
   if (memcmp(H->magic, Magic, sizeof(H->magic)) != 0) {
      Error("Not an o65 object file: Invalid magic %02X %02X %02X", H->magic[0],
            H->magic[1], H->magic[2]);
   }
...
// Read relocation data for one segment
{
   ...
   while (1) {
      ...
      R->SymIdx =
          (R->SegID == O65_SEGID_UNDEF) ? ReadO65Size(F, &D->Header) : 0;
      switch (R->Type) {
