...
// will get moved to a lower position.
{
   ...
   --C->Count;
   memmove(C->Items + Index, C->Items + Index + 1,
           (C->Count - Index) * sizeof(void *));
}
...
void CollMoveMultiple(Collection *C, unsigned Start, unsigned Count,
                      unsigned Target)
...
// to higher indices.
{
   ...
   // Check the range
   PRECONDITION(Start < C->Count && Start + Count <= C->Count &&
                Target <= C->Count);
...
// Collections on return.
{
   ...
   // Copy the items
   memcpy(Dest->Items + Dest->Count, Source->Items,
...
void CollSort(Collection *C, int (*Compare)(void *, const void *, const void *),
              void *Data)
